...
rm -f "${package_output_path}/wiznote-macos-u14-${current_date}.dmg"
if [ -e "$package_home" ]; then
        ...
        hdiutil convert -format UDZO "${package_output_path}/tmp.dmg" -o  "${package_output_path}/wiznote-macos-u14-${current_date}.dmg" && \
        rm -f "${package_output_path}/tmp.dmg"
        rm -rf ./${package_home}/WizNote.app
else
