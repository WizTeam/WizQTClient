...
cmake -DWIZNOTE_USE_QT5=YES -DCMAKE_BUILD_TYPE=Release -DPRIVATE_DEPLOYMENT=YES -DCMAKE_PREFIX_PATH=~/Qt5.4.0/5.4/clang_64/lib/cmake ../WizQTClient && \
make -j5 && \
cd ../WizQTClient
...
mv ../WizQTClient-Release-QT5/WizNote.app ../WizQTClient-Release-QT5/中民投笔记.app 

setFile -a V ${package_home}/wiznote-disk-cover.jpg

current_date=`date "+%Y-%m-%d"`
rm -f "${package_output_path}/tmp.dmg"
rm -f "${package_output_path}/wiznote-privatedeploy-${current_date}.dmg"
if [ -e "$package_home" ]; then
	...
	cp -R ../WizQTClient-Release-QT5/中民投笔记.app ${package_home} && \
