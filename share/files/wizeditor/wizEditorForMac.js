...
const xssUtils = require('./libs/xssUtils');

const ENV = require('./config/env');
const Lang = require('./locale/Lang');
const Amend = require('./common/amendUtils/Amend');
const AmendInfo = require('./common/amendUtils/AmendInfo');
const AmendUser = require('./common/amendUtils/AmendUser');
const AmendUserAction = require('./common/amendUtils/AmendUserAction');
const AmendUtils = require('./common/amendUtils/AmendUtils');
const BlockCore = require('./common/blockUtils/BlockCore');
const BlockUtils = require('./common/blockUtils/BlockUtils');
const CodeCore = require('./common/codeUtils/CodeCore');
const CodeStyle = require('./common/codeUtils/CodeStyle');
const CodeUtils = require('./common/codeUtils/CodeUtils');
const CommandExtend = require('./common/CommandExtend');
const DomUtils = require('./common/DomUtils');
const Editor = require('./common/editorCore/Editor');
const EditorEvent = require('./common/editorCore/EditorEvent');
const EditorUtils = require('./common/editorCore/EditorUtils');
const FormatPainter = require('./common/FormatPainter');
const FrameViewCore = require('./common/frameViewUtils/FrameViewCore');
const FrameViewStyle = require('./common/frameViewUtils/FrameViewStyle');
const FrameViewUtils = require('./common/frameViewUtils/FrameViewUtils');
const HighlightUtils = require('./common/HighlightUtils');
const HistoryUtils = require('./common/HistoryUtils');
const ImgClick = require('./common/imgUtils/ImgClick');
const ImgCore = require('./common/imgUtils/ImgCore');
const ImgResize = require('./common/imgUtils/ImgResize');
const ImgUtils = require('./common/imgUtils/ImgUtils');
const LinkUtils = require('./common/LinkUtils');
const MarkdownRender = require('./common/MarkdownRender');
const MathJaxRender = require('./common/MathJaxRender');
const NightModeUtils = require('./common/NightModeUtils');
const PasteUtils = require('./common/PasteUtils');
const RangeUtils = require('./common/RangeUtils');
const Reader = require('./common/readerCore/Reader');
const ReaderEvent = require('./common/readerCore/ReaderEvent');
const SelectPlugin = require('./common/SelectPlugin');
const TabKey = require('./common/TabKey');
const TableCore = require('./common/tableUtils/TableCore');
const TableMenu = require('./common/tableUtils/TableMenu');
const TableUtils = require('./common/tableUtils/TableUtils');
const TableZone = require('./common/tableUtils/TableZone');
const TodoCore = require('./common/todoUtils/TodoCore');
const TodoRouteForClient = require('./common/todoUtils/TodoRouteForClient');
const TodoStyle = require('./common/todoUtils/TodoStyle');
const TodoUtils = require('./common/todoUtils/TodoUtils');
const ToolbarMarkdown = require('./common/frameViewUtils/ToolbarMarkdown');
const WizStyle = require('./common/WizStyle');

const version = '0.1';

const WizDocument = function (documentOptions, coreCallback) {
  const env = new ENV(documentOptions, version);
  ...
  let core = {
    ...
    require: {
      ...
      mathJaxRender: new MathJaxRender(),
      nightModeUtils: new NightModeUtils(),
      ...
      todoCore: new TodoCore(),
      todoRouteForClient: new TodoRouteForClient(),
      todoStyle: new TodoStyle(),
      todoUtils: new TodoUtils(),
      toolbarMarkdown: new ToolbarMarkdown(),
      wizStyle: new WizStyle(),
    },
  };

  for (let key of Object.keys(core.require)) {
    let lib = core.require[key];
    lib.initCore(core);
    delete lib.initCore;
  }

  const reader = core.require.reader;
  const editor = core.require.editor;
  const editorEvent = core.require.editorEvent;
  const amend = core.require.amend;
  const amendInfo = core.require.amendInfo;
  const amendUser = core.require.amendUser;
  const commandExtend = core.require.commandExtend;
  const codeCore = core.require.codeCore;
  const domUtils = core.require.domUtils;
  const editorUtils = core.require.editorUtils;
  const formatPainter = core.require.formatPainter;
  const highlightUtils = core.require.highlightUtils;
  const historyUtils = core.require.historyUtils;
  const imgUtils = core.require.imgUtils;
  const linkUtils = core.require.linkUtils;
  const markdownRender = core.require.markdownRender;
  const mathJaxRender = core.require.mathJaxRender;
  const nightModeUtils = core.require.nightModeUtils;
  const pasteUtils = core.require.pasteUtils;
  const rangeUtils = core.require.rangeUtils;
  const tableCore = core.require.tableCore;
  const todoCore = core.require.todoCore;
  const toolbarMarkdown = core.require.toolbarMarkdown;
  const wizStyle = core.require.wizStyle;
  ...
  let bodyClassList = env.body.className.split(' ');
  let bodyClass;
  ...
  let documentInstance = {
    version: version,
    destroy: () => {
      if (window.WizTemplate) {
        window.WizTemplate.off();
      }
      editor.off();
      reader.off();
      env.readonly = null;
    ...
    editor: {
      on: (options, callback) => {
        ...
        let scrollTopLast = domUtils.getPageScroll().top;
          ...
          setTimeout(() => {
            if (!env.client.type.isPhone || !env.client.type.isPad) {
              if (env.client.type.isMac && env.options.reader.type !== CONST.NOTE_READER_TYPE.COMMON) {
                setTimeout(function () {
                  env.doc.documentElement.scrollTop = scrollTopLast;
                }, 100);
              } else {
                env.doc.documentElement.scrollTop = scrollTopLast;
              }
        ...
        options = options || {};
        let docBody = options.documentBody;
        // todo 非 markdown 笔记需要考虑 header 的部分
        if (typeof docBody === 'string') {
          docBody = xssUtils.xssFilter(docBody);
          env.body.innerHTML = docBody;
        }
        editor.on(options, _callback);
      },
      /**
       * 备份光标位置
       */
       ...
       * 在编辑时，查找指定文本
       */
      find: (str, matchcase, searchBackward, loop) => {
      ...
      getContentHtml: (options) => {
        ...
        domUtils.clearChild(env.body, excludeList);
       ...
       */
      insertB64Html: (b64Html, notInsertEmptyDiv) => {
        editorUtils.insertHtml(base64.decode(b64Html), notInsertEmptyDiv);
      },
      /**
       * 在光标位置插入 html
       */
      insertHtml: (html, notInsertEmptyDiv) => {
      ...
      isModified: () => {
        ...
        //     console.log('\n\n\n---------------------------------------------------------------------------------------------------------------------------\n\n\n');
        // }
        let result = env.readonly === false;
        if (!result) {
          return result;
        }
        let curHtml = domUtils.getContentHtml();
        let originalHtml = editor.getOriginalHtml();
       ...
       * 修改光标选中文本的样式 和 属性
       */
      modifySelectionDom: (style, attr) => {
        editorUtils.modifySelectionDom(style, attr);
      },
      paste: (html, txt) => {
        pasteUtils.pasteFromClient(html, txt);
      },
      pasteB64: (b64Html, b64Txt) => {
        documentInstance.editor.paste(base64.decode(b64Html), base64.decode(b64Txt));
      },
      /**
       * 编辑器 redo
       */
      ...
      removeFormat: (removeAll, isRemoveColor, isRemoveAllStyles) => {
        wizStyle.removeFormat(removeAll, isRemoveColor, isRemoveAllStyles);
      },
      replace: (from, to, matchcase) => {
        return editorUtils.replace(from, to, matchcase);
      },
      replaceAll: (from, to, matchcase) => {
    ...
    reader: {
      on: (options, callback) => {
        ...
        if (options && options.reader && options.reader.type) {
          env.options.reader.type = options.reader.type;
        }
        env.options.pc.pluginModified = false;
        let docBody = options && options.documentBody;
        // todo 非 markdown 笔记需要考虑 header 的部分
        if (typeof docBody === 'string') {
          docBody = xssUtils.xssFilter(docBody);
        ...
        } else if (env.options.reader.type === CONST.NOTE_READER_TYPE.MATHJAX) {
      ...
      getWordCount: () => {
        ...
        let count = utils.getWordCount(text);
        return JSON.stringify(count);
      },
      setPluginModify: (flag) => {
    ...
    insertCustomStyle: (id, customCss, isTemp) => {
      wizStyle.insertCustomStyle(id, customCss, isTemp);
    },
    insertDefaultStyle: (onlyReplace, customCss) => {
  ...
  function initFrame() {
    let toolbarContainer, bodyContainer, iframe, body;
    let editorContainer = env.options.container;
    ...
    if (!env.options.noFrame) {
      ...
      if (!iframe) {
        ...
        iframe.src = 'javascript:void(function(){document.open();document.write("<!DOCTYPE html><html><head></head><body><div><br/></div></body></html>");document.close();}())';
...
WizDocument.version = version;

module.exports = WizDocument;

},{"./common/CommandExtend":6,"./common/DomUtils":7,"./common/FormatPainter":8,"./common/HighlightUtils":9,"./common/HistoryUtils":10,"./common/LinkUtils":11,"./common/MarkdownRender":12,"./common/MathJaxRender":13,"./common/NightModeUtils":14,"./common/PasteUtils":15,"./common/RangeUtils":16,"./common/SelectPlugin":17,"./common/TabKey":18,"./common/WizStyle":19,"./common/amendUtils/Amend":20,"./common/amendUtils/AmendInfo":21,"./common/amendUtils/AmendUser":22,"./common/amendUtils/AmendUserAction":23,"./common/amendUtils/AmendUtils":24,"./common/blockUtils/BlockCore":25,"./common/blockUtils/BlockUtils":26,"./common/codeUtils/CodeCore":27,"./common/codeUtils/CodeStyle":28,"./common/codeUtils/CodeUtils":29,"./common/editorCore/Editor":30,"./common/editorCore/EditorEvent":31,"./common/editorCore/EditorUtils":32,"./common/frameViewUtils/FrameViewCore":33,"./common/frameViewUtils/FrameViewStyle":34,"./common/frameViewUtils/FrameViewUtils":35,"./common/frameViewUtils/ToolbarMarkdown":36,"./common/imgUtils/ImgClick":37,"./common/imgUtils/ImgCore":38,"./common/imgUtils/ImgResize":39,"./common/imgUtils/ImgUtils":40,"./common/readerCore/Reader":41,"./common/readerCore/ReaderEvent":42,"./common/tableUtils/TableCore":43,"./common/tableUtils/TableMenu":44,"./common/tableUtils/TableUtils":45,"./common/tableUtils/TableZone":46,"./common/todoUtils/TodoCore":47,"./common/todoUtils/TodoRouteForClient":48,"./common/todoUtils/TodoStyle":49,"./common/todoUtils/TodoUtils":50,"./config/const":51,"./config/env":52,"./libs/base64":56,"./libs/utils":59,"./libs/xssUtils":61,"./locale/Lang":62}],6:[function(require,module,exports){
/**
 * 代码区域操作核心包 core
 */
const CONST = require('../config/const');
...
const CommandExtend = function () {
  let core = null;
  let env = null;
  let domUtils = null;
  let historyUtils = null;
  let rangeUtils = null;
  let todoUtils = null;
  ...
  let getHBlockList = () => {
    ...
    let start = rangeUtils.getRangeDetail(range.startContainer, range.startOffset).container;
    let end = rangeUtils.getRangeDetail(range.endContainer, range.endOffset).container;
    let startBlock = domUtils.getBlockParent(start, true);
    if (startBlock && startBlock !== env.body) {
      start = startBlock;
    }
    let parent = (start === end) ? start : domUtils.getParentRoot([start, end]);
    if (parent !== env.body) {
    ...
    let startIndex = domUtils.getIndexList(start).join('.');
    let endIndex = domUtils.getIndexList(end).join('.');
    for (let i = 1; i < 7; i++) {
  ...
  let getBlockList = () => {
    ...
    let end = rangeUtils.getRangeDetail(range.endContainer, range.endOffset).container;
    let startBlock = domUtils.getBlockParent(start, true);
    let endBlock = domUtils.getBlockParent(end, true);
    let endIndex = domUtils.getIndexList(end).join('.');

    let target = startBlock, next;
    let childState;
    let isBlock, isEmptyText,
  ...
  let patchExecForFormatBlock = (blockType) => {
    let range = rangeUtils.getRange();
    if (!range) {
      return;
    }
    let start = rangeUtils.getRangeDetail(range.startContainer, range.startOffset);
    let end = rangeUtils.getRangeDetail(range.endContainer, range.endOffset);
    let prev = domUtils.getPreviousNode(start.container, false) || env.body;
    let next = domUtils.getNextNode(end.container, false) || env.body;

    let hList = getHBlockList();
   ...
   */
  let patchExecForSubAndSup = (command) => {
   ...
   */
  let patchQueryForSubAndSup = (command) => {
    let tagName = command === 'subscript' ? 'sub' : 'sup';
    // 设置 sub || sup
    let range = rangeUtils.getRange();
    let start;
    if (range) {
  ...
  let _event = {
    ...
    handler: {
      onExecCommand: (...args) => {
        this.execCommand.apply(this, args);
      }
    }
  };
  ...
  this.clearSubSup = () => {
    let rangeResult = rangeUtils.getRangeDomList({
      noSplit: false
    });
    let p;
    let range, span;
    ...
    let firstP = domUtils.getParentByTagName(rangeResult.list[0], ['sub', 'sup'], true);
    // 只有一个 dom
    ...
    let lastP = domUtils.getParentByTagName(rangeResult.list[rangeResult.list.length - 1], ['sub', 'sup'], true);
    if (lastP) {
      domUtils.splitDomAfterSub(lastP, rangeResult.list[rangeResult.list.length - 1]);
    }

    let isPeelSub = false;
    for (let i = 0; i < rangeResult.list.length; i++) {
      let dom = rangeResult.list[i];
  ...
  this.execCommand = (...args) => {
    let range = rangeUtils.getRange();
    let startTodoMain, startTodoCheckbox, startTodoId;
    if (range && range.collapsed) {
  ...
  this.queryCommandState = (command) => {
    let result;
...
},{"../config/const":51,"../libs/utils":59}],7:[function(require,module,exports){
/**
...
const DomUtils = function () {
   ...
   * 添加 class name
   */
  this.addClass = (domList, className) => {
    if (!domList) {
      return;
    }
    if (!!domList.nodeType) {
   ...
   * 将 dom 插入到 target 后
   */
  this.after = (newDom, target) => {
    this.before(newDom, target, true);
  };

  /**
   * 给 DOM.getAttribute('style') 对象 设置 样式
   */
  this.appendStyle = (styleStr, styleObj) => {
    if (!styleStr) {
      return;
    }
    let styleList = styleStr.split(';'), i, j, t;
   ...
   */
  this.before = (newDom, target, isAfter) => {
    isAfter = !!isAfter;
    if (!target || !newDom) {
      return;
    }
    let isBody = target === env.body,
    ...
    if (!utils.isArray(newDom)) {
      ...
      last = nextDom;
      for (let i = newDom.length - 1; i >= 0; i--) {
        let d = newDom[i];
  ...
  this.canEdit = (dom) => {
    //过滤 script、style等标签
    let filterTag = ['script', 'style'];
    return dom && (dom.nodeType === 1 || dom.nodeType === 3) &&
      (this.isTag(dom, 'br') || !this.isEmptyDom(dom)) &&
      !(this.getParentByTagName(dom, CONST.TAG.TMP_TAG, true, null)) &&
      !((dom.nodeType === 1 && this.isTag(dom, filterTag)) ||
        (dom.nodeType === 3 && dom.parentNode && this.isTag(dom.parentNode, filterTag)));
  };

  /**
   * 清理 dom 内无用的 childNodes（主要用于 处理 剪切板的 html）
   */
  this.childNodesFilter = (dom) => {
    if (!dom || dom.nodeType !== 1) {
      return;
    }
    for (let i = dom.childNodes.length - 1; i >= 0; i--) {
      let d = dom.childNodes[i];
      if (d.nodeType === 1) {
        if (/link|style|script|meta/ig.test(d.nodeName)) {
          dom.removeChild(d);
        }
        this.childNodesFilter(d);
   ...
   */
  this.clearChild = (dom, excludeList) => {
    if (!dom) {
      return;
    }
    let isExclude = excludeList.indexOf(dom) >= 0;
    let str;
    if (!isExclude && dom.nodeType === 3 && !this.isUsableTextNode(dom) &&
      !this.getParentByTagName(dom, 'pre', false)) {
      // pre 内不进行清理
      this.remove(dom);
      return;
    ...
    if (!isExclude && dom.nodeType === 1) {
      let ns = dom.childNodes, i, item;
      for (i = ns.length - 1; i >= 0; i--) {
        item = ns[i];
        this.clearChild(item, excludeList);
      }
      this.mergeChildSpan(dom, excludeList);

      if (excludeList.indexOf(dom) < 0 &&
        dom.childNodes.length === 0 && dom.nodeType === 1 && !this.isSelfClosingTag(dom) &&
//                    dom.tagName.toLowerCase() == 'span' && !!dom.getAttribute(CONST.ATTR.SPAN)) {
   ...
   */
  this.clearStyle = (dom, styleKey, isSingle) => {
    let removeStyle = (_dom, _styleKey) => {
      _dom.style[styleKey] = '';
      if (this.isTag(_dom, 'font') && _styleKey === 'color') {
   ...
   */
  this.clearStyleFromHtml = (html, whiteList) => {
    let reg = /(<[^<>]* style=(['"]))((\r?\n|(?!(\2|[<>])).)*)/ig;
    ...
    let whiteListReg = whiteList.length > 0 ? new RegExp('^(' + whiteList.join('|') + ')', 'i') : null;

    html = html.replace(reg, (m0, m1, m2, m3) => {
      let result = [m1];
      // 先还原特殊字符，处理后再替换回去 例如：
      ...
      m3 = m3.replace(/&quot;/ig, '"');
      let styleList = m3.split(';'), style,
        index, k, v;
      for (let i = 0, j = styleList.length; i < j; i++) {
        style = styleList[i];
        index = style.indexOf(':');
        k = '';
        v = '';
        ...
        if (k && v && whiteListReg && whiteListReg.test(k)) {
          ...
          if (!/^(inherit|initial|transparent)$/i.test(v)) {
   ...
   * 复制 dom
   */
  this.clone = (dom, excludeInner) => {
    let result, tmp, attList, atts, i, j, k;
    if (!dom) {
      return null;
    }
    if (excludeInner) {
   ...
   * 比较 IndexList
   */
  this.compareIndexList = (a, b) => {
    let j = Math.min(a.length, b.length);
    for (let i = 0; i < j; i++) {
      let x = a[i];
      let y = b[i];
      if (x < y) {
   ...
   */
  this.convertImageToBase64 = (src, width, height, callback) => {
    ...
    img.onload = () => {
      let canvas = env.doc.createElement("canvas");
      canvas.width = width;
   ...
   */
  this.createSpan = () => {
    let s = env.doc.createElement('span');
   ...
   */
  this.css = (dom, style) => {
    if (!dom || !style || this.isTag(dom, 'br')) {
      //禁止给 br 添加任何样式
      return;
    }
    let targetStyle = {};
   ...
   */
  this.fixFontSize = () => {
    let domList = env.body.querySelectorAll('[style]');
    let fontSizeReg = /font-size\s*:\s*([%.\w]*)/i,
      result;
    for (let i = 0; i < domList.length; i++) {
      ...
      if (!this.isTag(dom, 'html')) {
        dom.style.fontSize = this.getFontSizeRem(result[1], {
          useRootSize: true
        });
      }
    }
   ...
   */
  this.fixOrderList = () => {
    let orderList = env.body.querySelectorAll('ol');
    for (let i = 0; i < orderList.length; i++) {
      ...
      while (p) {
        level++;
        p = this.getParentByTagName(p, ['ul', 'ol'], false);
      }
      this.removeClass(ol, CONST.CLASS.ORDER_LIST_LEVEL);
      this.addClass(ol, CONST.CLASS.ORDER_LIST_LEVEL[level % 3]);
    }
  };
   ...
   */
  this.fragmentFilterForCopy = (fragment) => {
    // 清理 codeMirror 内多余元素
    let removeList = (list) => {
   ...
   */
  this.getBlockParent = (dom, includeSelf) => {
    if (!dom) {
      return null;
    }
    return this.getParentByFilter(dom, (obj) => {
      return this.isBlock(obj);
    }, includeSelf);
  };
   ...
   */
  this.getComputedStyle = (dom, name, includeParent) => {
    ...
    while (!value) {
      ...
      let s = env.win.getComputedStyle(dom, null);
      value = s[name] || '';
   ...
   * 获取当前页面源码
   */
  this.getContentHtml = (options) => {
    // PC 客户端使用的 Chrome 引擎在 设置列表后，会自动出现 font-size 的样式，必须要进行修正
    this.fixFontSize();

    let isSaveTemp = options && options.isSaveTemp;
    env.event.call(CONST.EVENT.BEFORE_GET_DOCHTML, null);

    //将 input text、textarea 的内容设置到 html 内
    let obj;
    let objList = env.body.getElementsByTagName('input');
    ...
    for (let i = 0, j = objList.length; i < j; i++) {
      obj = objList[i];
      this.setTextarea(obj);
    }
    //处理 table 容器内 table 之外的内容
    objList = env.body.querySelectorAll('.' + CONST.CLASS.TABLE_CONTAINER);
    for (let i = 0, j = objList.length; i < j; i++) {
      this.moveOutFromTableContainer(objList[i]);
    }

    let tmpStyle = [CONST.NAME.TMP_STYLE];
    if (!isSaveTemp) {
    ...
    if (env.options.container && env.options.noFrame) {
      if (env.options.editor.type === CONST.NOTE_EDITOR_TYPE.MARKDOWN) {
        ...
        // todo 普通笔记不适合采用 noFrame 方式
        content = '';
        throw new Error('普通笔记不适合采用 noFrame 方式');
      }
    } else {
    ...
    content = this.removeStyleByNameFromHtml(content, tmpStyle);
    ...
    */
    ...
    let bodyReg = /(<[\w]*[^<>]*[ ]+)contenteditable([ ]*=[ ]*['"][^'"<>]*['"])?/ig;
   ...
   * 根据字号获取 rem 值
   */
  this.getFontSizeRem = (fontSize, _options) => {
    ...
    let s = env.win.getComputedStyle(env.doc.body.parentNode),
      pxRoot = parseFloat(s.fontSize);
    // 粘贴操作时， pt、px 都按照当前 根字体大小计算 em
    // 其他情况 pt 单位按照 默认字体大小计算 em（因为客户端设置字体的数字都是根据默认大小传递的）
    let ptRoot = options.useRootSize ? defaultFontSize * this.getRootSizeRate() : defaultFontSize;
   ...
   * 根据 dom 树索引集合 获取 dom
   */
  this.getFromIndexList = (indexList) => {
    if (!indexList || indexList.length === 0) {
      return null;
    }
    let d = env.body, offset;
    try {
      for (let i = 0, j = indexList.length - 1; i < j; i++) {
  ...
  this.getImageData = (img) => {
    let size = this.getImageSize(img.src);
   ...
   */
  this.getIndexList = (dom) => {
    let e = dom, indexList = [];
    while (e && !this.isBody(e)) {
      indexList.splice(0, 0, this.getIndex(e));
      e = e.parentNode;
    }
    return indexList;
  };
  ...
  this.getInnerText = (ele) => {
    ...
    let sty = (n, prop) => {
      ...
      let s = n.currentStyle || n.ownerDocument.defaultView.getComputedStyle(n, null);
      if (n.tagName === "SCRIPT") {
        return "none";
      }
      if (!s[prop]) {
    ...
    let blockTypeNodes = "table-row,block,list-item";
    let isBlock = (n) => {
      // 判断是否为block元素
      let s = sty(n, "display") || "feaux-inline";
   ...
   */
  this.getListA2B = (options) => {
    let startDom = options.startDom,
    ...
    if (startDom !== endDom && endOffset === 0) {
      endDom = this.getPreviousNode(endDom, false, startDom);
      if (!endDom) {
        endDom = startDom;
      }

      if (this.isSelfClosingTag(endDom)) {
        //如果 修正后的 endDom 为 自闭合标签， 需要特殊处理
        endOffset = 1;
      } else {
        endOffset = this.getEndOffset(endDom);
      }
    }
    ...
    if (startDom === endDom && startOffset !== endOffset) {
      ...
      if (isText && !startDom.parentNode.getAttribute(CONST.ATTR.SPAN_DELETE) &&
        !noSplit) {
        startDom = this.splitRangeText(startDom, startOffset, endOffset, false);
        endDom = startDom;
        changeStart = true;
        changeEnd = true;
      } else if (startDom.nodeType === 1 &&
        startDom.childNodes.length > 0 && !this.isSelfClosingTag(startDom)) {
        startDom = startDom.childNodes[startOffset];
      ...
      if (startDom.nodeType === 3 && !startDom.parentNode.getAttribute(CONST.ATTR.SPAN_DELETE) &&
        !noSplit) {
        startDom = this.splitRangeText(startDom, startOffset, null, false);
        changeStart = true;
      ...
      if (endDom.nodeType === 3 && endOffset > 0 &&
        !endDom.parentNode.getAttribute(CONST.ATTR.SPAN_DELETE) && !noSplit) {
        endDom = this.splitRangeText(endDom, 0, endOffset, true);
        changeEnd = true;
      } else if (!this.isSelfClosingTag(endDom) && endDom.nodeType === 1 && endOffset > 0) {
        endDom = this.getLastDeepChild(endDom.childNodes[endOffset - 1]);
        changeEnd = true;
      }
    }
    if (changeStart) {
    ...
    //make the array
    let curDom = startDom, result = [];
    if (startOffset === startDom.length) {
      curDom = this.getNextNode(curDom, false, endDom);
    }
    ...
    let startDomBak = this.getPreviousNode(result[0], false, null),
      endDomBak = this.getNextNode(result[result.length - 1], false, null);
    ...
    let startOffsetBak = this.getEndOffset(startDomBak),
      endOffsetBak = 0;
   ...
   * endIndex    //1.4.2
   */
  this.getListFromTagAndIndex = (tagName, container, startIndex, endIndex) => {
    let result = [];
    let tagList = container.querySelectorAll(tagName);
    for (let i = 0; i < tagList.length; i++) {
      let tag = tagList[i];
      let tagIndex = this.getIndexList(tag).join('.');
   ...
   */
  this.getNextBlock = (tmpEnd) => {
    let isNext = false, tmp;

    if (this.isBlock(tmpEnd) || this.isTag(tmpEnd, 'br')) {
      return tmpEnd;
    }
   ...
   */
  this.getNextNode = (dom, onlyElement, endDom) => {
    let originalDom = dom;
    if (!dom || dom === endDom) {
      return null;
    }
   ...
   */
  this.getNextNodeCanEdit = (dom, onlyElement, endDom) => {
    dom = this.getNextNode(dom, onlyElement, endDom);
    while (dom && !this.canEdit(dom)) {
      dom = this.getNextNode(dom, onlyElement, endDom);
    }
    return dom;
  };
   ...
   */
  this.getParentByClass = (node, className, includeSelf) => {
    return this.getParentByFilter(node, (dom) => {
      return this.hasClass(dom, className);
    }, includeSelf);
  };
   ...
   */
  this.getParentByFilter = (node, filterFn, includeSelf) => {
   ...
   */
  this.getParentByTagName = (node, tagNames, includeSelf, excludeFn) => {
    if (!node) {
      return null;
    }
    tagNames = utils.listToMap(utils.isArray(tagNames) ? tagNames : [tagNames]);
    return this.getParentByFilter(node, (node) => {
  ...
  this.getParentList = (obj) => {
    ...
    while (!!p && p !== env.body &&
    !this.hasClass(p, CONST.CLASS.WIZ_BODY)) {
      list.splice(0, 0, p);
      p = p.parentNode;
    }
    return list;
   ...
   * 获取多个 dom 共同的父节点
   */
  this.getParentRoot = (domList) => {
    if (!domList || domList.length === 0) {
      return null;
    }
    let tmpIdx, pNode, parentList = [];
    pNode = domList[0].nodeType === 1 ? domList[0] : domList[0].parentNode;
    while (pNode && !this.isBody(pNode)) {
      parentList.push(pNode);
      pNode = pNode.parentNode;
    }
    for (let i = 1, j = domList.length; i < j; i++) {
      pNode = domList[i];
      while (pNode) {
   ...
   */
  this.getPrevBlock = (tmpStart) => {
    let isPre = false, tmp;
    if (this.isBlock(tmpStart) || this.isTag(tmpStart, 'br')) {
      return tmpStart;
    }
    while (tmpStart) {
   ...
   */
  this.getPreviousNode = (dom, onlyElement, startDom) => {
    let originalDom = dom;
    if (!dom || dom === startDom) {
      return null;
    }
   ...
   * 获取 DOM 的 下一个 可编辑的叶子节点
   */
  this.getPreviousNodeCanEdit = (dom, onlyElement, endDom) => {
    dom = this.getPreviousNode(dom, onlyElement, endDom);
    while (dom && !this.canEdit(dom)) {
      dom = this.getPreviousNode(dom, onlyElement, endDom);
    }
    return dom;
   ...
   */
  this.getTab = () => {
    let x = env.doc.createElement('span');
    x.innerHTML = '&nbsp;&nbsp;&nbsp;&nbsp;';
   ...
   */
  this.getWizAmendParent = (dom) => {
    return this.getParentByFilter(dom, (node) => {
      //node.childNodes.length == 0 时，键盘敲入的字符加在 span 外面
      return ( node && node.nodeType === 1 &&
        (node.getAttribute(CONST.ATTR.SPAN_INSERT) ||
   ...
   */
  this.hideCodeFromHtml = (html) => {
    let regHide = /(<div [^<>]*)(CodeMirror-activeline-background|CodeMirror-cursors|CodeMirror-selected|CodeMirror-hscrollbar|CodeMirror-vscrollbar)([^<>]*>)/ig;
    html = html.replace(regHide, '$1 wiz-hide wiz_$2$3');
    let regNoClass = /(<span [^<>]*)(CodeMirror-matchingbracket)([^<>]*>)/ig;
   ...
   */
  this.hideTableFromHtml = (html) => {
    //做 RegExp 做 test 的时候 不能使用 g 全局设置， 否则会影响 索引
    let regexForTest = /(<[^<> ]*[^<>]* class[ ]*=[ ]*['"])([^'"]*)(['"])/i;
    let regex = /(<[^<> ]*[^<>]* class[ ]*=[ ]*['"])([^'"]*)(['"])/ig;
    if (!regexForTest.test(html)) {
      return html;
    }

    let result = [], m, lastIndex = 0,
      str, reg;
  ...
  this.html2Markdown = (container, options) => {
    let dom, childNodes, str;
    options = options || {};
    ...
    if (!options.isInA) {
      options.isInA = this.isTag(container, 'a');
    }

    childNodes = container.childNodes;
    for (let i = childNodes.length - 1; i >= 0; i--) {
      this.html2Markdown(childNodes[i], options);
    }

    if (this.isTag(container, 'a')) {
      str = container.innerText;
      dom = env.doc.createElement('span');
      dom.innerText = '[' + str + '](' + container.href + ')';
      this.before(dom, container);
      this.remove(container);
    } else if (this.isTag(container, 'img')) {
      str = container.alt || utils.getFileNameByUrl(container.src);
      dom = env.doc.createElement('span');
      dom.innerText = '![' + str + '](' + container.src + ')';
      this.before(dom, container);
      this.remove(container);
    } else if (this.isTag(container, ['h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'li'])) {
      str = container.innerText;
      dom = env.doc.createElement('div');
      dom.innerText = str;
      this.before(dom, container);
      this.remove(container);
    } else if (this.isTag(container, ['ol', 'ul'])) {
      while(container.lastChild) {
        this.after(container.lastChild, container);
      }
      this.remove(container);
    }
  };
  ...
  this.isBlock = (dom) => {
    ...
    if (this.isWizDom(dom)) {
      // 对于 Wiz 专用 tag 全部认为是 block dom
      return true;
    }
    let displayValue = this.getComputedStyle(dom, 'display', false);
  ...
  this.isEmptyDom = (dom) => {
    ...
    if (dom.childNodes.length === 0) {
      return this.isTag(dom, 'br') || !this.isSelfClosingTag(dom);
    }

    for (let i = 0, j = dom.childNodes.length; i < j; i++) {
      if (!this.isEmptyDom(dom.childNodes[i])) {
        return false;
      }
    }
    return true;
   ...
   */
  this.isFillChar = (node, isInStart) => {
    return node.nodeType === 3 &&
      !node.nodeValue.replace(new RegExp((isInStart ? '^' : '') + CONST.FILL_CHAR), '').length;
  };

  this.isParent = (target, parent) => {
   ...
   */
  this.isSameAttr = (n, m) => {
    let attrA = n.attributes,
      attrB = m.attributes;
    if (attrA.length !== attrB.length) {
      return false;
    }
    for (let i = 0, j = attrA.length; i < j; i++) {
      let a = attrA[i];
      if (a.name === 'style') {
        continue;
      }
  ...
  this.isSameSpan = (n, m) => {
    return !!n && !!m && n.nodeType === 1 && m.nodeType === 1 &&
      this.isTag(n, 'span') && n.tagName === m.tagName &&
      n.getAttribute(CONST.ATTR.SPAN) === CONST.ATTR.SPAN &&
      this.isSameStyle(n, m) && this.isSameAttr(n, m);
  };

  /**
   * 判断 dom 的 style （inline）是否相同
   */
  this.isSameStyle = (n, m) => {
    let styleA = {};
    let styleB = {};
    this.appendStyle(n.getAttribute('style'), styleA);
    this.appendStyle(m.getAttribute('style'), styleB);
    for (let k in styleA) {
   ...
   */
  this.isSelfClosingTag = (node) => {
    let selfLib = /^(area|base|br|col|command|embed|hr|img|input|keygen|link|meta|param|source|track|wbr)$/i;
  ...
  this.isTag = (dom, tagNames) => {
    ...
    for (let i = 0, j = tagNames.length; i < j; i++) {
   ...
   */
  this.mergeAtoB = (objA, objB, isOverlay) => {
    this.mergeStyleAToB(objA, objB, isOverlay);
    this.mergeAttrAtoB(objA, objB, isOverlay);
  };
  ...
  this.mergeChildSpan = (dom, excludeList) => {
    ...
    for (let i = 0, j = dom.children.length; i < j; i++) {
      this.mergeChildSpan(dom.children[i], excludeList);
    }
    this.mergeSiblingSpan(dom, excludeList);

    let n = dom.children[0], tmp;
    if (!!n && excludeList.indexOf(n) < 0 && dom.childNodes.length === 1 &&
      dom.getAttribute(CONST.ATTR.SPAN) === CONST.ATTR.SPAN &&
      n.getAttribute(CONST.ATTR.SPAN) === CONST.ATTR.SPAN) {
      this.mergeChildToParent(dom, n);
    } else {
      while (!!n) {
        if (excludeList.indexOf(n) < 0 && excludeList.indexOf(dom) < 0 && this.isSameSpan(dom, n)) {
   ...
   */
  this.mergeChildToParent = (parent, child) => {
    if (!parent || !child || child.parentNode !== parent) {
      return;
    }
    while (child.childNodes.length > 0) {
      this.before(child.childNodes[0], child);
    }
    this.mergeAtoB(parent, child, false);
    this.mergeAtoB(child, parent, true);
   ...
   * @param excludeList
   */
  this.mergeSiblingSpan = (parentDom, excludeList) => {
    let n = parentDom.childNodes[0], m, tmp;
    if (!n) {
      return;
    }
    while (n) {
      m = n.nextSibling;
      if (m && excludeList.indexOf(m) < 0 && excludeList.indexOf(n) < 0 && this.isSameSpan(n, m)) {
  ...
  this.mergeStyleAToB = (objA, objB, isOverlay) => {
    ...
    let sA = objA.getAttribute('style'),
    ...
    let styleObj = {};
    if (!!isOverlay) {
      this.appendStyle(sB, styleObj);
      this.appendStyle(sA, styleObj);
    } else {
      this.appendStyle(sA, styleObj);
      this.appendStyle(sB, styleObj);
    }
  ...
  this.modifyChildNodesStyle = (dom, style, attr) => {
    if (!dom) {
      return;
    }
    let ns = dom.childNodes, done = false;
    for (let i = 0; i < ns.length; i++) {
      let item = ns[i];
      if (!done && this.isUsableTextNode(item)) {
        done = true;
        this.modifyStyle(dom, style, attr);
      } else if (item.nodeType === 1) {
        this.modifyChildNodesStyle(item, style, attr);
      }
    }
  };
   ...
   */
  this.modifyNodeStyle = (item, style, attr, isLast) => {
    if (item.nodeType === 1) {
      if (this.isSelfClosingTag(item)) {
        this.modifyStyle(item, style, attr);
      } else {
        this.modifyChildNodesStyle(item, style, attr);
      }

    } else if (this.isUsableTextNode(item)) {
      item = this.splitRangeText(item, null, null, isLast);
      this.modifyStyle(item, style, attr);
    }
    return item;
  };
   ...
   */
  this.modifyNodesStyle = (domList, style, attr) => {
    if (domList.length === 0) {
      return;
    }
    for (let i = 0, j = domList.length; i < j; i++) {
      let item = domList[i];
      domList[i] = this.modifyNodeStyle(item, style, attr, i === j - 1);
    }
  };
   ...
   */
  this.modifyNodesStyleAndClear = (domList, style, attr, excludeList) => {
    //modify style
    this.modifyNodesStyle(domList, style, attr);
    ...
    t = this.getParentRoot(ps);
    //如果是 修订节点，则找到修订节点的 父节点进行清理操作
    tempAmend = this.getWizAmendParent(t);
    t = tempAmend ? tempAmend.parentNode : t;
    this.clearChild(t, excludeList);
  };
   ...
   */
  this.modifyStyle = (dom, style, attr) => {
    let isSelfClosingTag = this.isSelfClosingTag(dom);
    let isTodoCheckbox = this.hasClass(dom, CONST.CLASS.TODO_CHECKBOX);
    if (isTodoCheckbox) {
    ...
    if (!!dom && !isSelfClosingTag &&
      (!this.isTag(dom, 'span') || dom.getAttribute(CONST.ATTR.SPAN) !== CONST.ATTR.SPAN)) {
      d = this.createSpan();
    ...
    if (hasClearList.length > 0) {
      ...
      while (tmpP) {
        ...
        if (this.isBlock(tmpP) || this.hasClass(tmpP, CONST.CLASS.TODO_MAIN)) {
          break;
        }
        if (this.isTag(tmpP, 'a')) {
          tmpA = tmpP;
        }
        p = tmpP;
        tmpP = p.parentNode;
      ...
      if (!tmpA) {
        this.splitDomSingle(p, d);
      } else if (tmpA === p) {
        filterForA(tmpA);
        this.splitDomSingle(tmpA.firstChild, d);
      } else {
        this.splitDomSingle(p, tmpA);
        filterForA(tmpA);
        this.splitDomSingle(tmpA.firstChild, d);
      }
    }
   ...
   */
  this.moveOutFromTableContainer = (container) => {
    if (!container) {
      return;
    }

    let move = (mainDom, _container) => {
      if (!mainDom || !_container) {
        return;
      }

      let isInContainer = mainDom === _container;
      let childList, dom, target, i,
      ...
      for (i = childList.length - 1; i >= 0; i--) {
        ...
        if (dom.nodeType === 1 &&
          ((isInContainer && this.hasClass(dom, CONST.CLASS.TABLE_BODY)) ||
            (!isInContainer && this.isTag(dom, ['table', CONST.TAG.TMP_TAG])))) {
          if (!hasTableChild) {
            // hasTableChild 必须要进行判断，
            // 否则由于用户反复编辑有可能会出现 一个 container 下有多个 tableBody 的情况，从而导致渲染后 table 消失
            if (this.hasClass(dom, CONST.CLASS.TABLE_BODY) || this.isTag(dom, 'table')) {
              before = true;
              hasTableChild = true;
            }
            continue;
  ...
  this.peelDom = (dom, checkFun) => {
    ...
    let childNodes, child, i;
    childNodes = dom.childNodes;
    for (i = childNodes.length - 1; i >= 0; i--) {
      child = childNodes[i];
      if (!checkFun || checkFun(child)) {
        this.after(child, dom);
        if (!result.start) {
          result.start = child;
          result.end = child;
        } else {
   ...
   */
  this.peelTag = (tagName) => {
    let list = env.body.querySelectorAll(tagName),
      i, tag;
    for (i = list.length - 1; i >= 0; i--) {
      tag = list[i];
      while (tag.firstChild) {
        this.before(tag.firstChild, tag);
      }
      this.remove(tag);
    }
  };
   ...
   */
  this.peelTagFromHtml = (html, tag) => {
    let reg = new RegExp('<' + tag + '( [^>]*)?>|<\/' + tag + '>', 'ig');
   ...
   * 移除 class name
   */
  this.removeClass = (domList, className) => {
    if (!domList) {
      return;
    }
    if (!!domList.nodeType) {
    ...
    for (let i = domList.length - 1; i >= 0; i--) {
      ...
      if (dom.nodeType === 1) {
        ...
        for (let j = className.length; j >= 0; j--) {
          let css = className[j];
  ...
  this.removeByName = (name) => {
    let s = env.doc.getElementsByName(name);
  ...
  this.removeByTag = (tag) => {
    let s = env.doc.getElementsByTagName(tag);
    for (let i = s.length - 1; i >= 0; i--) {
      let dom = s[i];
      this.remove(dom);
    }
   ...
   */
  this.removeEmptyParent = (pDom) => {
    if (!pDom) {
      return;
    }
    let p;
    if (this.isEmptyDom(pDom)) {
      if (pDom === env.body || this.isTag(pDom, ['td', 'th'])) {
   ...
   */
  this.removeListFilter = (domList, filter) => {
    let removeList = [], dom;
    if (!domList || !filter) {
      return removeList;
    }
   ...
   */
  this.removeStyleByNameFromHtml = (html, nameList) => {
    let name = '(' + nameList.join('|') + ')';
    let regStyle = new RegExp('<style( ([^<>])+[ ]+|[ ]+)name *= *([\'"])' + name + '\\3[^<>]*>[^<]*<\/style>', 'ig');
    let regLink = new RegExp('<link( ([^<>])+[ ]+|[ ]+)name *= *([\'"])' + name + '\\3[^<>]*>', 'ig');
  ...
  this.replaceTagName = (dom, tagName) => {
    if (this.isTag(dom, tagName)) {
      return;
    }
    let isTodo = this.hasClass(dom, CONST.CLASS.TODO_LAYER);
    let tag = env.doc.createElement(tagName);
    this.after(tag, dom);
   ...
   */
  this.removeByTagFromHtml = (html, tag) => {
    let reg = new RegExp('<' + tag + '( [^>]*)?>((?!<\/' + tag + '>).|\r|\n)*?<\/' + tag + '>', 'ig');
    return html.replace(reg, '');
  };
   ...
   */
  this.search = (dom, expStr, list) => {
    let tmpList = dom.querySelectorAll(expStr),
      i, j, d;
    list = list ? list : [];
   ...
   * 设置区域可编辑
   */
  this.setContenteditable = (content, enable) => {
  ...
  this.setLayout = (options) => {
    ...
    let confirmPos = this.getPosition(layerObj),
      targetPos = target.nodeType ? this.getPosition(target) : target,
      scrollPos = this.getPageScroll(),
   ...
   * 设置 textarea 的值
   */
  this.setTextarea = (textarea, text) => {
    let textType = (textarea.textContent === undefined ? 'innerText' : 'textContent');
    if (text) {
   ...
   */
  this.splitDomAfterSub = (mainDom, subDom) => {
    let tmpP, next;
    ...
    if (next) {
      this.splitDomBeforeSub(mainDom, next);
    }
    return mainDom;
  };
   ...
   * 返回 subDom 所在的 mainDom
   */
  this.splitDomBeforeSub = (mainDom, subDom) => {
    if (!mainDom || !subDom) {
      return mainDom;
    }
    let parents = [subDom], p = subDom.parentNode;
    let isFirstChild = p.firstChild === subDom;
    while (p) {
      parents.push(p);
   ...
   */
  this.splitDomSingle = (mainDom, subDom) => {
    ...
    // 拆分 subDom 前面的内容
    let mainP = this.splitDomBeforeSub(mainDom, subDom);
    mainP = this.splitDomAfterSub(mainP, subDom);
    return mainP;
  };
   ...
   */
  this.splitRangeText = (node, start, end, isLast) => {
    if (!this.isUsableTextNode(node)) {
      return node;
    }
    let p, s, t, v = node.nodeValue;
    p = node.parentNode;
//            var isWizSpan = this.isWizSpan(p);
    ...
    if (!start && !end || (start === 0 && end === node.nodeValue.length)) {
      ...
      if (p.childNodes.length > 1 || this.isTag(p, ['td', 'th'])) {
        if (isLast) {
...
},{"../config/const":51,"../libs/utils":59}],8:[function(require,module,exports){
/**
 * 代码区域操作核心包 core
 */
const CONST = require('../config/const');
...
const FormatStyleList = ['background', 'background-color', 'color',
  'font-family', 'font-size', 'font-weight', 'font-style',
  'text-decoration', 'text-align'];
const FormatTitleList = ['h1', 'h2', 'h3', 'h4', 'h5', 'h6'];
const FormatSubSupList = ['sub', 'sup'];
const FormatFontList = ['b', 'i', 'u', 'strike'];
const FormatCommand = {
  'b': 'bold',
  'div': 'formatBlock',
  'h1': 'formatBlock',
...
const FormatPainter = function () {
  ...
  let analyseStyle = (start, obj, styleList, result) => {
    let startComputeStyle = env.win.getComputedStyle(start);
    for (let i = styleList.length - 1; i >= 0; i--) {
      let name = styleList[i];
      let style = obj.style[name];
  ...
  let checkBlockTag = (tagName) => {
    return tagName.charAt(0) === 'h' || tagName === 'div';
  };
  let checkSubSupTag = (tagName) => {
    return /sub|sup/i.test(tagName);
  };
  let getStyle = (start) => {
    let styleList = FormatStyleList.concat(), styleResult = {};
    let tagList = FormatTitleList.concat(FormatSubSupList, FormatFontList);
    let tagMap = {}, tagName, tagObj;
    let hasTitle = false, hasSubSup = false, hasI = false, hasB = false;
    let startObj = start.nodeType === 1 ? start : start.parentNode;
    let obj = startObj;
  ...
  let _event = {
    ...
    handler: {
      ...
      onMouseUp: () => {
        ...
        for (let i = 0; i < target.tagList.length; i++) {
          let tag = target.tagList[i];
          let commandName = FormatCommand[tag.name];
          ...
          let commandState = commandExtend.queryCommandState(commandName);
...
},{"../config/const":51,"../libs/utils":59}],9:[function(require,module,exports){
/**
 * 超链接操作基本方法集合
 */
const CONST = require('../config/const');
...
const activeColor = '#FF9632';

const HighlightUtils = function () {
  let core = null;
  let env = null;
  let codeUtils = null;
  let domUtils = null;
  ...
  let highlightAll = (key, focusFirst) => {
    let sel = env.win.getSelection();
    ...
    sel.collapse(env.body, env.body.childNodes.length);
    ...
    hasHighlight = !!env.body.querySelector(CONST.TAG.TMP_HIGHLIGHT_TAG) ||
      !!env.body.querySelector(CONST.TAG.CODE_MIRROR + ' .' + _class.searching);
  ...
  let highlightTarget = (key) => {
    ...
    let codeContainer = codeUtils.getContainerFromChild(range.startContainer);
    if (codeContainer) {
  ...
  let highlightSingle = (key, aBackwards, rangeStart) => {
    let sel, range, item, tmp;
    sel = env.win.getSelection();

    if (rangeStart === -1) {
      sel.collapse(env.body, 0);
      sel.extend(env.body, 0);
    } else if (rangeStart === 1) {
      sel.collapse(env.body, env.body.childNodes.length);
      sel.extend(env.body, env.body.childNodes.length);
    }
...
},{"../config/const":51,"../libs/utils":59}],10:[function(require,module,exports){
/**
 * undo、redo 工具包
 */
const CONST = require('../config/const'),
...
let getCm = (body, cmContainerId) => {
  let cm = null;
  let cmContainer = body.querySelector('#' + cmContainerId);
...
const HistoryUtils = function() {
  ...
  let MaxRedo = 100;
  let codeMirrorDocList = {};

  let saveCmDocList = () => {
    let containerList = getCmContainerList(env.body);
    let container, cm, i;
  ...
  this.addCallback = (callback) => {
    ...
    for (let i = 0, j = this.callback.length; i < j; i++) {
      if (this.callback[i] === callback) {
        return;
      }
    }
   ...
   */
  this.canSave = (s1, s2) => {
    let result = {add: false, replace: false, direct: 0};
    if (s1.content.length !== s2.content.length ||
      !!s1.content.localeCompare(s2.content)) {
    ...
    function compareFocus(f1, f2) {
      ...
      let result = compareIndexList(f1.start, f2.start);
      if (result < 1) {
        return result;
      }
   ...
   */
  this.getCodeMirrorDoc = (containerId) => {
    let cmDoc = codeMirrorDocList[containerId] || null;
    if (cmDoc) {
  ...
  this.redo = () => {
    ...
    let historyObj = this.stack[++this.stackIndex];
    if (historyObj.type === CONST.HISTORY.TYPE.CODE_MIRROR) {
      this.restoreCodeMirror('redo', historyObj);
    } else {
      // 只有处理 CodeMirror 外的内容时才 save
      saveCmDocList();
      this.restoreSnap(historyObj.snap);
    }
  ...
  this.restoreCodeMirror = (restoreType, historyObj) => {
    if (!this.enable || !historyObj) {
      return;
    }
    let cmContainerId, cm;
   ...
   */
  this.saveSnap = (keepIndex, options) => {
    if (!this.enable ||
      (env.compositionStart &&
        (!options || options.type === CONST.HISTORY.TYPE.COMMON))) {
    ...
    let canSave, cm, cmContainerId,
      prevCmContainerId, prevHistoryObj,
      snap, prevSnap;

    prevHistoryObj = this.stackIndex > 0 ? this.stack[this.stackIndex - 1] : null;
    if (prevHistoryObj &&
      ...
      if (cm) {
        ...
        if (this.stackIndex >= 0) {
          this.stack.splice(this.stackIndex, this.stack.length - this.stackIndex);
        }
      ...
      if (!keepIndex && prevHistoryObj &&
        ...
        canSave = this.canSave(snap, prevHistoryObj.snap);
      }
  ...
  this.undo = () => {
    // console.log('.....undo....');
    if (!this.enable ||
      this.stackIndex <= 0 || this.stack.length === 0) {
      this.stackIndex = 0;
      return;
    }
    ...
    if (historyObj.nextChange.type === CONST.HISTORY.TYPE.CODE_MIRROR) {
      this.restoreCodeMirror('undo', historyObj);
    } else {
      // 只有处理 CodeMirror 外的内容时才 save
      saveCmDocList();
...
},{"../config/const":51,"../libs/utils":59}],11:[function(require,module,exports){
/**
 * 超链接操作基本方法集合
 */
const CONST = require('../config/const');
      ...
      onKeyDown: (e) => {
        ...
        if (keyCode === 32 || keyCode === 13) {
          ...
          if (range.toString().replace(CONST.FILL_CHAR_REG, '').match(/(?:https?:\/\/|ssh:\/\/|ftp:\/\/|file:\/|www\.)/i)) {
            ...
            let a = env.doc.createElement('a'), text = env.doc.createTextNode(' '), href;
...
//                    a.setAttribute('_src', href);
  ...
  this.getCurrentLink = () => {
    ...
    let currentNode = domUtils.getParentByTagName(range.startContainer, 'a', true);
    if (!currentNode) {
      return '';
    }
...
},{"../config/const":51}],12:[function(require,module,exports){
/**
 * markdown & mathjax 渲染处理
 */
const CONST = require('../config/const');
const utils = require('../libs/utils');
const markdownBase = require('../libs/Markdown.Converter');
const markdownExtra = require('../libs/Markdown.Extra');
const MathJaxRender = require('../common/MathJaxRender');
const dependLoader = require('../libs/dependLoader');
const LoadController = require('../libs/LoadController');
const xssUtils = require('../libs/xssUtils');
 ...
 * mdOptions.win
 */
const MarkdownRender = function (mdOptions) {
  mdOptions = mdOptions || {};
   ...
   */
  let patchForSafari = () => {
    let tmp = mdDoc.createElement('span');
    tmp.innerHTML = '<math>a</math>';
    tmp.style.opacity = '0';
    mdContainer.appendChild(tmp);
    let tmpText = tmp.innerText;
    domUtils.remove(tmp);
    if (tmpText) {
      return;
    }

    let tagList = mdDoc.getElementsByTagName('math');
    let tag, parent, span;
  ...
  let loadDependency = (callback) => {
    loadController.addCallback(callback);
    if (loadController.loading) {
      return;
    }
    if (!mdWin.$ || !mdWin.Diagram || !mdWin.flowchart) {
      loadController.loading = true;
      dependLoader.loadJs(mdDoc,
        dependLoader.getDependencyFiles(env.dependency, 'js', 'markdown'),
        () => {
  ...
  const Render = {
    ...
    markdownConvert: () => {
      ...
      let start, end, last, blocks, math, braces,
        inline = false;
      let $body = mdWin.$(mdContainer);
      $body.addClass(CONST.CLASS.MARKDOWN_BODY);

      let converter = new markdownBase.Converter({
      ...
      try {
        ...
        // 判断是否含有mathjax语法
        let judgeMathjaxText = text.replace(/\n/g, '\\n').replace(/\r\n?/g, "\n").replace(/```(.*\n)+?```/gm, '');
     ...
     * @param dom 传入的dom对象
     */
    markdownPreProcess: (dom) => {
      //处理 table 容器
      utils.forEach(dom.querySelectorAll('.wiz-table-container'), (target) => {
        let span = mdDoc.createElement('span');
      ...
      //处理 旧版本 todolist
      utils.forEach(dom.querySelectorAll('label.wiz-todo-label'), (target) => {
        //检测如果是遗留的 label 则不进行特殊处理
        let img = target.querySelector('.wiz-todo-img');
        if (!img) {
          return;
        }
      ...
      //处理 todolist
      utils.forEach(dom.querySelectorAll('.wiz-todo-layer'), (target) => {
        //检测如果是遗留的 todoList 则不进行特殊处理
        let checkbox = target.querySelector('.wiz-todo-checkbox');
        if (!checkbox) {
          return;
        }
      ...
      //处理 a 超链接
      utils.forEach(dom.querySelectorAll('a'), (target) => {
      ...
      //处理 img
      utils.forEach(dom.querySelectorAll('img'), (target) => {
        let span = mdDoc.createElement('span');
        span.appendChild(mdDoc.createTextNode(target.outerHTML));
        target.parentNode.insertBefore(span, target);
        target.parentNode.removeChild(target);
      ...
      //处理段落 p
      utils.forEach(dom.querySelectorAll('p'), (target) => {
        let div = mdDoc.createElement('div');
    ...
    tocReady: (markdownStr) => {
      return markdownStr.replace(/(^[ ]*)\[toc]([ ]*(\n|$))/igm, '$1[](' + WizToc + ')$2');
    },
    tocRender: () => {
      let idMap = {}, tocHtml = [], maxH = 6;
      mdWin.$('h1,h2,h3,h4,h5,h6', mdContainer).each((index, item) => {
        let n = parseInt(item.tagName.charAt(1), 10);
        maxH = Math.min(maxH, n);
      });
      mdWin.$('h1,h2,h3,h4,h5,h6', mdContainer).each((index, item) => {
        let id = (item.textContent || item.innerText).replace(/[()<> '"]/g, '');
        if (!idMap[id]) {
          idMap[id] = 1;
        } else {
          ++idMap[id];
      ...
      mdWin.$('a', mdContainer).each((index, item) => {
        item = mdWin.$(item);
    ...
    flowRender: () => {
      let f = mdWin.$('.language-flow', mdContainer).parents('pre');
      f.each((fIndex, fObj) => {
        let id = 'wiz-flow-' + fIndex;
        let flowStr = mdWin.$('textarea', fObj).val();
        if (flowStr.length > 0) {
          try {
            fObj.style.display = 'none';
            let diagram = mdWin.flowchart.parse(flowStr);
            let flowLayer = mdDoc.createElement('div');
            flowLayer.id = id;
    ...
    sequenceRender: () => {
      let _do = (list) => {
        list.each((fIndex, fObj) => {
          let id = 'wiz-sequence-' + fIndex;
          let seqStr = mdWin.$('textarea', fObj).val();
          if (seqStr.length > 0) {
            try {
              fObj.style.display = 'none';
              let diagram = mdWin.Diagram.parse(seqStr);
              let seqLayer = mdDoc.createElement('div');
              seqLayer.id = id;
      ...
      let f = mdWin.$('.language-sequence', mdContainer).parents('pre');
      _do(f);
  ...
  this.getMarkdownSrcForEditor = (options) => {
    ...
    // 清理 临时 tag
    let wizTmpList = tmp.querySelectorAll(CONST.TAG.TMP_TAG);
    for (let i = wizTmpList.length - 1; i >= 0; i--) {
  ...
  this.do = (_options, callback) => {
    ...
    let loadCallback = () => {
      ...
      if (_options.documentSrc) {
        mdContainer.innerHTML = _options.documentSrc.replace(/\n/g, '<br/>');
      }

      let timeout = env.options.reader.timeout.markdown;
  ...
  this.init = () => {
    ...
    mdCallback = mdOptions.callback || env.options.reader.callback.markdown;
  }
};
...
},{"../common/MathJaxRender":13,"../config/const":51,"../libs/LoadController":53,"../libs/Markdown.Converter":54,"../libs/Markdown.Extra":55,"../libs/dependLoader":57,"../libs/utils":59,"../libs/xssUtils":61}],13:[function(require,module,exports){
/**
 * markdown & mathjax 渲染处理
 */
const dependLoader = require('../libs/dependLoader'),
...
const MathJaxRender = function (mjOptions) {
  mjOptions = mjOptions || {};
  ...
  let loadDependency = (callback) => {
    ...
    if (!mjWin.MathJax) {
      ...
      dependLoader.loadJs(mjDoc,
        dependLoader.getDependencyFiles(env.dependency, 'js', 'mathJax'),
        () => {
          loadController.loading = false;
          loadController.callback();
        });
  ...
  this.do = (_options, callback) => {
    ...
    let loadCallback = () => {
      let timeout = env.options.reader.timeout.mathJax;
  ...
  this.init = () => {
    ...
    mjCallback = mjOptions.callback || env.options.reader.callback.mathJax;
  }
...
},{"../libs/LoadController":53,"../libs/dependLoader":57,"../libs/scriptLoader":58}],14:[function(require,module,exports){
/**
 * 夜间模式的基本方法集合
 */
const CONST = require('../config/const');

const NightModeUtils = function () {
  let core = null;
  let env = null;
  let domUtils = null;
  let wizStyle = null;
  ...
  this.on = (color, bgColor, brightness) => {
    if (color) {
      _color = color;
    }
    if (bgColor) {
  ...
  this.off = () => {
    let style = env.doc.getElementById(_style_id);
    if (style) {
      style.remove();
    }
  };
...
},{"../config/const":51}],15:[function(require,module,exports){
/**
 * paste 粘贴操作基本库
 */
const CONST = require('../config/const');
...
const PasteUtils = function () {
  ...
  let clearClipHtml = (html) => {
    ...
    let fragmentReg = /<body[^>]*>((\r?\n|.)*)(\r?\n)*<\/body>/gi;
    let headReg = /<head[^>]*>((\r?\n|.)*)(\r?\n)*<\/head>/i;
    let styleReg = /<style[^>]*>[^<>]*<\/style>/ig;
    let body = fragmentReg.exec(html), head, m;
    if (body) {
      ...
      html = html.replace(/<!--.*?(-->)/g, '');
      let startTag = [/<body( [^<>]*)?>/i, /<html( [^<>]*)?>/i];
      let endTag = [/<\/body>/i, /<\/html>/i];

      for (let i = 0, j = startTag.length; i < j; i++) {
        let reg = startTag[i];
      ...
      for (let i = 0, j = endTag.length; i < j; i++) {
        let reg = endTag[i];
        let result = html.match(reg);
        if (result) {
  ...
  let insertBr = () => {
    let range = rangeUtils.getRange();
    let start = rangeUtils.getRangeDetail(range.startContainer, range.startOffset);
    let line = domUtils.getParentByTagName(start.container, ['li', 'td', 'th'], true);
    if (domUtils.isEmptyDom(line)) {
      // 对于 列表内必须特殊处理
      let span = env.doc.createElement('span');
  ...
  let pasteWithStandard = (html, txt) => {
    let container = env.doc.createElement('div');
    let fixed;
    ...
    let range = rangeUtils.getRange();
    let start, line;
    let tmpParent, tmpChild, pasteTag, targetBlock;
    let lastTag = null;
    ...
    if (html) {
      ...
      if (env.options.editor.type === CONST.NOTE_EDITOR_TYPE.MARKDOWN) {
        domUtils.html2Markdown(container);
      }
      ...
      while (container.firstChild) {
        ...
        if (/^h[1-6]$/i.test(targetBlock.tagName) &&
          ...
          (/^h[1-6]$/i.test(pasteTag.tagName) && targetBlock !== env.body &&
            !domUtils.isParent(targetBlock, lastTag))) {
  ...
  this.paste = (html, txt) => {
    ...
    let cmContainer = codeUtils.getContainerFromChild(range.startContainer);
    if (cmContainer) {
    ...
    let codeList = codeUtils.oldPatch.fixOldCode();
    for (let i = 0; i < codeList.length; i++) {
      let codeContainer = codeList[i];
  ...
  this.pasteFromClipBoard = (e) => {
    ...
    let htmlSrc = e.clipboardData.getData('text/html') || '',
      txt = e.clipboardData.getData('text/plain');

    this.paste(htmlSrc, txt);
    utils.stopEvent(e);
    return true;
  };
...
},{"../config/const":51,"../libs/utils":59,"../libs/xssUtils":61}],16:[function(require,module,exports){
/**
 * 范围操作的基本方法集合
 */
const CONST = require('../config/const');
...
const RangeUtils = function () {
  ...
  this.caretFocus = () => {
    ...
    if (rect && rect.top < 0) {
      env.doc.documentElement.scrollTop += rect.top;
    } else if (rect && (rect.top + rect.height) > cH) {
      env.doc.documentElement.scrollTop += (rect.top + rect.height - cH);
    }

    if (rect && rect.left < 0) {
      env.doc.documentElement.scrollLeft += rect.left;
    } else if (rect && (rect.left + rect.width) > cW) {
      env.doc.documentElement.scrollLeft += (rect.left + rect.width - cW);
    }
  };
  ...
  this.clearFillCharByCollapsed = () => {
    ...
    if (range && range.collapsed && range.startContainer.nodeType === 3) {
      ...
      this.setRange(txtNode, txtPre.length);
    }
  };
   ...
   */
  this.fixRange = (rangeResult) => {
    //reset the selection's range
    //自闭合标签 需要特殊处理
    let isStartBak = !rangeResult.startDom.parentNode,
    ...
    this.setRange(isStartBak ? rangeResult.startDomBak : rangeResult.startDom,
  ...
  this.fixScroll = () => {
    ...
    let _fixScroll = () => {
      let rect, winSize, scrollTop, scrollLeft;
      let range = this.getRange();
      ...
      rect = this.getRangeClientRect();
      if (!rect) {
        return;
      }
      winSize = domUtils.getWindowSize();
      let webViewHeight = winSize.height, toolbarHeight = 0;
      ...
      let pageScroll = domUtils.getPageScroll();
      ...
      if (env.client.type.isIOS && rect.top + toolbarHeight < 0) {
        env.doc.documentElement.scrollTop = scrollTop + rect.top + toolbarHeight;
        ...
        env.doc.documentElement.scrollTop = scrollTop + rect.top + rect.height + toolbarHeight - webViewHeight;
      } else if (!env.client.type.isIOS && rect.top < 0) {
        env.doc.documentElement.scrollTop = scrollTop + rect.top;
      } else if (!env.client.type.isIOS && rect.top + rect.height > winSize.height) {
        env.doc.documentElement.scrollTop = scrollTop + rect.top + rect.height - winSize.height;
      }
      if (rect.left < scrollLeft) {
        env.doc.documentElement.scrollLeft = scrollLeft + rect.left;
      } else if (rect.left + rect.width > winSize.width) {
        env.doc.documentElement.scrollLeft = scrollLeft + rect.left + rect.width - winSize.width;
      }
    };
  ...
  this.getRange = () => {
    ...
    let emptyDiv;
    // 避免 env.body 内是空的
    if (range.startContainer === env.body && range.startOffset === 0 && range.collapsed &&
      (!env.body.firstChild || domUtils.isTag(env.body.firstChild, CONST.TAG.TMP_TAG))) {
      ...
      this.setRange(emptyDiv, 0);
    }

    let rangeDetail = this.getRangeDetail(range.startContainer, range.startOffset);
    let p = domUtils.getParentByClass(rangeDetail.container, CONST.CLASS.WIZ_BODY, true);
    if (!p) {
      range = null;
    }
    return range;
   ...
   */
  this.getRangeAnchor = (isBackward) => {
    let range = this.getRange();
    if (!range) {
      return null;
    }
    let rangeContainer = isBackward ? range.startContainer : range.endContainer,
   ...
   * 获取 光标矩形区域 & 位置
   */
  this.getRangeClientRect = () => {
    let range = this.getRange();
    if (!range) {
      return null;
    }

    let rects, rect = range.getBoundingClientRect(), start;
   ...
   */
  this.getRangeDetail = (startContainer, startOffset) => {
   ...
   */
  this.getRangeDomList = (options) => {
    let range = this.getRange();
    if (!range) {
      return null;
    }
    let startDom = range.startContainer, startOffset = range.startOffset,
   ...
   * 获取 光标范围内 Dom 共同的父节点
   */
  this.getRangeParentRoot = () => {
    let range = this.getRange(), startDom, endDom;
    if (!range) {
      return null;
    }
  ...
  this.insertNode = (dom) => {
    ...
    if (range.collapsed && domUtils.isTag(range.startContainer, 'br')) {
      ...
      let index = domUtils.getIndex(range.startContainer);
      this.setRange(p, index, p, index + 1);
      range = this.getRange();
    }
    range.insertNode(dom);
  };
   ...
   */
  this.modifyCaretStyle = (style, attr) => {
    let sel = env.doc.getSelection();
    let focusNode = sel.focusNode;
    let range, key, value, hasSameStyle = true,
      n;
    ...
    //clear redundant span & TextNode
    //var p = focusNode;
    let p = focusNode.parentNode ? focusNode.parentNode : focusNode;
    domUtils.clearChild(p, [n]);

    //reset the selection's range
    this.setRange(n.childNodes[0], 1, n.childNodes[0], 1);
    //sel.removeAllRanges();
    //sel.addRange(range);
  };
   ...
   */
  this.modifyRangeStyle = (style, attr) => {
    let rangeResult, rangeList, rangeLength;
    //get the RangeList
    rangeResult = this.getRangeDomList({
      noSplit: false
    });
    if (!rangeResult) {
      return;
    ...
    rangeList = [];
    for (let dom of rangeResult.list) {
      let code = codeUtils.getContainerFromChild(dom);
      if (!code) {
        rangeList.push(dom);
      }
    }
    ...
    //modify style
    domUtils.modifyNodesStyleAndClear(rangeList, style, attr,
   ...
   * 修改 光标范围内的 Dom 样式 & 属性
   */
  this.modifySelectionDom = (style, attr) => {
    let range = this.getRange();
    if (!range) {
      return false;
    }
   ...
   */
  this.selectCharIncludeFillChar = (isBackward) => {
    let sel = env.doc.getSelection(),
      range = sel.getRangeAt(0),
      direction = isBackward ? 'backward' : 'forward';

    let tmpCurDom, tmpOffset, tmpNextDom, s;
    if (range.startContainer.nodeType === 1) {
      tmpCurDom = this.getRangeAnchor(false);
      ...
      if (range.startContainer === tmpCurDom && domUtils.isTag(tmpCurDom, 'br') && domUtils.isEmptyDom(tmpCurDom.parentNode)) {
        if (tmpCurDom.parentNode.nextSibling) {
          this.setRange(tmpCurDom.parentNode, 0, tmpCurDom.parentNode.nextSibling, 0);
        } else {
    ...
    range = sel.getRangeAt(0);
    s = range.toString();
    tmpCurDom = this.getRangeAnchor(isBackward);
  ...
  this.setRange = (start, startOffset, end, endOffset) => {
    ...
    let maxStart = domUtils.getEndOffset(start),
   ...
   */
  this.setRangeToEnd = (dom) => {
    let target = dom;
    let offset = domUtils.getEndOffset(dom);

    if (domUtils.isSelfClosingTag(target)) {
      target = target.parentNode;
      offset = domUtils.getIndex(dom) + 1;
    }
    this.setRange(target, offset);
  };

};

module.exports = RangeUtils;
},{"../config/const":51}],17:[function(require,module,exports){
/**
 * 下拉框插件
 */
const CONST = require('../config/const');
...
const SelectPlugin = function() {
  ...
  let _event = {
    ...
    handler: {
      onMouseDown: (e) => {
        let target = e.target;
        let header = selectUtils.getHeaderFromDom(target);
        if (header) {
          selectUtils.showOptions(header);
          return;
        }
        let option = selectUtils.getOptionFromDom(target);
        if (option) {
          selectUtils.selectOption(option);
        }
        let container = selectUtils.getContainerFromDom(target);
        if (!container) {
  ...
  let selectUtils = {
    ...
    selectOption: (option) => {
      let container = selectUtils.getContainerFromDom(option);
      if (!container) {
        return;
      }
      let selectedItem = container.querySelectorAll('.' + CONST.CLASS.SELECT_PLUGIN_OPTIONS_ITEM + '.' + _class.selected);
      ...
      let headerText = selectUtils.getHeaderText(container);
    ...
    showOptions: (header) => {
      let container = selectUtils.getContainerFromDom(header);
  ...
  this.create = (optionsData, selectClass, defaultValue) => {
    let container = env.doc.createElement('div');
    let header = env.doc.createElement('div');
    let headerText = env.doc.createElement('span');
    let headerIcon = env.doc.createElement('i');
    let options = env.doc.createElement('ul');
    ...
    for (let i = 0, j = optionsData.length; i < j; i++) {
      let text = optionsData[i].text;
      let value = optionsData[i].value;
      let mockOption = env.doc.createElement('li');
...
},{"../config/const":51}],18:[function(require,module,exports){
/**
 * tab 键操作处理
 */
const CONST = require('../config/const');
...
},{"../config/const":51,"../libs/utils":59}],19:[function(require,module,exports){
/**
 * 默认的样式集合
 */
const CONST = require('../config/const');

const TmpEditorStyle = {
    iosPhone: '.' + CONST.CLASS.EDITING + '.' + CONST.CLASS.WIZ_BODY + ' {' +
    'overflow-y:scroll;' +
    '-webkit-overflow-scrolling: touch;' +
    '-webkit-tap-highlight-color: rgba(0, 0, 0, 0);' +
    '}' +
    '.' + CONST.CLASS.EDITING + '.' + CONST.CLASS.WIZ_BODY + '[contenteditable=true], .wiz-template-editable {' +
    'padding-bottom: 44px !important;' +
    '}' +
    '.' + CONST.CLASS.EDITING + '.' + CONST.CLASS.WIZ_BODY + ' td,' +
    '.' + CONST.CLASS.EDITING + '.' + CONST.CLASS.WIZ_BODY + ' th {position:static;}' +
    '.' + CONST.CLASS.EDITING + '.' + CONST.CLASS.WIZ_BODY + ' th:before,' +
    '.' + CONST.CLASS.EDITING + '.' + CONST.CLASS.WIZ_BODY + ' td:before,' +
    '.' + CONST.CLASS.EDITING + '.' + CONST.CLASS.WIZ_BODY + ' th:after,' +
    '.' + CONST.CLASS.EDITING + '.' + CONST.CLASS.WIZ_BODY + ' td:after {display:none;}',
    iosPad: '.' + CONST.CLASS.EDITING + '.' + CONST.CLASS.WIZ_BODY + ' {' +
    'min-width: 90%;' +
    'max-width: 100%;' +
    'min-height: 100%;' +
    'background: #ffffff;' +
    ...
    selectPlugin: '.' + CONST.CLASS.WIZ_BODY + ' .wiz-select-plugin-container {position:relative;display:inline-block;width:160px;height:28px;border-radius:4px;padding:0;margin-left:5px;cursor:pointer;}' +
    '.' + CONST.CLASS.WIZ_BODY + ' .wiz-select-plugin-container,' +
    ' .' + CONST.CLASS.WIZ_BODY + ' .wiz-select-plugin-options {box-sizing:border-box;background:white;border:1px solid #e7e7e7;color:#333;box-shadow: 1px 1px 5px #d0d0d0;}' +
    '.' + CONST.CLASS.WIZ_BODY + ' .wiz-select-plugin-header {line-height:28px;font-size:14px;padding: 0 0 0 5px;overflow:hidden;margin-right:27px;white-space:nowrap;}' +
    '.' + CONST.CLASS.WIZ_BODY + ' .wiz-select-plugin-header i {position:absolute;top:0;right:0;line-height:26px;padding:0 6px;border-left:1px solid #e7e7e7;box-sizing:border-box;}' +
    '.' + CONST.CLASS.WIZ_BODY + ' .wiz-select-plugin-options {display:none;background:white;list-style:none;padding:0px;white-space:nowrap;max-height:200px;min-width:160px;max-width:260px;overflow-x:hidden;overflow-y:auto;position:absolute;top:30px;right:-1px;}' +
    ...
    '.' + CONST.CLASS.WIZ_BODY + ' .wiz-select-plugin-options-item {font-size:11px;padding:0 5px;height:20px;line-height:20px;}' +
    '.' + CONST.CLASS.WIZ_BODY + ' .wiz-select-plugin-options-item.selected {background:#448aff;color:white;}' +
    '.' + CONST.CLASS.WIZ_BODY + ' .wiz-select-plugin-options-item:hover {background:#448aff !important;color:white !important;}' +
    '.' + CONST.CLASS.WIZ_BODY + ' .wiz-select-plugin-container.active .wiz-select-plugin-options {display:block;}',
    table: '.' + CONST.CLASS.EDITING + '.' + CONST.CLASS.TABLE_BODY + '.' + CONST.CLASS.TABLE_MOVING + ' *,' +
    ' .' + CONST.CLASS.EDITING + '.' + CONST.CLASS.TABLE_BODY + '.' + CONST.CLASS.TABLE_MOVING + ' *:before,' +
    ' .' + CONST.CLASS.EDITING + '.' + CONST.CLASS.TABLE_BODY + '.' + CONST.CLASS.TABLE_MOVING + ' *:after {cursor:default !important;}' +
    ' .' + CONST.CLASS.EDITING + '.' + CONST.CLASS.WIZ_BODY + ' td,' +
    ' .' + CONST.CLASS.EDITING + '.' + CONST.CLASS.WIZ_BODY + ' th {position:relative;}' +
    ...
    '.wiz-table-tools .wiz-table-menu-sub > div{font-size:15px;min-width:63px;}' +
    ...
    '.wiz-table-tools .wiz-table-cell-align .wiz-table-cell-align-item[data-align-type=align] {margin-right:8px}' +
    ...
    '.wiz-table-tools .wiz-table-cell-align-item i.editor-icon.align {font-size:17px;}' +
    ...
    '.wiz-table-tools .wiz-table-cell-align .wiz-table-cell-align-item:last-child {margin-right: 0;}' +
    '.' + CONST.CLASS.EDITING + '.' + CONST.CLASS.WIZ_BODY + ' th.wiz-selected-cell-multi,' +
    '.' + CONST.CLASS.EDITING + '.' + CONST.CLASS.WIZ_BODY + ' td.wiz-selected-cell-multi {background: rgba(0,102,255,.05);}' +
    '.' + CONST.CLASS.EDITING + '.' + CONST.CLASS.WIZ_BODY + ' th:before,' +
    '.' + CONST.CLASS.EDITING + '.' + CONST.CLASS.WIZ_BODY + ' td:before,' +
    '#wiz-table-col-line:before,#wiz-table-range-border_start_right:before,' +
    '#wiz-table-range-border_range_right:before {content: " ";position: absolute;top: 0;bottom: 0;right: -5px;width: 9px;cursor: col-resize;background: transparent;z-index:' + CONST.CSS.Z_INDEX.tableTDBefore + ';}' +
    '.' + CONST.CLASS.EDITING + '.' + CONST.CLASS.WIZ_BODY + ' th:after,' +
    '.' + CONST.CLASS.EDITING + '.' + CONST.CLASS.WIZ_BODY + ' td:after,' +
    '#wiz-table-row-line:before,#wiz-table-range-border_start_bottom:before,' +
    '#wiz-table-range-border_range_bottom:before {content: " ";position: absolute;left: 0;right: 0;bottom: -5px;height: 9px;cursor: row-resize;background: transparent;z-index:' + CONST.CSS.Z_INDEX.tableTDBefore + ';}'
  },
  TmpReaderStyle = {
    ios: '.' + CONST.CLASS.READONLY + '.' + CONST.CLASS.WIZ_BODY + ' img {' +
    'max-width: 100%;' +
  ...
  TmpCommonStyle = {
    ...
    '.' + CONST.CLASS.WIZ_BODY + ' {min-height:100%;box-sizing:border-box;word-wrap: break-word !important;outline:none;position:relative;}' +
    '.' + CONST.CLASS.WIZ_BODY + ' a {word-wrap: break-word;}' +
    '.' + CONST.CLASS.WIZ_BODY + ' img::selection {background-color: rgba(0, 0, 255, 0.3);}',
    ...
    '.' + CONST.CLASS.WIZ_BODY + ' td,' +
    '.' + CONST.CLASS.WIZ_BODY + ' th {outline:none;}'
  },
  DefaultStyle = {
    common: 'html, .' + CONST.CLASS.WIZ_BODY + ' {' +
    'font-size: 12pt;' +
    '}' +
    '.' + CONST.CLASS.WIZ_BODY + ' {' +
    'font-family: ' + CONST.CSS.DEFAULT_FONT + ';' +
    'line-height: 1.6;' +
    'margin: 0 auto;' +
    'padding: 20px 16px;padding: 1.25rem 1rem;' +
    '}' +
    '.' + CONST.CLASS.WIZ_BODY + ' h1,' +
    '.' + CONST.CLASS.WIZ_BODY + ' h2,' +
    '.' + CONST.CLASS.WIZ_BODY + ' h3,' +
    '.' + CONST.CLASS.WIZ_BODY + ' h4,' +
    '.' + CONST.CLASS.WIZ_BODY + ' h5,' +
    '.' + CONST.CLASS.WIZ_BODY + ' h6 {margin:20px 0 10px;margin:1.25rem 0 0.625rem;padding: 0;font-weight: bold;}' +
    '.' + CONST.CLASS.WIZ_BODY + ' h1 {font-size:20pt;font-size:1.67rem;}' +
    '.' + CONST.CLASS.WIZ_BODY + ' h2 {font-size:18pt;font-size:1.5rem;}' +
    '.' + CONST.CLASS.WIZ_BODY + ' h3 {font-size:15pt;font-size:1.25rem;}' +
    '.' + CONST.CLASS.WIZ_BODY + ' h4 {font-size:14pt;font-size:1.17rem;}' +
    '.' + CONST.CLASS.WIZ_BODY + ' h5 {font-size:12pt;font-size:1rem;}' +
    '.' + CONST.CLASS.WIZ_BODY + ' h6 {font-size:12pt;font-size:1rem;color: #777777;margin: 1rem 0;}' +
    '.' + CONST.CLASS.WIZ_BODY + ' div,' +
    '.' + CONST.CLASS.WIZ_BODY + ' p,' +
    '.' + CONST.CLASS.WIZ_BODY + ' ul,' +
    '.' + CONST.CLASS.WIZ_BODY + ' ol,' +
    '.' + CONST.CLASS.WIZ_BODY + ' dl,' +
    '.' + CONST.CLASS.WIZ_BODY + ' li {margin:0;}' +
    '.' + CONST.CLASS.WIZ_BODY + ' blockquote,' +
    '.' + CONST.CLASS.WIZ_BODY + ' table,' +
    '.' + CONST.CLASS.WIZ_BODY + ' pre,' +
    '.' + CONST.CLASS.WIZ_BODY + ' code {margin:8px 0;}' +
    '.' + CONST.CLASS.WIZ_BODY + ' ul,' +
    '.' + CONST.CLASS.WIZ_BODY + ' ol {padding-left:32px;padding-left:2rem;}' +
    '.' + CONST.CLASS.WIZ_BODY + ' ol.wiz-list-level1 > li {list-style-type:decimal;}' +
    '.' + CONST.CLASS.WIZ_BODY + ' ol.wiz-list-level2 > li {list-style-type:lower-latin;}' +
    '.' + CONST.CLASS.WIZ_BODY + ' ol.wiz-list-level3 > li {list-style-type:lower-roman;}' +
    '.' + CONST.CLASS.WIZ_BODY + ' blockquote {padding:0 12px;padding:0 0.75rem;}' +
    '.' + CONST.CLASS.WIZ_BODY + ' blockquote > :first-child {margin-top:0;}' +
    '.' + CONST.CLASS.WIZ_BODY + ' blockquote > :last-child {margin-bottom:0;}' +
    '.' + CONST.CLASS.WIZ_BODY + ' img {border:0;max-width:100%;height:auto !important;margin:2px 0;}' +
    '.' + CONST.CLASS.WIZ_BODY + ' table {border-collapse:collapse;border:1px solid #bbbbbb;}' +
    '.' + CONST.CLASS.WIZ_BODY + ' td,' +
    '.' + CONST.CLASS.WIZ_BODY + ' th {padding:4px 8px;border-collapse:collapse;border:1px solid #bbbbbb;min-height:28px;word-break:break-all;box-sizing: border-box;}' +
    '.wiz-hide {display:none !important;}'
  };

const WizStyle = function () {
  let core = null;
  let env = null;
  let domUtils = null;
  ...
  this.insertDefaultStyle = (isReplace, customCss) => {
    this.replaceStyleById(CONST.ID.WIZ_DEFAULT_STYLE, DefaultStyle.common, isReplace);
    if (!customCss) {
      return;
    }
    let css, hasCustomCss = false;
    if (typeof customCss === 'string') {
      css = customCss;
      hasCustomCss = true;
    } else {
      css = 'html, body{';
      for (let k in customCss) {
  ...
  this.insertCustomStyle = (id, customCss, isTemp) => {
    if (!id || !customCss) {
      return;
    }
    let options = {id: id};
    ...
    this.insertStyle(options, customCss);
  };
  this.insertStyle = (options, css) => {
    let s = env.doc.createElement('style');
    if (options.name) {
      s.setAttribute('name', options.name);
    }
    if (options.id) {
      s.setAttribute('id', options.id);
      this.removeStyleById(options.id);
    }
    env.doc.getElementsByTagName('HEAD')[0].insertBefore(s, null);
    s.innerHTML = css;
    return s;
  };
  this.insertTmpEditorStyle = () => {
    this.insertStyle({
  ...
  this.removeFormat = (removeAll, isRemoveColor, isRemoveAllStyles) => {
    let isWizDom = (dom) => {
  ...
  this.removeStyleById = (id) => {
    let s = env.doc.getElementById(id);
    if (s && domUtils.isTag(s, ['style', 'link'])) {
      domUtils.remove(s);
    }
  };
  this.removeStyleByName = (name) => {
      let sList = env.doc.getElementsByName(name);
  ...
  this.replaceStyleById = (id, css, onlyReplace) => {
...
},{"../config/const":51}],20:[function(require,module,exports){
/**
 * 修订功能 专用工具包
 */
const CONST = require('../../config/const'),
...
const Amend = function () {
  ...
  this.initCore = (_core) => {
    ...
    //为 domUtils 打补丁
    (() => {
      let modifyNodeStyle = domUtils.modifyNodeStyle;
      //针对 修订特殊处理 image
      domUtils.modifyNodeStyle = (item, style, attr, isLast) => {
        let p;
        if (item.nodeType === 1 &&
      ...
      let addDomForGetDomList = domUtils.addDomForGetDomList;
      //忽略 在修订模式下 已经删除的内容
      domUtils.addDomForGetDomList = (main, sub) => {
        //忽略 在修订模式下 已经删除的内容
  ...
  let h6Patch = () => {
    let range, block, hObj, newLine, isLast = false;

    let isLastDom = (parent, dom) => {
      if (!parent) {
        return false;
      }
      let lastDom = domUtils.getLastDeepChild(parent);
      let p = domUtils.getParentByFilter(lastDom, (obj) => {
        return obj === dom;
      }, true);
  ...
  let amendEvent = {
    ...
    onKeyDown: (e) => {
      ...
      if (sel.rangeCount === 0 || env.compositionStart) {
        return;
      }

      if (this.isAmendEditing()) {
    ...
    onKeyDownAmend: (e) => {
      let keyCode = e.keyCode || e.which;
      // console.info(e);
      let sel = env.doc.getSelection(), range,
        endDom, endOffset,
...
//                  console.log(endDom.nodeValue);
       ...
       */
      if (keyCode === 13) {
        let delDom = amendUtils.getWizDeleteParent(endDom),
        ...
        if (aDom && aDom.childNodes.length === 1 &&
          ...
          (() => {
            let p = aDom.parentNode, b = env.doc.createElement('br');
    ...
    onKeyDownReverse: (e) => {
      ...
      // console.info(e);
      let sel = env.doc.getSelection();
      let fixed = amendUtils.fixedAmendRange();

      let fixClearLine = (dom, direct) => {
       ...
       */
      historyUtils.saveSnap(false);
      this.splitAmendDomByRange(fixed);
      //删除 range 选中区域后，必须要避免光标进入 table 容器内
      let check = tableUtils.checkCaretInTableContainer();
     ...
     */
    onMouseDown: (e) => {
      let isInfo = amendInfo.isInfo(e.target);
      if (isInfo) {
        utils.stopEvent(e);
        return;
      }
  ...
  this.accept = (target) => {
    ...
    //合并文档， 清除冗余html
    domUtils.clearChild(env.body, []);
  };

  this.changeCurUser = (user) => {
    env.options.userInfo = user;
    amendUser.initUser();
  };

  this.fixPaste = (start, end, user) => {
   ...
   */
  this.fragmentFilter = (fragment) => {
    if (!fragment) {
      return false;
    }

    let delDom = fragment.querySelectorAll('.' + CONST.CLASS.WIZ_BODY +
      ' span[' + CONST.ATTR.SPAN_DELETE + '="' + amendUser.getCurUser().hash + '"]');
    for (let i = delDom.length - 1; i >= 0; i--) {
      let delDomItem = delDom[i];
  ...
  this.hasAmendSpanByCursor = () => {
    let amendDoms = amendUtils.getAmendDoms({
      selection: true,
      selectAll: false
    });
...
//        sel.selectAllChildren(nSpanStart);
  ...
  this.refuse = (target) => {
    ...
    if (amendDoms) {
      ...
      let saveDeletedInsert = amendDoms.deletedInsertList.length > 0 &&
        amendDoms.deleteList.length === 0 &&
...
},{"../../config/const":51,"../../libs/utils":59}],21:[function(require,module,exports){
/**
 * 修订信息显示图层 相关对象
 */
const CONST = require('../../config/const');
...
const AmendInfo = function () {
  let core = null;
  let env = null;
  let lang = null;
  let amendUser = null;
  let amendUserAction = null;
  let amendUtils = null;
  let domUtils = null;
  ...
  let _event = {
    ...
    bindInfoBtn: () => {
      ...
      if (env.client.type.isIOS || env.client.type.isAndroid) {
        this.main.addEventListener('touchend', _event.handler.onClick);
      } else {
        this.main.addEventListener('click', _event.handler.onClick);
      }

    },
    unbindInfoBtn: () => {
      if (env.client.type.isIOS || env.client.type.isAndroid) {
        this.main.removeEventListener('touchend', _event.handler.onClick);
      } else {
        this.main.removeEventListener('click', _event.handler.onClick);
      }
    },
    ...
    handler: {
      ...
      onMouseMove: (e) => {
        ...
        let eventClient = utils.getEventClientPos(e);
        ...
        if (selectedDoms || targetDom) {
          ...
          this.show(selectedDoms || targetDom, pos);
        } else {
          this.hide(false);
        }
      },
   ...
   */
  let createAmendInfo = () => {
    let mask = env.body.querySelector('#' + CONST.ID.AMEND_INFO), container;
  ...
  let hideInfo = () => {
    if (this.main) {
      ...
      this.img.src = '';
      this.name.innerText = '';
      this.name.setAttribute('title', '');
      this.content.innerText = '';
      domUtils.css(this.main, {
        'display': 'none'
      });
      this.template.appendChild(this.main);
    }
  };
  let initMultiInfo = (pos) => {
    this.curPos = pos;
    this.singleUser.style.display = 'none';
    this.multiUser.style.display = 'block';
  };
  let initUserInfo = (pos) => {
    let dom = this.cur,
    ...
    this.curPos = pos;
    this.img.src = user ? user.imgUrl : '';
    this.name.innerText = name;
    this.name.setAttribute('title', name);
    this.content.innerText = isDelete ? lang.Amend.Delete : lang.Amend.Edit;
    this.time.innerText = time;

    this.multiUser.style.display = 'none';
    this.singleUser.style.display = 'block';
  };
  /**
   * 删除 修订信息 图层
   */
  ...
  let showInfo = (pos) => {
    if (this.main.parentNode === this.template) {
      env.body.appendChild(this.main);
      this.singleUser = env.body.querySelector('#' + CONST.ID.AMEND_INFO_SINGLE);
      this.multiUser = env.body.querySelector('#' + CONST.ID.AMEND_INFO_MULTI);
      this.img = env.body.querySelector('#' + CONST.ID.AMEND_INFO_IMG);
      this.name = env.body.querySelector('#' + CONST.ID.AMEND_INFO_NAME);
      this.content = env.body.querySelector('#' + CONST.ID.AMEND_INFO_CONTENT);
      this.time = env.body.querySelector('#' + CONST.ID.AMEND_INFO_TIME);
      this.tools = env.body.querySelector('#' + CONST.ID.AMEND_INFO_TOOLS);
      this.btnAccept = env.body.querySelector('#' + CONST.ID.AMEND_INFO_ACCEPT);
      this.btnRefuse = env.body.querySelector('#' + CONST.ID.AMEND_INFO_REFUSE);
    }
  ...
  this.hide = (quick) => {
    ...
    if (quick) {
      ...
      this.hideTimer = setTimeout(hideInfo, CONST.AMEND.INFO_TIMER);
    }
  };
  /**
   * 修订信息 显示图层 初始化
  ...
  this.init = (options, cb) => {
    this.template = env.doc.createElement('div');
    this.main = createAmendInfo();
    this.readonly = !!(options && options.readonly);

    domUtils.setContenteditable(this.main, false);
   ...
   */
  this.isInfo = (dom) => {
    let amendInfoMain = domUtils.getParentByFilter(dom, (node) => {
  ...
  this.show = (dom, pos) => {
    ...
    let isSelection = utils.isArray(dom);
    let isMulti = isSelection && dom.length > 1;
    let cur = !isSelection ? dom : (isMulti ? null : dom[0]);
    let showFlag = false;

    this.isSelection = isSelection;
    if ((this.isMulti !== isMulti) ||
      (cur !== this.cur)) {
      ...
      showFlag = true;
    } else if (!this.curPos || Math.abs(this.curPos.left - pos.left) > 75 ||
      Math.abs(this.curPos.top - pos.top) > 24) {
...
const AmendUser = function () {
   ...
   * 根据 user 信息生成 修订颜色
   */
  let createUserColor = (user) => {
    let userKey = user.hash,
  ...
  let getImgUrl = (guid) => {
    if (env.client.type.isWeb) {
      ...
      try {
        let avatarFileName = external.GetAvatarByUserGUID(guid);
   ...
   */
  let getUserNameFromClient = (guid) => {
    if (env.client.type.isWeb) {
  ...
  let loadUsers = () => {
    ...
    try {
      ...
      for (let i in usersForSave) {
        if (usersForSave.hasOwnProperty(i)) {
          let u = usersForSave[i];
          u.user_guid = i;
          let tmpName = getUserNameFromClient(i);
          if (tmpName) {
            u.user_name = tmpName;
          } else {
            u.user_name = u.name;
  ...
  let AmendUserClass = function (userInfo) {
    if (!userInfo) {
      userInfo = {};
    }
  ...
  let curAmendUser = null;
  let userDom = null;
  let users = null;       //用户 AmendUserClass 对象集合，用于 修订功能
  let usersForSave = null;//用户 保存数据，用于保存到 meta 内
  ...
  this.initUser = () => {
    ...
    let userInfo = env.options.userInfo;
    if (!userInfo) {
      return null;
    }
  ...
  // };
  this.setUsersData = () => {
    let _usersData = env.options.usersData;
    if (!_usersData) {
      return;
    }
    for (let i = 0, j = _usersData.length; i < j; i++) {
      let u = _usersData[i];
      let u1 = users[u.user_guid];
      let u2 = usersForSave[u.user_guid];
      if (u1 && u.user_name) {
        u1.name = u.user_name;
      }
      if (u1 && u.img_url) {
...
},{"../../config/const":51}],23:[function(require,module,exports){
/**
 * 专门用于记录用户行为的 log
 */
...
const AmendUserAction = function () {
  let core = null;
  let env = null;
...
const AmendUtils = function () {
   ...
   */
  this.add2SelectedAmendDoms = (amendDoms, dom) => {
    if (!dom) {
      return;
    }
   ...
   */
  this.createDomForImg = (type, user) => {
    let tmp = env.doc.createElement('span');
    this.setDefaultAttr(tmp, user);
    ...
    this.setUserImgContainerStyle(tmp);
   ...
   * 创建用于 新建内容的 span
   */
  this.createDomForInsert = (user) => {
    let tmp = env.doc.createElement('span');
    this.setDefaultAttr(tmp, user);
    this.setUserInsertStyle(tmp, user);
   ...
   * 标记 删除的 img
   */
  this.deleteImg = (img, user) => {
    //必须首先判断 img 是否为已已标记修订的 img span 内
    let imgSpan = this.getWizAmendImgParent(img), mask;
    if (imgSpan) {
      //如果是已删除的， 则直接忽略
    ...
    let rangeEdge = rangeUtils.isRangeEdge(img);
    let nSpan = this.packageImg(img, CONST.TYPE.IMG_DELETE, user);
  ...
  this.fixedAmendRange = () => {
    ...
    let leftDom, rightDom, startInnerDom, endInnerDom,
      startImg, endImg;
    if (sel.isCollapsed) {
      ...
      startImg = this.getWizAmendImgParent(startInnerDom);
      endImg = this.getWizAmendImgParent(endInnerDom);
  ...
  this.fixSelectionByDeleteImg = () => {
    ...
    startDom = this.getWizAmendImgParent(range.startContainer);
    ...
    if (!sel.isCollapsed) {
      endDom = this.getWizAmendImgParent(range.endContainer);
      if (!endDom) {
        endDom = range.endContainer;
        endOffset = range.endOffset;
      } else {
  ...
  this.getAmendDoms = (options) => {
    ...
    if (options.selection) {
      ...
      result.insertList = this.getWizSpanFromRange(options.selectAll, insertAttr);
      //清理出 删除&新增内容
      result.deletedInsertList = domUtils.removeListFilter(result.insertList, (dom) => {
        return (dom.getAttribute(CONST.ATTR.SPAN_DELETE));
      });
      deleteAttr[CONST.ATTR.SPAN_DELETE] = '';
      result.deleteList = this.getWizSpanFromRange(options.selectAll, deleteAttr);
      //清理出 删除&新增内容
      tmp = domUtils.removeListFilter(result.deleteList, (dom) => {
      ...
      for (let i = 0, j = options.domList.length; i < j; i++) {
        let d = options.domList[i];
  ...
  this.getSameTimeStampDom = (dom) => {
    ...
    let findWizSibling = (target, isPrev, result) => {
      let wizAmend, tmp, amendTypeTmp,
      ...
      let sibling = getSibling(target, isPrev);
      while (sibling) {
        wizAmend = this.getWizInsertParent(sibling) || this.getWizDeleteParent(sibling);
        sibling = wizAmend;
        //首先判断是否同一用户
   ...
   * 获取选择范围内 修订 dom 集合
   */
  this.getSelectedAmendDoms = () => {
    let sel = env.doc.getSelection(),
      range = sel.getRangeAt(0),
      startDom, endDom, startOffset, endOffset;

    let amends = this.getAmendDoms({
      selection: true,
      selectAll: false
    });
    ...
    let checkStart = (list, startDom, startOffset) => {
    ...
    let checkEnd = (list, endDom, endOffset) => {
      if (list.length === 0) {
        return null;
      }
      let maxLength = (endDom.nodeType === 3 ? endDom.length : endDom.childNodes.length);
    ...
    let start = checkStart(amends.deleteList, startDom, startOffset);
    if (!start) {
   ...
   */
  this.getWizAmendImgParent = (dom) => {
    return domUtils.getParentByFilter(dom, (node) => {
      return (node && node.nodeType === 1 &&
   ...
   */
  this.getWizSpanFromRange = (isAll, options) => {
    let exp = 'span';
    if (!options) {
      return [];
    }
    ...
    if (isAll) {
      ...
      let tmp = env.body.querySelectorAll(exp);
    ...
    for (let i = 0, j = domList.length; i < j; i++) {
      ...
      if (domUtils.compareIndexList(startIndex, dIdx) <= 0 &&
        domUtils.compareIndexList(endIndex, dIdx) >= 0) {
        result.push(d);
      }
    }
    return result;
   ...
   */
  this.isAmendEdited = () => {
    let amendDoms = this.getAmendDoms({
      selection: true,
      selectAll: true
    });
   ...
   */
  this.isWizAmend = (dom) => {
    return this.getWizAmendParent(dom);
  };
  /**
   * 判断 是否为 删除内容
   */
  this.isWizDelete = (dom) => {
    return !!this.getWizDeleteParent(dom);
  };
  /**
   * 判断 是否为 新增内容
   */
  this.isWizInsert = (dom) => {
    return !!this.getWizInsertParent(dom);
  };

  /**
   * 粘贴后，修改新粘贴的内容样式（设置为当前用户新建内容）
   */
  this.modifyDomForPaste = (nSpanStart, nSpanEnd, user) => {
    if (!nSpanStart || !nSpanEnd) {
      return;
    }

    if (nSpanStart.childNodes.length === 1 &&
      nSpanStart.innerText === CONST.FILL_CHAR) {
      nSpanStart.innerHTML = '';
    }
    if (nSpanEnd.childNodes.length === 1 &&
      nSpanEnd.innerText === CONST.FILL_CHAR) {
      nSpanEnd.innerHTML = '';
    }

    let parent = domUtils.getParentRoot([nSpanStart, nSpanEnd]);
    if (!parent) {
      return;
    }

    let tmpP, tmpD, tmpWizAmend,
      domResult, domList;
    ...
    for (let i = 0, j = domList.length; i < j; i++) {
      let d = domList[i];
      tmpP = d.parentNode;
      tmpWizAmend = this.getWizAmendParent(d);
      if (!tmpP) {
        continue;
      }
      if (tmpWizAmend) {
        ...
        if (domUtils.isWizSpan(tmpP) && tmpP.children.length === 0) {
          ...
          tmpD = this.createDomForInsert(user);
          tmpD.innerHTML = '';
      ...
      if (domUtils.isTag(d, 'img')) {
        d = this.packageImg(d, CONST.TYPE.IMG_INSERT, user);
      } else if (domUtils.isSelfClosingTag(d)) {
        continue;
      }
      this.setDefaultAttr(d, user);
      this.setUserInsertStyle(d, user);
    }
   ...
   */
  this.packageImg = (img, type, user) => {
    //添加元素的顺序不要随便改动， 会影响 selection 光标的位置
    let pNode, nextNode, tmpNode,
      nSpan = this.createDomForImg(type, user);
    pNode = img.parentNode;
    nextNode = img.nextSibling;
    while (nextNode && nextNode.nodeType === 3 &&
    nextNode.nodeValue === CONST.FILL_CHAR) {
      tmpNode = nextNode;
    ...
    //添加遮罩
    let mask = env.doc.createElement('img');
   ...
   * 删除 有当前用户删除标记的内容
   */
  this.removeUserDel = (parentRoot, user) => {
    let deleteDomList = [], dom;
    if (!parentRoot) {
    ...
    if (parentRoot) {
      ...
      dom = this.getWizAmendImgParent(parentRoot);
    ...
    for (let i = 0, j = deleteDomList.length; i < j; i++) {
      let dom = deleteDomList[i];
  ...
  this.removeSelection = (user) => {
    let sel = env.doc.getSelection(),
      ...
      endDom = range.endContainer,
      endOffset = range.endOffset,
      startImg = this.getWizAmendImgParent(startDom),
      endImg = this.getWizAmendImgParent(endDom),
      splitInsert;

    //如果开始 或结尾 是 修订的内容，但不是 img 的时候， 需要进行拆分后处理
    if (!endImg) {
      splitInsert = this.splitInsertDom(endDom, endOffset, true, user);
      if (splitInsert.isInsert && splitInsert.split) {
        rangeUtils.setRange(startDom, startOffset, endDom, endOffset);
      }
    }
    if (!startImg) {
      splitInsert = this.splitInsertDom(startDom, startOffset, true, user);
      if (splitInsert.isInsert && splitInsert.split) {
    ...
    let style = this.getDeletedStyle(user);
   ...
   */
  this.setUserImgContainerStyle = (dom) => {
    domUtils.css(dom, CONST.CSS.IMG.SPAN);
  };
  /**
   * 初始化用户修订样式
   */
  this.setUserInsertStyle = (dom, user) => {
    domUtils.css(dom, {
      'color': user.color,
  ...
  this.splitAmendDomByRange = (fixed) => {
    ...
    if (!sel.isCollapsed) {
      ...
      startImg = this.getWizAmendImgParent(fixed.leftDom);
      endImg = this.getWizAmendImgParent(fixed.rightDom);
      if (endImg) {
        endDom = endImg;
        endOffset = 0;
    ...
    let newDom = this.splitAmendDomForReverse(endDom, endOffset);
    if (newDom) {
   ...
   */
  this.splitAmendDomForReverse = (endDom, endOffset) => {
    let imgDom = this.getWizAmendImgParent(endDom);
    ...
    let insertDom = this.getWizInsertParent(endDom),
      deleteDom = this.getWizDeleteParent(endDom),
      amendDom = insertDom || deleteDom,
      newDom = this.createDomForReverse();

    if (imgDom) {
      domUtils.before(newDom, imgDom, endOffset > 0);
    } else if (amendDom) {
      amendDom = this.splitWizDomWithTextNode(endDom, endOffset);
      if (amendDom) {
   ...
   */
  this.splitDeletedDom = (endDom, endOffset) => {
    if (endDom.nodeType === 1) {
      return false;
    }
    let splitDom = null;
    if (this.isWizDelete(endDom)) {
      splitDom = this.splitWizDomWithTextNode(endDom, endOffset);
      return !!splitDom;
    }
    return false;
  };
   ...
   */
  this.splitInsertDom = (endDom, endOffset, forceSplit, user) => {
    let result = {
      insertDom: null,
      isInsert: false,
      split: false
    ...
    let imgDom = this.getWizAmendImgParent(endDom),
      insertDom = this.getWizInsertParent(endDom), time1, time2;
    ...
    if (result.split) {
      result.split = !!this.splitWizDomWithTextNode(endDom, endOffset);
    }
   ...
   */
  this.splitSelectedAmendDoms = (amendDoms) => {
    ...
    if (amendDoms.start && amendDoms.end &&
      mendDoms.start.dom === amendDoms.end.dom) {
      //选择范围在 一个 dom 内部的时候，会把一个 dom 拆分为 3 段
      //为保证主 dom 不丢失，所以一定要先 end 后 start
      this.splitWizDomWithTextNode(amendDoms.end.dom, amendDoms.end.offset);
      node = this.splitWizDomWithTextNode(amendDoms.start.dom, amendDoms.start.offset);
      node = node.nextSibling;
      this.add2SelectedAmendDoms(amendDoms, node);
      startDom = node;
      startOffset = 0;
      endDom = node;
      ...
      if (amendDoms.end) {
        node = this.splitWizDomWithTextNode(amendDoms.end.dom, amendDoms.end.offset);
        this.add2SelectedAmendDoms(amendDoms, node);
        endDom = node;
        endOffset = node.childNodes.length;
      }
   ...
   * 返回最后拆分的 Dom
   */
  this.splitWizDomWithTextNode = (endDom, endOffset) => {
    if (!endDom || endDom.nodeType !== 3) {
      return null;
    }
    let tmpSplitStr, tmpSplit, tmpParent, tmpDom,
      lastSplit = null;
...
},{"../../config/const":51,"../../libs/utils":59}],25:[function(require,module,exports){
/**
...
const BlockCore = function () {
  ...
  let checkScroll = () => {
    ...
    let viewHeight = env.options.document.documentElement.clientHeight;
    let pageScroll = domUtils.getPageScroll();
    let viewTop = pageScroll.top + viewHeight;

    for (let i = blockList.length - 1; i >= 0; i--) {
      let blockObj = blockList[i];
  ...
  let findBlocks = () => {
    ...
    let scrollTarget;
    // 查找 table
    let _tableList = env.body.querySelectorAll('table');
    for (let i = _tableList.length - 1; i >= 0; i--) {
      let table = _tableList[i];
      let tableContainer = tableUtils.getContainer(table);
      if (!tableContainer) {
    ...
    if (!(env.client.type.isPhone || env.client.type.isPad)) {
      let _codeList = env.body.querySelectorAll('.' + CONST.CLASS.CODE_MIRROR);
      for (let i = _codeList.length - 1; i >= 0; i--) {
        let code = _codeList[i];
        let codeContainer = codeUtils.getContainerFromChild(code);
        if (!codeContainer) {
  ...
  let setScrollSize = (blockObj) => {
    ...
    let targetWidth = blockObj.target.scrollWidth;
  ...
  let _event = {
    ...
    handler: {
      onMouseDown: (e) => {
        let isLeft = (e.type !== 'mousedown' || e.button === 0 || e.button === 1);
        if (!isLeft) {
          return;
        }
...
},{"../../config/const":51}],26:[function(require,module,exports){
/**
...
const BlockUtils = function() {
  ...
  let checkPageFirst = (checkFirst, clientY, getBlockFun) => {
    let isFirst = false, isLast = false,
      offset, container;
    let obj = checkFirst ? domUtils.getFirstDeepChild(env.body) : domUtils.getLastDeepChild(env.body);
   ...
   */
  this.checkAndInsertEmptyLine = (e) => {
    ...
    if (target === env.body || target === env.body.parentNode) {
    ...
    if (container && (isAfter || isBefore)) {
      historyUtils.saveSnap(false);
      this.insertEmptyLine(container, isAfter);
    } else {
      dom = null;
    }
  };
  ...
  this.insertEmptyLine = (blockContainer, after) => {
    if (!blockContainer) {
      return;
    }
    let newLine = env.doc.createElement('div');
    let dom = env.doc.createElement('br');
...
},{"../../config/const":51,"../../libs/utils":59}],27:[function(require,module,exports){
/**
 * 代码区域操作核心包 core
 */
const CONST = require('../../config/const');
...
const CodeCore = function() {
  ...
  let _event = {
    ...
    handler: {
      ...
      onMouseDown: (e) => {
        ...
        let tools = codeUtils.getToolsFromChild(e.target);
        if (tools) {
          utils.stopEvent(e);
        }
      },
  ...
  this.insertCode = () => {
    ...
    let curObj, container, textarea;
    let tempObj, tempRange, src = '';
    if (range) {
      if (!range.collapsed) {
...
},{"../../config/const":51,"../../libs/LoadController":53,"../../libs/dependLoader":57,"../../libs/utils":59}],28:[function(require,module,exports){
/**
 * codeMirror 相关样式处理
 */
const CONST = require('../../config/const');
...
let CSS = {
  tmp: {
    common: '.wiz-code-tools {display:none;position: absolute; top: -32px; right: 0; opacity: .95; z-index: 110;}' +
    '.' + CONST.CLASS.EDITING + ' .CodeMirror-focused .wiz-code-tools {display:block;}' +
    '.CodeMirror-sizer {border-right: 0 !important;}' +
    '.' + CONST.CLASS.WIZ_BODY + ' pre.prettyprint {padding:0;}' +
    '.' + CONST.CLASS.WIZ_BODY + ' pre.prettyprint code {white-space: pre;}' +
    '.' + CONST.CLASS.WIZ_BODY + ' pre.prettyprint.linenums {box-shadow:none; overflow: auto;-webkit-overflow-scrolling: touch;}' +
    '.' + CONST.CLASS.WIZ_BODY + ' pre.prettyprint.linenums ol.linenums {box-shadow: 40px 0 0 #FBFBFC inset, 41px 0 0 #ECECF0 inset; padding: 10px 10px 10px 40px !important;}',
...
const CodeStyle = function() {
  ...
  this.clearStyle = () => {
    let styleList = env.doc.getElementsByName(CONST.NAME.CODE_STYLE);
    for (let i = styleList.length - 1; i >= 0; i--) {
      let style = styleList[i];
      let theme = style.id.replace(CONST.ID.CODE_STYLE + '-', '');
      if (!env.doc.querySelector('.' + CONST.CLASS.CODE_CONTAINER + '[data-theme=' + theme + ']')) {
        wizStyle.removeStyleById(style.id);
      }
    }
    let codeList = env.doc.querySelectorAll('.' + CONST.CLASS.CODE_CONTAINER);
  ...
  this.insertCommon = () => {
    let style = env.doc.getElementById(CONST.ID.CODE_STYLE);
    if (!style) {
      wizStyle.replaceStyleById(CONST.ID.CODE_STYLE, CSS.common, false);
    }
  };
  ...
  this.insertTheme = (theme) => {
    if (CSS.theme[theme] &&
      !env.doc.querySelector('#' + CONST.ID.CODE_STYLE + '-' + theme)) {
      wizStyle.insertStyle({
...
},{"../../config/const":51}],29:[function(require,module,exports){
/**
 * 代码区域操作的基本方法集合
 */
const CONST = require('../../config/const');
...
(() => {
  for (let key in CODE_MIRROR.MODE) {
    if (CODE_MIRROR.MODE.hasOwnProperty(key)) {
      ModeOptions.push({
        text: key,
        value: key
      });
      let same = CODE_MIRROR.MODE[key].same;
...
const CodeUtils = function () {
  ...
  this.canCreateCode = () => {
    let range = rangeUtils.getRange();
    if (!range ||
      this.getContainerFromChild(range.startContainer) ||
  ...
  this.changeMode = (container, mode, readOnly) => {
    if (!container) {
      return;
    }
    let cm = container.codeMirror;
  ...
  this.changeTheme = (container, theme, readOnly) => {
    if (!container) {
      return;
    }
    theme = theme || 'default';
  ...
  this.clearCodeMirror = (containerList) => {
    containerList = containerList ? containerList : this.getContainerList();
    for (let i = containerList.length - 1; i >= 0; i--) {
      let container = containerList[i];
  ...
  this.clearCodeForMarkdown = (tmpBody) => {
    let containerList = tmpBody.querySelectorAll('.' + CONST.CLASS.CODE_CONTAINER);
    let readOnly = env.readonly;
    for (let i = containerList.length - 1; i >= 0; i--) {
      let container = containerList[i];
      let textarea = container.querySelector('textarea');
      let mode = this.getCmMode(container, readOnly);
      let theme = this.getCmTheme(container, readOnly);
      let src = '```' + mode + ',' + theme + '\n' + textarea.value + '\n```';

      let pre = env.doc.createElement('pre');
  ...
  this.fixCodeContainer = (options) => {
    options = options || {};
    let containerList = options.container ? [options.container] : this.getContainerList(options.body);
    for (let i = containerList.length - 1; i >= 0; i--) {
      let container = containerList[i];
      ...
      this.fixCodeContainerParent(container);
      if (env.options.reader.codeNoIDE) {
        this.initCodeNoIDE(container);
      } else {
        this.initCodeMirror({
          container: container,
          readOnly: options.readOnly
  ...
  this.fixCodeContainerParent = (container) => {
    ...
    // 避免将 有序、无序列表打断
    let orderObj = domUtils.getParentByTagName(container, ['ol', 'ul'], true);
    if (orderObj) {
      return;
    }
    parentList = domUtils.getParentList(container);
    if (parentList.length > 0) {
      if (domUtils.hasClass(parentList[0], CONST.CLASS.WIZ_EIDTOR_PREVIEW_CONTAINER) ||
        domUtils.hasClass(parentList[0], CONST.CLASS.WIZ_BODY)) {
        return;
      }
  ...
  this.getCmMode = (container, readOnly) => {
    let mode = (container.getAttribute('data-mode') || '').toLowerCase();
    let lastMode = '';
  ...
  this.getCmTheme = (container, readOnly) => {
    let theme = (container.getAttribute('data-theme') || '').toLowerCase();
    let lastTheme = '';
    if (container.isNew && env.win.localStorage && !readOnly) {
  ...
  this.initCodeNoIDE = (container) => {
    ...
    this.removeCmWrapper(container);

    let textarea = container.querySelector('textarea');
    let codeSrc = '', pre, code;
    if (textarea) {
  ...
  this.initCodeMirror = (_options) => {
    let container = _options.container;
    let readOnly = _options.readOnly;
    if (!container) {
      return;
    }
    codeStyle.insertCommon();

    let options, textarea, cm, cmDiv, cmTag,
      mode, theme;
    if (!container.id) {
    ...
    mode = this.getCmMode(container, readOnly);
    theme = this.getCmTheme(container, readOnly);
    ...
    let cursorDirect, lastCursorPos = '';
    cm.on('keydown', (cm, e) => {
      let keyCode = e.keyCode || e.which;
    ...
    cm.on('beforeSelectionChange', (cm, obj) => {
      let range = obj.ranges[0],
        ...
        head = range.head,
        newCur;
      let curCursorPos = [anchor.line, anchor.ch, head.line, head.ch].join('.');
      let codeContainer;
      // 选中区间时，不处理光标
      if (obj.origin === "+move" &&
        ...
        if (cursorDirect === -1) {
          ...
          if (newCur) {
            codeContainer = this.getContainerFromChild(newCur);
            if (codeContainer) {
    ...
    cm.on('copy', (cm, e) => {
      ...
      if ((env.client.type.isPhone || env.client.type.isPad) && readOnly) {
        e.codemirrorIgnore = true;
      }
    });
    cm.on('focus', (cm, obj) => {
      let container = this.getContainerFromChild(cm.display.wrapper);
      this.initTools(container, readOnly);
    ...
    // patch
    let sizer = container.querySelector('.CodeMirror-sizer');
    let emptyLine;
    if (sizer) {
  ...
  this.initTools = (container, readOnly) => {
    ...
    let cmWrapper = this.getCmWrapper(container);
    let codeTools = container.querySelector('.' + CONST.CLASS.CODE_TOOLS);
    let modeSelector, themeSelector;
    let mode = this.getCmMode(container, readOnly);
  ...
  this.insertCodeSrc = (container, src) => {
    let cm = container.codeMirror;
    if (!cm) {
      return;
    }
  ...
  this.onKeyDown = (e) => {
    ...
    let startLast = rangeUtils.getRangeDetail(range.startContainer, range.startOffset),
      start, block, target;
    let direct, charMove,
    ...
    if (code === 8 && startLast.offset === 0) {
      ...
      container = this.getContainerFromChild(target);
    }
    /**
     * Delete
     */
    if (code === 46 && (domUtils.isTag(startLast.container, 'br') ||
      ...
      if (container && domUtils.isTag(startLast.container, 'br')) {
        ...
        this.focusToFirst(container.codeMirror);
        return true;
      }
    }
    ...
    if (charMove) {
      ...
      if (block) {
        container = this.getContainerFromChild(block);
  ...
  this.removeCmWrapper = (container) => {
    ...
    let tmpWrapper = container.querySelector('.' + CONST.CLASS.CODE_MIRROR + '[data-id=' + container.id + ']');
  ...
  this.highlight = {
    ...
    clearAll: () => {
      ...
      for (let i = 0; i < containerList.length; i++) {
        let container = containerList[i];
        this.highlight.clear(container);
      }
    },
    overlay: (_query) => {
      let query = new RegExp(_query.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g, "\\$&"), "gi");
      return {
        token: (stream) => {
    ...
    search: (container, query) => {
      ...
      state.query = query;
      cm.removeOverlay(state.overlay);
      state.overlay = this.highlight.overlay(state.query);
  ...
  this.oldPatch = {
    oldCodeReady: () => {
      ...
      let codeList;
      let code, codeI;
      let split = (_code, selector) => {
        let domList;
        domList = _code.querySelectorAll(selector);
        for (let domI = domList.length - 1; domI >= 0; domI--) {
          let dom = domList[domI];
          // a 标签内如果 text 为空，才进行处理（只有 a 里面是图片 才分割）
          if (!utils.isEmpty(dom.innerText)) {
            continue;
          }
          let main = domUtils.getParentByClass(dom, CONST.CLASS.CODE_CONTAINER_OLD, false);
          if (main) {
            let domP = domUtils.splitDomSingle(main, dom);
            let div = env.doc.createElement('div');
            div.appendChild(dom);
      ...
      code = env.body.querySelector('.' + CONST.CLASS.CODE_CONTAINER_OLD +
        ' .' + CONST.CLASS.CODE_CONTAINER_OLD);
      // 清理所有被嵌套的 代码段
      while(code) {
        let parentCode = domUtils.getParentByClass(code, CONST.CLASS.CODE_CONTAINER_OLD, false);
    ...
    fixOldCode: (body) => {
      ...
      this.oldPatch.oldCodeReady();
      codeList = body.querySelectorAll('.' + CONST.CLASS.CODE_CONTAINER_OLD);
      ...
      for (let i = codeList.length - 1; i >= 0; i--) {
        let code = codeList[i];
        let container = this.fixCode(code);
        let src = this.oldPatch.getSrc(code);
        let mode = this.oldPatch.getMode(code);
        container.removeChild(code);
        if (!src) {
          continue;
        }
        let textarea = env.doc.createElement('textarea');
        textarea.value = src;
    ...
    getMode: (code) => {
      const regCode = /language-([\w]+)/i;
      const regPre = /lang-([\w]+)/i;
      let modeObj = code.querySelector('code'), result,
        mode;
    ...
    getSrc: (code) => {
      ...
      let domList = code.querySelectorAll('code');
      // 修正空行
      for (let i = 0, j = domList.length; i < j; i++) {
        let dom = domList[i];
        if (!dom.innerText) {
  ...
  this.pastePatch = {
    fix: () => {
      let codeList = env.body.querySelectorAll('.' + CONST.CLASS.CODE_CONTAINER_PASTE);
      for (let i = codeList.length; i >= 0; i--) {
        let container = codeList[i];
        domUtils.removeClass(container, CONST.CLASS.CODE_CONTAINER_PASTE);
        this.fixCodeContainer({container: container});
      }
    },
    ready: (dom, isInTable) => {
      let codeList = dom.querySelectorAll('.' + CONST.CLASS.CODE_MIRROR);
      let codeContainer, textarea;
      let container, codeMirror, rowList, mode, theme,
        codeLine, src;
      for (let codeIndex = codeList.length - 1; codeIndex >= 0; codeIndex--) {
        codeMirror = codeList[codeIndex];
        container = this.getContainerFromChild(codeMirror);
        if (!container) {
...
},{"../../config/const":51,"../../libs/utils":59}],30:[function(require,module,exports){
/**
 * 编辑器 基础工具包
 */
const CONST = require('../../config/const');
...
const Editor = function() {
  ...
  let frameViewCore = null;
  let historyUtils = null;
  let imgCore = null;
  let selectPlugin = null;
  let tabKey = null;
  let tableCore = null;
  let todoCore = null;
  let wizStyle = null;
  ...
  this.on = (callback) => {
    ...
    // 此问题无法重现，暂且屏蔽（开启时必须要注意保存笔记时会把 fontSize 属性保存）
    // env.body.style.fontSize = window.getComputedStyle(env.body).fontSize;
  };
  this.getOriginalHtml = () => {
    return originalHtml;
  };
  this.setOriginalHtml = () => {
...
},{"../../config/const":51,"../../libs/dependLoader":57}],31:[function(require,module,exports){
/**
 * editor 使用的基本事件处理
 */
const CONST = require('../../config/const');
...
const EditorEvent = function() {
  ...
  let eventTrackHandler = {};
  let selectTimer = null;

  let eventStringify = (event) => {
    let s = [];
    for (let k in event) {
      let v = event[k];
      let t = (typeof v).toLowerCase();
  ...
  let _getCaretStyle = () => {
    ...
    for (let i = 0, j = rangeList.length; i < j; i++) {
      let o = rangeList[i];
      ...
      for (let k in style) {
        if (style.hasOwnProperty(k)) {
          let v = domUtils.getComputedStyle(o, k, true);
          if (!v) {
            continue;
          }
    ...
    this.triggerListener(EditorEventType.SelectionChange, [result]);
  };
  ...
  this.bind = () => {
    ...
    env.body.addEventListener('cut', handler.onCut);
  ...
  this.unbind = () => {
    ...
    env.body.removeEventListener('cut', handler.onCut);
  ...
  this.addListener = (eName, fun) => {
    if (!fun || typeof fun !== 'function') {
      return;
    }
    let h = editorListener[eName];
    if (!h) {
      return;
    }
    for (let i = 0, j = h.length; i < j; i++) {
      if (h[i] === fun) {
        return;
      }
    }
  ...
  this.startTrackEvent = (eventName, id) => {
    if (!eventTrackHandler[id]) {
      eventTrackHandler[id] = (event) => {
...
},{"../../config/const":51,"../../libs/utils":59}],32:[function(require,module,exports){
/**
 * 编辑器 基础工具包
 */
const CONST = require('../../config/const');
...
const EditorUtils = function() {
  ...
  let afterInsertTimer;
  let afterInsert = (lastNode) => {
    ...
    afterInsertTimer = setTimeout(() => {
      let start, target = lastNode;
  ...
  this.find = (str, matchcase, searchBackward, loop) => {
    if (!str) {
      return false;
    }
    let result = env.win.find(str, matchcase, searchBackward);
    if (!result && !!loop) {
  ...
  this.readyForInsert = () => {
    ...
    if (range.collapsed) {
      ...
      startOffset = range.startOffset;
      // 判断光标是否在 附件的 <a> 标签内
      cannotInsertDom = domUtils.getParentByFilter(startDom, (node) => {
        if (node.nodeType !== 1) {
          return false;
        }
    ...
    let fixed = amendUtils.fixedAmendRange();
    let newDom = amend.splitAmendDomByRange(fixed);
  ...
  this.insertDom = (domList, notInsertEmptyDiv) => {
    if (!domList) {
      return;
    }
    // 尽量避免 插入Dom 时，造成的 div 反复嵌套，在 结尾处，必须找到最后有 nextSibling 的父节点
    let getBlockEnd = (result) => {
    ...
    let tmpDom = this.readyForInsert(),
      tmpParent,
  ...
  this.insertHtml = (html, notInsertEmptyDiv) => {
    if (!html) {
      return;
    }
    let template = env.doc.createElement('div'),
      doms = [];
    template.innerHTML = html;
    for (let i = 0, j = template.childNodes.length; i < j; i++) {
      doms.push(template.childNodes[i]);
    }
    this.insertDom(doms, notInsertEmptyDiv);
    template = null;
  ...
  this.replace = (from, to, matchcase) => {
    if (!from) {
      return false;
    }
    let selectedTxt = getSelectedTxt(), span, txt;
    if (selectedTxt === from) {
      //替换
      if (!to) {
        to = '';
    ...
    // 查找下一个
    return this.find(from, matchcase);
  };
  this.replaceAll = (from, to, matchcase) => {
    if (!from) {
      return false;
    }
    rangeUtils.setRange(env.body, 0);
    while (this.replace(from, to, matchcase)) {
    }
  };
...
},{"../../config/const":51,"../../libs/utils":59}],33:[function(require,module,exports){
/**
 * frameView 操作核心包 core
 */
const CONST = require('../../config/const');

const FrameViewCore = function() {
  let core = null;
  let env = null;
  let frameViewStyle = null;
  let frameViewUtils = null;
  let toolbarMarkdown = null;
  ...
  this.on = () => {
    ...
    if (!env.readonly &&
      env.options.editor.type === CONST.NOTE_EDITOR_TYPE.MARKDOWN &&
      env.frame.toolbarContainer) {
...
},{"../../config/const":51}],34:[function(require,module,exports){
/**
 * todoList 相关样式处理
 */
const CONST = require('../../config/const');
...
const CSS = {
  // container:
  toolbar: '.' + CONST.CLASS.EDITING + '.' + CONST.CLASS.EDITOR_CONTAINER + ' {' +
  'border: 1px solid silver;padding-top: 5px;' +
  '}' +
  '.' + CONST.CLASS.WIZ_EIDTOR_TOOlBAR_CONTAINER + ' {' +
  'display:flex;' +
  '}' +
  '.' + CONST.CLASS.WIZ_EIDTOR_TOOlBAR_MAIN + ' {' +
  'flex:1;' +
  '}' +
  '.' + CONST.CLASS.WIZ_EIDTOR_TOOlBAR_MAIN + ' .toolbar-split {' +
  'width: 0;border-left: 1px solid #d9d9d9;border-right: 1px solid #fff;color: transparent;text-indent: -10px;margin: 0 6px;' +
  '}' +
  '.' + CONST.CLASS.WIZ_EIDTOR_TOOlBAR_CONTAINER + ' .editor-button {' +
  'display:inline-block;position:relative;' +
  '}' +
  '.' + CONST.CLASS.WIZ_EIDTOR_TOOlBAR_CONTAINER + ' .editor-button .editor-submenu {' +
  'position: absolute;display: none;width: 195px;padding: 5px 0;background: #fff;border-radius: 3px;border: 1px solid #E0E0E0;top:28px;left:-9px;box-shadow: 1px 1px 5px #d0d0d0;z-index:100;' +
  '}' +
  '.' + CONST.CLASS.WIZ_EIDTOR_TOOlBAR_CONTAINER + ' .editor-button.active .editor-submenu {' +
  'display:block;' +
  '}' +
  '.' + CONST.CLASS.WIZ_EIDTOR_TOOlBAR_CONTAINER + ' .editor-button .editor-submenu-label{' +
  'padding: 0 6px; font-size: 14px;' +
  '}' +
  '.' + CONST.CLASS.WIZ_EIDTOR_TOOlBAR_CONTAINER + ' .editor-button .editor-submenu table{' +
  'border: 0;border-collapse: initial;border-spacing: 3px;background-color: white;margin: 0 auto;' +
  '}' +
  '.' + CONST.CLASS.WIZ_EIDTOR_TOOlBAR_CONTAINER + ' .editor-button .editor-submenu table td{' +
  'border-collapse: collapse;border: 1px solid #bbbbbb;box-sizing: border-box;position: relative;padding: 3px;width: 16px;height: 16px;' +
  '}' +
  '.' + CONST.CLASS.WIZ_EIDTOR_TOOlBAR_CONTAINER + ' .editor-button .editor-submenu table td.selected{' +
  'border-color: #448aff;' +
  '}' +
  '.' + CONST.CLASS.WIZ_EIDTOR_TOOlBAR_CONTAINER + ' .editor-icon {' +
  'font-size:16px;padding:4px;border:1px solid transparent;border-radius: 3px;cursor: pointer;color:#2c3e50' +
  '}' +
  '.' + CONST.CLASS.WIZ_EIDTOR_TOOlBAR_CONTAINER + ' .editor-icon:hover {' +
  'background:#fcfcfc;border-color:#95a5a6;' +
  '}' +
  '.' + CONST.CLASS.WIZ_EIDTOR_TOOlBAR_CONTAINER + ' .editor-icon.disabled,' +
  '.' + CONST.CLASS.WIZ_EIDTOR_TOOlBAR_CONTAINER + ' .editor-icon.disabled:hover {' +
  'background:#fcfcfc;border-color:transparent;color:#94a0ad;' +
  '}',
  preview: '.' + CONST.CLASS.WIZ_EIDTOR_PREVIEW_CONTAINER + '{' +
  'position:absolute;top:2px;bottom:0;left:0;right:0;background:#FFF;display:none;z-index:999;overflow:auto;' +
  '}' +
  '.' + CONST.CLASS.WIZ_EIDTOR_PREVIEW_CONTAINER + '.' + CONST.CLASS.EDITOR_PREVIEW_SHOW + '{' +
  'display:block;' +
  '}',
  frame: 'body {overflow-y: hidden;}'
};

const FrameViewStyle = function () {
  let core = null;
  let env = null;
  let domUtils = null;
  let wizStyle = null;
  ...
  this.insertFrameStyle = () => {
    this.removeFrameStyle();
    wizStyle.insertStyle({
      id: CONST.ID.WIZ_EIDTOR_FRAME_STYLE,
      name: CONST.NAME.TMP_STYLE
    }, CSS.frame);
  };
  this.insertPreviewStyle = () => {
  ...
  this.insertToolbarStyle = () => {
    this.removeToolbarStyle();
    let s = env.frame.toolbarDoc.getElementById(CONST.ID.WIZ_EIDTOR_TOOlBAR_STYLE);
    if (!s) {
...
},{"../../config/const":51}],35:[function(require,module,exports){
/**
 * frame 操作工具包
 */
const CONST = require('../../config/const');

const FrameViewUtils = function () {
  let core = null;
  let env = null;
  let domUtils = null;
  ...
  let adjustContainerTimer;
  let _handler = {
    onAdjustContainerSize: () => {
      if (!env.options.container || env.options.noFrame) {
        if (env.options.container && env.options.noFrame) {
          env.body.style.minHeight = env.options.editor.minHeight + 'px';
        }
        return;
      }
      clearTimeout(adjustContainerTimer);
      adjustContainerTimer = setTimeout(() => {
        let last = env.body.lastChild;
        while (last &&
        (last.nodeType !== 1 ||
          domUtils.isTag(last, CONST.TAG.TMP_TAG) ||
          env.win.getComputedStyle(last).display === 'none' ||
          last.clientHeight === 0)) {
          last = last.previousSibling;
        }
        // +25 的作用非常大，可以避免在最下行输入回车时，界面抖动
        let height = Math.max(last ? (last.offsetTop + last.offsetHeight + 25) : 0, env.options.editor.minHeight);
  ...
  let _event = {
    bind: () => {
      _event.unbind();
      if (env.readonly && env.options.container && !env.options.noFrame) {
        env.doc.addEventListener('DOMSubtreeModified', this.onAdjustContainerSize);
      }
      if (env.options.container && !env.options.noFrame) {
        env.doc.addEventListener('resize', this.onAdjustContainerSize);
      }
    },
    unbind: () => {
      env.doc.removeEventListener('DOMSubtreeModified', this.onAdjustContainerSize);
      env.doc.removeEventListener('resize', this.onAdjustContainerSize);
    }
  };
...
},{"../../config/const":51}],36:[function(require,module,exports){
/**
 * toolbar 操作核心包 core
 */
const CONST = require('../../config/const');
...
const ToolbarMarkdown = function () {
  let core = null;
  let env = null;
  let lang = null;
  let codeUtils = null;
  ...
  let _event = {
    ...
    bindSubMenu: () => {
      ...
      let table = activeButton.querySelector('.' + _class.subMenuTable);
      if (table) {
    ...
    handler: {
      onClickButton: (e) => {
        ...
        if (!domUtils.hasClass(buttonIcon, _class.icon) ||
          domUtils.hasClass(buttonIcon, _class.disabled)) {
          return;
        }

        let button = buttonIcon.parentNode;
        let id = buttonIcon.id;
        let textNode, subMenu;
        switch (id) {
          case _id.bold:
            if (!env.isShowPreview) {
              historyUtils.saveSnap(false);
      ...
      onClickDoc: (e) => {
        let target = e.target;
        let button = domUtils.getParentByClass(target, _class.button);
      ...
      onHistoryUndo: (data) => {
        let undo = ToolsButtonMap[_id.undo];
        let redo = ToolsButtonMap[_id.redo];
        if (data.undoIndex === 0) {
      ...
      onMouseOverTable: (e) => {
        ...
        let subMenu = domUtils.getParentByClass(target, _class.subMenu);
        let table = domUtils.getParentByClass(target, _class.subMenuTable);
        let label = subMenu.querySelector('.' + _class.subMenuLabel);
        if (cols && rows) {
      ...
      onSelectTable: (e) => {
        let target = e.target;
        let button = domUtils.getParentByClass(target, _class.button);
        let table = domUtils.getParentByClass(target, _class.subMenuTable, true);
      ...
      onSelectionChange: (data) => {
        let bold = ToolsButtonMap[_id.bold];
        let italic = ToolsButtonMap[_id.italic];
        let link = ToolsButtonMap[_id.link];
        let quotes = ToolsButtonMap[_id.quotes];
        let listOl = ToolsButtonMap[_id.listOl];
        let listUl = ToolsButtonMap[_id.listUl];
        let code = ToolsButtonMap[_id.code];
        let table = ToolsButtonMap[_id.table];
        let undo = ToolsButtonMap[_id.undo];
        let redo = ToolsButtonMap[_id.redo];
        if (!data) {
          domUtils.addClass([bold, italic, link, quotes, listOl, listUl, code, table, undo, redo], _class.disabled);
          return;
        } else if ((code && data.isInCode === '1') ||
          (table && data.isInTable === '1')) {
          domUtils.addClass([bold, italic, link, quotes, listOl, listUl, code, table], _class.disabled);
        } else {
          domUtils.removeClass([bold, italic, link, quotes, listOl, listUl, code, table], _class.disabled);
        }
  ...
  let _view = {
    checkBlock: (block) => {
      ...
      // 检查 block 是否合法
      return !(domUtils.getParentByClass(block, CONST.CLASS.CODE_CONTAINER, true) ||
        domUtils.getParentByClass(block, CONST.CLASS.TABLE_CONTAINER, true));

    },
    checkDomInList: (dom, list) => {
    ...
    clearImgUploadingTmp: (imgId) => {
      let tmpImg = _view.getImgUploadingTmp(imgId);
      if (tmpImg) {
        domUtils.remove(tmpImg);
      }
    },
    createSubMenu: (button) => {
      let subMenu = button.querySelector('.' + _class.subMenu);
      if (subMenu) {
        return subMenu;
      }
    ...
    createTableSubMenu: (subMenu) => {
      let table = subMenu.querySelector('.' + _class.subMenuTable);
      let label, tbody;
      let selectedList;
      if (table) {
        // 每次都要清理之前已经选中的区域
      ...
      for (let i = 1; i < 11; i++) {
        let tr = env.doc.createElement('tr');
        for (let j = 1; j < 11; j++) {
          let td = env.doc.createElement('td');
          td.setAttribute('data-index', i + '-' + j);
          td.setAttribute('unselectable', 'on');
          td.setAttribute('onmousedown', 'return false;');
          tr.appendChild(td);
        }
    ...
    createToolbar: () => {
      ...
      let mainToolsDiv = env.doc.createElement('div');
      domUtils.addClass(mainToolsDiv, CONST.CLASS.WIZ_EIDTOR_TOOlBAR_MAIN);
      let sideToolsDiv = env.doc.createElement('div');
      ...
      let mainTools = MainTools;
      let sideTools = SideTools;
      utils.forEach(mainTools, (tool) => {
        mainToolsDiv.appendChild(_view.createToolIcon(tool));
      });
      utils.forEach(sideTools, (tool) => {
        sideToolsDiv.appendChild(_view.createToolIcon(tool));
      });
    },
    createToolIcon: (tool) => {
      let button = env.doc.createElement('div');
      let toolIcon = env.doc.createElement('i');
      let id;
      if (tool.id === _id.split) {
        ...
        toolIcon.setAttribute('unselectable', 'on');
        toolIcon.setAttribute('onmousedown', 'return false;');
        id = tool.id;
        toolIcon.id = id;
      ...
      if (tool.id === _id.image) {
        let label = env.doc.createElement('label');
        label.setAttribute('for', uploaderId);
    ...
    getImgUploadingTmp: (imgId) => {
      return env.body.querySelector('#' + _view.getImgUploadingTmpId(imgId));
    },
    getImgUploadingTmpId: (imgId) => {
      return _id.tmpImg + '_' + imgId;
    },
    getLineStartList: () => {
      let range = rangeUtils.getRange();
      let result = [], blockList = [], block, lineStart;
      if (!range) {
        return result;
      }
      //首先获取 block 块集合
      let startDetail = rangeUtils.getRangeDetail(range.startContainer, range.startOffset);
      let startBlock = domUtils.getPrevBlock((!range.collapsed && startDetail.isEnd) ? domUtils.getNextNode(startDetail.container) : startDetail.container);
      let endDetail, endBlock;
      if (!range.collapsed) {
        ...
        while (endBlock !== env.body && endBlock !== startBlock &&
        !!block && block !== env.body) {
    ...
    insertImgUploadingTmp: (imgId, imgName) => {
      let range = rangeUtils.getRange();
      if (!range) {
        return;
      }
      ...
      let emptyLine = env.doc.createElement('br');
      // emptyLine.appendChild(env.doc.createElement('br'));
      range.insertNode(emptyLine);

      let isMarkdownEditor = env.options.editor.type === 'markdown';
      let tmpImg = env.doc.createElement(CONST.TAG.TMP_TAG);
      if (isMarkdownEditor) {
        tmpImg.innerHTML = '![<span class="' + _class.imgName + '">' + imgName + '</span>](' +
          '<span class="progress_txt">0%</span>)';
      } else {
        tmpImg.innerHTML = '<div style="border:1px solid #BDBDBD;color:#BDBDBD;width:200px;text-align:center;">' +
          '<div class="' + _class.imgName + '" style="padding:8px;font-size:14px;overflow:hidden;white-space:nowrap;text-overflow:ellipsis;">' + imgName + '</div>' +
          '<div class="progress_txt" style="padding-bottom:8px;font-size:12px;">0%</div>' +
          '<div class="progress_line" style="width:0%;height:2px;background:#66BB6A;"></div>' +
          '</div>';
      }
      tmpImg.style.color = '#BDBDBD';
      tmpImg.id = _view.getImgUploadingTmpId(imgId);
      tmpImg.setAttribute('contenteditable', 'false');
      range.insertNode(tmpImg);

      range.insertNode(env.doc.createElement('br'));

      let emptyLineIndex = domUtils.getIndex(emptyLine);
    ...
    insertTextAtLineStart: (text, lineStartList) => {
      let dom, range;
      ...
      for (let i = 0; i < lineStartList.length; i++) {
        let lineStart = lineStartList[i];
        dom = env.doc.createTextNode(text);
        if (domUtils.isTag(lineStart, 'br') &&
          domUtils.isEmptyDom(lineStart.parentNode)) {
    ...
    preview: () => {
      let previewContainer = env.body.querySelector('.' + CONST.CLASS.WIZ_EIDTOR_PREVIEW_CONTAINER);
      if (!previewContainer) {
  ...
  this.setImgUploaderId = (_uploaderId) => {
    ...
    if (!env.readonly && env.frame.toolbarContainer) {
      ...
      imageIcon.parentNode.setAttribute('for', uploaderId);
    }
  };
  this.onImgUploadBegin = (imgId, imgName) => {
    _view.insertImgUploadingTmp(imgId, imgName);
  };
  this.onImgUploadComplete = (imgId, imgUrl) => {
    let tmpImg = _view.getImgUploadingTmp(imgId);
    if (!tmpImg) {
      return;
    }
    let img;
    let imgName = tmpImg.querySelector('.' + _class.imgName);
    imgName = imgName ? imgName.innerText : '';

    if (env.options.editor.type === CONST.NOTE_EDITOR_TYPE.MARKDOWN) {
      img = env.doc.createTextNode('![' + imgName + '](' + imgUrl + ')');
    } else {
      img = imgUtils.makeDomByPath(imgUrl);
    }

    domUtils.before(img, tmpImg);
    domUtils.remove(tmpImg);
  };
  this.onImgUploadError = (imgId) => {
    _view.clearImgUploadingTmp(imgId)
  };
  this.onImgUploadProgress = (imgId, progress) => {
    let tmpImg = _view.getImgUploadingTmp(imgId);
    if (!tmpImg) {
      return;
    }
    let progressTxt = tmpImg.querySelector('.progress_txt');
    let progressLine = tmpImg.querySelector('.progress_line');
    if (progressTxt) {
      progressTxt.firstChild.nodeValue = progress + '%';
    }
    if (progressLine) {
...
},{"../../common/MarkdownRender":12,"../../config/const":51,"../../libs/utils":59}],37:[function(require,module,exports){
/**
 * 点击 Img 操作
 */
const CONST = require('../../config/const');
...
const ImgClick = function () {
  ...
  let imgCheck = (img) => {
    const imgClassExp = new RegExp('(' + CONST.CLASS.IMG_NOT_DRAG + ')|(' + CONST.CLASS.IMG_ATTACHMENT + ')', 'i');
    let result = !!(img && domUtils.isTag(img, 'img') && !imgClassExp.test(img.className) && img.src);
    if (!result) {
      return result;
    }
    ...
    if (env.readonly) {
      ...
      p = domUtils.getParentByFilter(img, (node) => {
  ...
  let clickImgForEdit = (e) => {
    ...
    // 删除之前遗漏的 img 状态
    let imgList = env.body.querySelectorAll('img[' + CONST.ATTR.IMG_EDITING + ']'),
      attr = {};
...
},{"../../config/const":51,"../../libs/utils":59}],38:[function(require,module,exports){
/**
 * img 组件注册
 */
const ImgCore = function () {
  let core = null;
  let env = null;
  let imgClick = null;
  let imgResize = null;
  ...
  this.setImgFullPath = () => {
    ...
    let imgList = env.body.querySelectorAll('img');
    let indexFilesPathReg = new RegExp('^(\\.\\/)?' + env.options.indexFilesPath.escapeRegex() + '\\/', 'i');
...
const ImgResize = function () {
  ...
  let createHandles = () => {
    let container = getHandleContainer();
    if (container) {
      return container;
    }
    ...
    for (let i = 0; i < handleSuffix.length; i++) {
      let handle = env.doc.createElement('div');
  ...
  let getHandleContainer = () => {
    let container = env.body.querySelector('.' + CONST.CLASS.IMG_RESIZE_CONTAINER);
  ...
  let removeImgAttributes = () => {
    let imgList = env.body.querySelectorAll('.' + CONST.CLASS.IMG_RESIZE_ACTIVE);
  ...
  let resetHandlesSize = (img) => {
    ...
    let handles = container.querySelectorAll('.' + CONST.CLASS.IMG_RESIZE_HANDLE);
  ...
  let scaleImgSize = (rate, widthDraged, heightDraged, img) => {
    if (!img) {
      return;
    }

    let widthSized = heightDraged * rate;
    let heightSized = widthDraged / rate;
  ...
  let setHandleSize = (imgOptions, handle) => {
    if (!imgOptions || !handle) {
      return;
    }
    let offset = imgOptions.offset;
    let x = offset.left, y = offset.top,
      width = imgOptions.width, height = imgOptions.height;

    let handleName = handle.getAttribute('data-type');
    let left = 0, top = 0;
    switch (handleName) {
      case 'lt':
        left = x - 7;
  ...
  let _event = {
    ...
    handler: {
      ...
      onContainerMouseDown: (e) => {
        ...
        let img = env.body.querySelector('.' + CONST.CLASS.IMG_RESIZE_ACTIVE);
...
},{"../../config/const":51,"../../libs/utils":59}],40:[function(require,module,exports){
/**
 * img 操作基本方法集合
 */
const CONST = require('../../config/const');
...
const ImgUtils = function () {
  ...
  this.getAll = (onlyLocal) => {
    let images = env.doc.images,
      imageSrcs = [],
      tmp = {};
    for (let img in images) {
      if (images.hasOwnProperty(img)) {
        //有特殊字符的文件名， 得到src 时是被转义后的名字，所以必须 decode 处理
        let src = decodeURIComponent(images[img].src);
  ...
  this.makeAttachmentDom = (guid, imgPath) => {
    let result = [], main, a, img;
  ...
  this.makeDomByPath = (imgPath) => {
    ...
    for (let i = 0, j = paths.length; i < j; i++) {
      let main = env.doc.createElement("div");
      result.push(main);
      let img = env.doc.createElement("img");
      img.src = paths[i];
      img.style.verticalAlign = 'bottom';
  ...
  this.remove = (selector) => {
    let imgList = env.body.querySelectorAll(selector);
    for (let i = imgList.length - 1; i >= 0; i--) {
      let img = imgList[i];
      img.parentElement.removeChild(img);
    }
  };
  this.replaceSrc = (selector, targetSrc) => {
    let imgList = env.body.querySelectorAll(selector),
      attr = {};
    attr[CONST.ATTR.IMG_EDITING] = '';
    for (let i = imgList.length - 1; i >= 0; i--) {
...
},{"../../config/const":51}],41:[function(require,module,exports){
/**
 * 阅读器 基础工具包
 */
const CONST = require('../../config/const');
...
const Reader = function() {
  ...
  let setDomReadOnly = (tag, readonly) => {
    let domList = env.body.getElementsByTagName(tag);
    for (let i = 0; i < domList.length; i++) {
      let obj = domList[i];
  ...
  this.off = () => {
    ...
    } else if (env.options.container && env.options.noFrame) {
...
},{"../../config/const":51}],42:[function(require,module,exports){
/**
 * reader 使用的基本事件处理
 */
const CONST = require('../../config/const');
...
const ReaderEvent = function () {
  ...
  let copySelection = (e) => {
    ...
    if (fragment) {
      ...
      let hackDiv = env.doc.createElement(CONST.TAG.TMP_TAG);
      let field = env.doc.createElement('textarea');
      field.value = text;
...
},{"../../config/const":51,"../../libs/utils":59}],43:[function(require,module,exports){
/**
 * 表格操作核心包 core
 */
const CONST = require('../../config/const');
...
const TableCore = function () {
  ...
  let _event = {
    ...
    handler: {
      afterRestoreHistory: () => {
        ...
        for (let i = 0, j = tmpCells.length; i < j; i++) {
          cells.push(tmpCells[i]);
        }

        let table = domUtils.getParentByTagName(cells[0], 'table', true, null);
        if (!table) {
          tableZone.clear();
          return;
        }
      ...
      onDragStart: (e) => {
        //表格内禁止拖拽操作
        let table = domUtils.getParentByTagName(e.target, 'table', true, null);
        if (table) {
          utils.stopEvent(e);
        }
      },
      onKeyDown: (e) => {
        ...
        let method = e.shiftKey ? 'extend' : 'move';
        let code = e.keyCode || e.which,
          direct,
          charMove = false,
        ...
        let last, cellData, check, codeContainer, cm;
        ...
        if (charMove) {
          ...
          if (charMove) {
            ...
            zone.start.cell === zone.end.cell &&
            zone.start.y_src + zone.start.cell.rowSpan >= zone.grid.length) {
      ...
      onMouseDown: (e) => {
        ...
        let isMenu = tableUtils.isMenu(e.target);
        if (isMenu) {
          return;
        }

        let cell = domUtils.getParentByTagName(e.target, ['th', 'td'], true, null);
        let table = cell ? domUtils.getParentByTagName(cell, 'table', false, null) : null;
        let pos = tableUtils.getMousePosition(e, table);
        let isZoneBorder = tableZone.isZoneBorder(e);
      ...
      onMouseOver: (e) => {
        let end = domUtils.getParentByTagName(e.target, ['td', 'th'], true, null);
        tableZone.modify(end);
      },
      onMouseUp: (e) => {
        tableUtils.fixSelection(e);

        let isLeft = (e.type !== 'mouseup' || e.button === 0 || e.button === 1);
        if (!isLeft) {
          return;
        }
        let isMenu, isZoneBorder;
        let zone = tableZone.getZone();
        ...
        let cell = domUtils.getParentByTagName(e.target, ['td', 'th'], true, null);
        tableZone.setEnd(cell);
        tableMenu.show();
      },
      onSelectionChange: (e) => {
        let check = tableUtils.checkCaretInTableContainer(e);
        let cell;
        // 保证自动修正 cell 后，还能正常显示光标，所以不再判断 zone.table
        // let zone = tableZone.getZone();
        // if (check.tableContainer && zone.table !== check.tableContainer.querySelector('table')) {
        let range = rangeUtils.getRange(), tableContainer;
        if (range && !range.collapsed) {
  ...
  this.deleteCols = () => {
    ...
    historyUtils.saveSnap(false);
    for (let i = zone.range.maxX; i >= zone.range.minX; i--) {
  ...
  this.deleteRows = () => {
    ...
    historyUtils.saveSnap(false);
    for (let i = zone.range.maxY; i >= zone.range.minY; i--) {
  ...
  this.deleteTable = () => {
    ...
    let parent = zone.table.parentNode;
    if (parent) {
      parent.removeChild(zone.table);
    }
    tableMenu.remove();
    tableZone.remove();
    parent = domUtils.getParentByFilter(parent, (dom) => {
  ...
  this.merge = () => {
    ...
    historyUtils.saveSnap(false);
    let cell = tableUtils.mergeCell(zone.grid, zone.range);
    if (cell) {
      tableZone.updateGrid();
      tableZone.setStart(cell).setEnd(cell);
    }
  ...
  this.onKeyDown = _event.handler.onKeyDown;
  this.setCellAlign = (align, valign) => {
    let zone = tableZone.getZone();
    if (!zone.range) {
      return;
    }
  ...
  this.split = () => {
    let zone = tableZone.getZone();
    let range = tableUtils.splitCell(zone.table, zone.grid, zone.range);
    if (range) {
      historyUtils.saveSnap(false);
      tableZone.updateGrid();
      zone = tableZone.getZone();
...
},{"../../config/const":51,"../../libs/utils":59}],44:[function(require,module,exports){
/*
 表格菜单 控制
 */
const CONST = require('../../config/const');
...
const TableMenu = function () {
  ...
  let createMenu = () => {
    let menu = env.body.querySelector('.' + CONST.CLASS.TABLE_TOOLS);
    if (menu) {
      return menu;
    }
      ...
      },
      {
        ...
        subMenu: {
          ...
          make: () => {
            ...
            let str = '<div class="wiz-table-menu-sub wiz-table-cell-align">';
            for (let i = 0; i < typeList.length; i++) {
              str += '<div>';
              for (let j = 0; j < typeList[i].length; j++) {
                let dataAlignType = (i === 0) ? 'valign' : 'align';
      ...
      },
      {
        ...
        subMenu: {
          ...
          make: () => {
            ...
            let str = '<div class="wiz-table-menu-sub wiz-table-color-pad">';
  ...
  let createSubMenuForList = (data) => {
    let html = '<ul class="wiz-table-menu-sub">';
  ...
  let getMenuTop = () => {
    let top, tableBody = menuObj.parentNode.querySelector('.' + CONST.CLASS.TABLE_BODY),
      tableBodyTop = tableBody ? tableBody.offsetTop : 0;
    top = tableBodyTop - menuObj.offsetHeight - 3;
    return top + 'px';
  };
  let fixMenuPos = () => {
    let container = menuObj.parentNode,
  ...
  let _event = {
    ...
    handler: {
      ...
      onClick: (e) => {
        //点击 一级菜单
        let item = domUtils.getParentByFilter(e.target, (dom) => {
          return domUtils.hasClass(dom, CONST.CLASS.TABLE_MENU_BUTTON);
        }, true);
        if (item) {
          this.showSub(item.parentNode);
          return;
        }

        //点击 菜单具体功能
        let container;
        item = domUtils.getParentByFilter(e.target, (dom) => {
        ...
        switch (type) {
          ...
          case CONST.TYPE.TABLE.SET_CELL_BG:
            let bg = item.getAttribute('data-color');
            tableCore.setCellBg(bg);
            container = domUtils.getParentByFilter(item, (dom) => {
      ...
      onMouseOver: (e) => {
        let colorItem = domUtils.getParentByFilter(e.target, (dom) => {
  ...
  this.hideSub = () => {
    ...
    let sub = menuObj.querySelectorAll('.' + CONST.CLASS.TABLE_MENU_ITEM + '.' + _class.active);
  ...
  this.show = () => {
    ...
    let container = domUtils.getParentByFilter(zone.table, (dom) => {
      return domUtils.hasClass(dom, CONST.CLASS.TABLE_CONTAINER);
    }, false);
    menuObj = createMenu();
  ...
  this.showSub = (item) => {
    ...
    //控制二级菜单 默认值
    let canMerge, canSplit, cellAlign,
...
},{"../../config/const":51}],45:[function(require,module,exports){
/**
 * 表格操作的基本方法集合
 */
const CONST = require('../../config/const');
...
const TableUtils = function () {
   ...
   * 判断当前是否允许新建表格
   */
  this.canCreateTable = (zone) => {
    let range = rangeUtils.getRange(), tmpCell;
    if (range) {
   ...
   * 判断选择的单元格是否能被拆分
   */
  this.canSplit = (grid, range) => {
    if (!grid || !range) {
      return false;
    }
    let splitMap = {}, canSplit = false;
    this.eachRange(grid, range, (cellData) => {
      let key = cellData.x_src + '_' + cellData.y_src;
   ...
   */
  this.checkCaretInTableContainer = (e) => {
    const eType = /^(mouse|touch)/i;
    let result = {
      tableContainer: null,
      tableMenu: false,
      before: false,
    ...
    if (domUtils.isTag(target, 'table')) {
      this.initTableContainer(target);
    }

    if (this.isMenu(target)) {
      result.tableContainer = this.getContainerExcludeTable(target);
      result.tableMenu = true;
      return result;
    }

    tableContainer = this.getContainerExcludeTable(target);
   ...
   * 清空选中区域单元格内的数据
   */
  this.clearCellValue = (grid, range) => {
    if (!grid || !range) {
      return;
    }
    this.eachRange(grid, range, (cellData) => {
   ...
   * 复制单元格 Dom
   */
  this.cloneCell = (cell, isClear) => {
    let newCell = env.doc.createElement(cell.tagName);
   ...
   * 创建 表格
   */
  this.createTable = (col, row) => {
    if (!col || !row) {
      return;
    }

    let table = env.doc.createElement('table'),
      tbody = env.doc.createElement('tbody');

    for (let r = 0; r < row; r++) {
      let tr = env.doc.createElement('tr');
      for (let c = 0; c < col; c++) {
        tr.appendChild(this.createCell(env.options.table.colWidth));
      }
      tbody.appendChild(tr);
    }
   ...
   * 删除指定的列
   */
  this.deleteCols = (grid, col) => {
    if (!grid || grid.length === 0 || col > grid[0].length) {
      return;
    }
    let table = domUtils.getParentByTagName(grid[0][0].cell, 'table', false, null);

    let tmpCellList = [], width = env.options.table.colWidth;
    ...
    for (let y = 0; y < grid.length; y++) {
      ...
      if (g.y_src === y && g.cell.colSpan > 1) {
        ...
        width = this.getCellWidth(g.cell);
        g.cell.parentElement.removeChild(g.cell);
      }
      grid[y].splice(col, 1);
    }

    for (let y = 0; y < tmpCellList.length; y++) {
      let cell = tmpCellList[y];
      cell.style.width = (this.getCellWidth(cell) - width) + 'px';
    }

    //如果所有单元格都删除了，则删除表格
    if (!table.getElementsByTagName('td').length &&
      !table.getElementsByTagName('th').length) {
  ...
  this.deleteRows = (grid, row) => {
    ...
    let table = domUtils.getParentByTagName(grid[0][0].cell, 'table', false, null),
      rows = table.rows;
    for (let x = grid[row].length - 1; x >= 0; x--) {
      let g = grid[row][x];
      if (g.x_src === x && g.y_src < g.y) {
        g.cell.rowSpan--;
      } else if (g.x_src === x && g.y_src === g.y &&
        g.cell.rowSpan > 1 && (row + 1) < grid.length) {
        //row+1 防止表格异常的 rowSpan 设置
        g.cell.rowSpan--;
        let cellData = this.getNextCellDataInRow(grid[row + 1], x);
   ...
   * 平均分配每列
   */
  this.distributeCols = (table, grid) => {
    if (!table || !grid) {
      return;
    }
    ...
    for (let y = rows.length - 1; y >= 0; y--) {
      for (let x = rows[y].cells.length - 1; x >= 0; x--) {
   ...
   * each 循环遍历 选中区域的单元格
   */
  this.eachRange = (grid, range, callback) => {
    if (!grid || !range || !callback || typeof callback !== 'function') {
      return;
    }
    let cbBreak = true;
    for (let y = range.minY; cbBreak !== false && y < grid.length && y <= range.maxY; y++) {
      for (let x = range.minX; cbBreak !== false && x < grid[y].length && x <= range.maxX; x++) {
  ...
  this.fixColTimer = -1;
  this.fixColWidth = (table, tableWidth, cells, cellMap, dx) => {
    if (dx >= 0) {
      return;
    }
    ...
    if (this.fixColTimer) {
      clearTimeout(this.fixColTimer);
    }
    this.fixColTimer = setTimeout(() => {
      let maxDx = dx;
      table.style.width = tableWidth + 'px';
      for (let i = 0, j = cells.length; i < j; i++) {
        let g = cells[i];
        let tmpDx = g.cell.offsetWidth - cellMap[this.getKeyForColWidth(g)] + dx;
      ...
      if (maxDx > dx) {
        ...
        for (let i = 0, j = cells.length; i < j; i++) {
          let g = cells[i];
          g.cell.style.width = cellMap[this.getKeyForColWidth(g)] - dx + maxDx + 'px';
  ...
  this.fixSelection = () => {
    ...
    let start = range.startContainer,
      ...
      endOffset = range.endOffset,
      startTr = this.getContainer(start),
      endTr = this.getContainer(end);
    if ((!startTr && !endTr) || (startTr && endTr)) {
      return;
    }

    let table, target = startTr ? startTr : endTr;

    while (table = this.getContainer(target)) {
      if (startTr) {
  ...
  this.fixTableWidth = (table) => {
    ...
    let rows = table.rows, tableWidth = 0;
    for (let i = 0; i < rows[0].cells.length; i++) {
      let cell = rows[0].cells[i];
      tableWidth += this.getCellWidth(cell);
    }
    table.style.width = tableWidth + 'px';
  };
  /**
   * 获取 选中区域内综合的单元格对齐方式
   */
  this.getAlign = (grid, range) => {
    if (!grid || !range) {
      return false;
    }
    let align, valign, cell,
      result = {
        align: '',
        valign: ''
      };
   ...
   */
  this.getCellsByRange = (grid, range) => {
    let cellList = [];
    if (!grid || !range) {
      return cellList;
    }
  ...
  this.getTableBody = (target) => {
    return domUtils.getParentByFilter(target, (dom) => {
   ...
   */
  this.getContainerExcludeTable = (target) => {
    let cell = domUtils.getParentByTagName(target, ['th', 'td'], true, null);
    return !cell ? this.getContainer(target) : null;
  };
  /**
   * 从 cell 集合中遍历获取 cell 内的叶子节点集合
   */
  this.getDomsByCellList = (cellList) => {
    let domList = [];
    if (!cellList) {
      return domList;
    }
    for (let i = 0, j = cellList.length; i < j; i++) {
      let cell = cellList[i];
      let tmpList = domUtils.getListA2B({
  ...
  this.getKeyForColWidth = (g) => {
    return g.x_src + '_' + g.y_src;
  };
  /**
   * 在表格内根据指定单元格 获取 下一个 单元格，到达最后一列时，自动从下一行查找
   */
  this.getNextCellInTable = (cell) => {
    let nextCell = cell.nextElementSibling;
    if (nextCell) {
      return nextCell;
    }
    let tr = cell.parentNode.nextElementSibling;
    while (tr) {
   ...
   * 在一行内 根据列数获取下一个单元格
   */
  this.getNextCellDataInRow = (gridRow, col) => {
    if (!gridRow) {
      return null;
    }
   ...
   */
  this.getMousePosition = (e, table) => {
    let eventClient = utils.getEventClientPos(e);
    let tableBody = this.getTableBody(table || e.target);
    let pageScroll = domUtils.getPageScroll();
    let clientX = eventClient.x + pageScroll.left + (tableBody ? tableBody.scrollLeft : 0);
    let clientY = eventClient.y + pageScroll.top + (tableBody ? tableBody.scrollTop : 0);
    return {
      x: clientX,
      y: clientY
    };
   ...
   */
  this.getRangeByCellData = (cellData) => {
    if (!cellData) {
      return {
        minX: 0,
        minY: 0,
   ...
   */
  this.getRangeByCellsData = (grid, startData, endData) => {
    if (!grid || !startData || !endData) {
      return null;
    }
    let startRange = this.getRangeByCellData(startData);
    if (startData.cell === endData.cell) {
      return startRange;
    }
    let endRange = this.getRangeByCellData(endData);
    let minX = Math.min(startRange.minX, endRange.minX),
    ...
    while (changeRange) {
      ...
      for (let y = minY; y <= maxY; y++) {
        for (let x = minX; x <= maxX; x++) {
          ...
          let g = grid[y][x];
          let k = g.x_src + '_' + g.y_src;
          if (cellMap[k]) {
            //如果该 Cell 已经被计算过，则不需要重新计算
            continue;
          }
          let gRange = this.getRangeByCellData(g);
   ...
   */
  this.getTemplateByHtmlForPaste = (html) => {
    let pasteTables,
    ...
    if (!pasteIsTable) {
      ...
      for (let i = pasteTables.length - 1; i >= 0; i--) {
        let pasteTable = pasteTables[i];
   ...
   */
  this.getTemplateByTxtForPaste = (txt) => {
    txt = (txt || '').trim();
    ...
    for (let y = 0; y < rows.length; y++) {
      let tr = rows[y];
      let cols = tr.cells;
      for (let x = cols.length; x < maxX; x++) {
   ...
   * 检查 并 初始化表格容器
   */
  this.initTableContainer = (_table) => {
    let tableList = _table ? [_table] : env.body.querySelectorAll('.' + CONST.CLASS.WIZ_BODY + ' table');

    let checkParent = (obj, filter) => {
    ...
    for (let i = 0, j = tableList.length; i < j; i++) {
      let table = tableList[i];
      let tableBody = checkParent(table, (parent) => {
        if (parent === env.body) {
          return false;
        }
      ...
      let container = checkParent(tableBody, (parent) => {
        if (parent === env.body) {
          return false;
        }
        if (parent.childNodes.length === 1) {
  ...
  this.insertCol = (grid, col) => {
    ...
    col = col || 0;
    let g, cell, newCell, nextCellData;
    let table = domUtils.getParentByTagName(grid[0][0].cell, 'table', false, null),
    ...
    for (let y = 0; y < grid.length; y++) {
      ...
      if (cell && cell !== lastCell && g.x_src < col) {
        ...
        g.cell.colSpan++;
        // 需要调整 style
        g.cell.style.width = (this.getCellWidth(g.cell) + env.options.table.colWidth) + 'px';

      } else if (!cell || (cell && g.x_src === col)) {
        newCell = this.createCell(env.options.table.colWidth);
        if (cell && g.y_src < g.y) {
          //cell.rowSpan > 1
          nextCellData = this.getNextCellDataInRow(grid[y], col);
  ...
  this.insertRow = (grid, row) => {
    ...
    let newCell;
    let tr = env.doc.createElement('tr');
    let gRow = grid[grid.length > row ? row : grid.length - 1];
    for (let x = 0; x < gRow.length; x++) {
      let g = gRow[x];
      if (grid.length > row && g.y_src < g.y && g.x_src === g.x) {
        ...
        newCell = this.cloneCell(g.cell, true);
        if (g.cell.colSpan > 1) {
    ...
    let target = gRow[0].cell.parentElement,
  ...
  this.isMenu = (dom) => {
    ...
    return !!domUtils.getParentByFilter(dom, (p) => {
   ...
   * 将指定的单元格范围进行合并
   */
  this.mergeCell = (grid, range) => {
    if (!this.canMerge(grid, range)) {
      return null;
    }
    ...
    let target = grid[range.minY][range.minX].cell;
    let temp = env.doc.createElement('div');
    this.eachRange(grid, range, (cellData) => {
   ...
   * 修改单元格 Dom 样式 & 属性
   */
  this.modifySelectionDom = (zone, style, attr) => {
    //处理单元格样式
    let align = style['text-align'] || null;
    let valign = style['text-valign'] || null;
    delete style['text-align'];
    delete style['text-valign'];

    this.eachRange(zone.grid, zone.range, (cellData) => {
      if (!cellData.fake) {
        domUtils.modifyNodesStyleAndClear(cellData.cell.childNodes, style, attr, []);
      }
    });
    if (align || valign) {
      this.setCellAlign(zone.grid, zone.range, {
        align: align,
        valign: valign
      });
    }
   ...
   * 设置单元格对齐方式
   */
  this.setCellAlign = (grid, range, _alignType) => {
    if (!grid || !range) {
      return;
    }
   ...
   * 设置单元格背景颜色
   */
  this.setCellBg = (grid, range, bgColor) => {
    if (!grid || !range) {
      return;
    }
   ...
   * 设置列宽
   */
  this.setColWidth = (table, grid, col, dx) => {
    let fixDx = () => {
      let cell, maxDx = dx;
      for (let y = 0; y < grid.length; y++) {
        let g = grid[y][col];
        let tmpDx = env.options.table.colWidthMin - g.cell.offsetWidth;
        if (g.cell.colSpan === 1) {
          maxDx = tmpDx;
          cell = g.cell;
          break;
    ...
    dx = fixDx();
    let tableWidth = table.offsetWidth + dx;
    let cells = [], cellMap = {};
    for (let i = 0, j = grid.length; i < j; i++) {
      let g = grid[i][col];
      let key = this.getKeyForColWidth(g);
      if (!cellMap[key]) {
    ...
    for (let i = 0, j = cells.length; i < j; i++) {
      let g = cells[i];
      g.cell.style.width = cellMap[this.getKeyForColWidth(g)] + 'px';
    }
    this.fixColWidth(table, tableWidth, cells, cellMap, dx);
  };
  /**
   * 设置行高
   */
  this.setRowHeight = (table, grid, row, dy) => {
    let g, cell, maxDy = dy;
    for (let x = 0; x < grid[row].length; x++) {
      g = grid[row][x];
      let tmpDy = env.options.table.rowHeightMin - g.cell.offsetHeight;
      if (g.cell.rowSpan === 1) {
        maxDy = tmpDy;
        cell = g.cell;
        break;
   ...
   * 拆分单元格
   */
  this.splitCell = (table, grid, range) => {
    let splitMap = this.canSplit(grid, range);
    if (!splitMap) {
      return null;
    }
    for (let key in splitMap) {
      if (splitMap.hasOwnProperty(key)) {
        let g = splitMap[key];
        let dy = g.cell.rowSpan;
        let dx = g.cell.colSpan;
        for (let y = g.y_src; y < g.y_src + dy; y++) {
          for (let x = g.x_src; x < g.x_src + dx; x++) {
            let item = grid[y][x];
            if (item.fake) {
              let nextCell = this.getNextCellDataInRow(grid[y], x);
              nextCell = nextCell ? nextCell.cell : null;
              let newCell = this.cloneCell(item.cell, true);
...
},{"../../config/const":51,"../../libs/utils":59}],46:[function(require,module,exports){
/**
 * 表格选择区域 控制
 */
const CONST = require('../../config/const');
...
const TableZone = function () {
  ...
  let updateRenderTimer, updateRenderTimes,
    domModifiedTimer;
  let clearSelectedCell = () => {
    if (!zone.table) {
      return;
    }
    let cells = zone.table.getElementsByClassName(CONST.CLASS.SELECTED_CELL);
  ...
  let colLineRender = (x) => {
    ...
    let rangeBorder = getRangeBorder();
    let minX = rangeBorder.colLine.minLeft;
    if (x < minX) {
      x = minX;
    }
  ...
  let getDomById = (parent, id, tagName) => {
    let dom = parent.querySelector('#' + id);
    if (!dom) {
  ...
  //   return false;
  // };
  let initTableContainer = (rangeBorder) => {
    tableUtils.initTableContainer(zone.table);
    let tableBody = zone.table.parentNode;
  ...
  let isSingleCell = () => {
    ...
    let cellA = zone.grid[zone.range.minY][zone.range.minX],
  ...
  let rangeRender = () => {
    ...
    let topSrc = env.doc.body.clientTop;
    let leftSrc = env.doc.body.clientLeft;
    let sLeft, sTop, sWidth, sHeight;
    let rLeft, rTop, rWidth, rHeight;

    let rangeCellStart = zone.start ? zone.start.cell : null;
    let rangeCell_A = zone.grid[zone.range.minY][zone.range.minX];
    let rangeCell_B = zone.grid[zone.range.maxY][zone.range.maxX];
  ...
  let rowLineRender = (y) => {
    ...
    let rangeBorder = getRangeBorder();
    let minY = rangeBorder.rowLine.minTop;
    if (y < minY) {
      y = minY;
    }
  ...
  let selectCellsData = (cellsData) => {
    if (!cellsData) {
      return;
    }
    for (let i = 0, j = cellsData.length; i < j; i++) {
  ...
  let _event = {
    ...
    handler: {
      onDragLineMove: (e) => {
        let rangeBorder = getRangeBorder();
        let pos = tableUtils.getMousePosition(e, zone.table);
      ...
      onDragLineEnd: (e) => {
        ...
        let pos = tableUtils.getMousePosition(e, zone.table);
        let cellData;
        let isDragCol = rangeBorder.colLine.style.display === 'block';
        let isDragRow = rangeBorder.rowLine.style.display === 'block';
      ...
      onDomModified: (e) => {
        let needAutoRetry = (e && e.type === 'DOMSubtreeModified' &&
          e.target.nodeType === 1 && e.target.querySelector('img'));
        if (domModifiedTimer) {
          clearTimeout(domModifiedTimer);
        }
        domModifiedTimer = setTimeout(() => {
      ...
      updateRender: (e, needAutoRetry) => {
        let autoUpdate = (needAutoRetry) => {
  ...
  this.getFragmentForCopy = () => {
    ...
    let table = env.doc.createElement('table'),
      tbody = env.doc.createElement('tbody');
    table.appendChild(tbody);
    for (let y = zone.range.minY; y <= zone.range.maxY; y++) {
      let tr = env.doc.createElement('tr');
      for (let x = zone.range.minX; x <= zone.range.maxX; x++) {
        let g = zone.grid[y][x];
        if (!g.fake) {
          let td = tableUtils.cloneCell(g.cell, false);
  ...
  this.isRangeActiving = () => {
    return zone.start && zone.active;
  };
  this.isSingleCell = isSingleCell;
  this.isZoneBorder = (e) => {
    let obj = e.target, x = e.offsetX, y = e.offsetY,
      eventClient = utils.getEventClientPos(e);
    let isScroll,
      isBodyBorder = false,
      isBorder = false,
    ...
    let isDot = !!domUtils.getParentByFilter(obj, (dom) => {
      return dom && dom.nodeType === 1 &&
        (dom.id === CONST.ID.TABLE_RANGE_BORDER + '_start_dot' ||
          dom.id === CONST.ID.TABLE_RANGE_BORDER + '_range_dot');
    }, true);

    if (!isDot) {
      isRight = !!domUtils.getParentByFilter(obj, (dom) => {
        if (dom && dom.nodeType === 1 &&
          (dom.id === CONST.ID.TABLE_RANGE_BORDER + '_start_right' ||
            dom.id === CONST.ID.TABLE_RANGE_BORDER + '_range_right')) {
          return true;
        }
    ...
    if (!isDot && !isRight) {
      isBottom = !!domUtils.getParentByFilter(obj, (dom) => {
        if (dom && dom.nodeType === 1 &&
    ...
    if (!isBottom && !isDot && !isRight) {
      isBorder = !!domUtils.getParentByFilter(obj, (dom) => {
        return dom && dom.nodeType === 1 && dom.id === CONST.ID.TABLE_RANGE_BORDER;
      }, true);
    }

    let bodyStyle, bodyLeft, bodyRight;
    if (!isBottom && !isDot && !isRight && !isBorder) {
      ...
      if (!isContainer && obj === env.body) {
        ...
        isBodyBorder = eventClient.x <= bodyLeft || eventClient.x >= (env.body.offsetWidth - bodyRight);
      }
    }
  ...
  this.modify = (endCell) => {
    ...
    // console.log('modify');
    let table = domUtils.getParentByTagName(endCell, ['table'], true, null);
    if (!table || table !== zone.table) {
      return this;
    }
    let endCellData = tableUtils.getCellData(zone.grid, endCell);
    zone.range = tableUtils.getRangeByCellsData(zone.grid, zone.start, endCellData);
    zone.end = endCellData;

    rangeRender();

    let tableBody = domUtils.getParentByFilter(zone.table, (dom) => {
      return domUtils.hasClass(dom, CONST.CLASS.TABLE_BODY);
    }, false);
  ...
  this.remove = () => {
    this.clear();
    let rangeBorder = getRangeBorder(), parent;
    if (rangeBorder) {
  ...
  this.setStart = (startCell, curX, curY) => {
    // console.log('setStart');
    if (!startCell) {
      this.clear();
      return this;
    }
    let table = domUtils.getParentByTagName(startCell, ['table'], true, null);
    if (!table) {
      //防止异常的 标签
      this.clear();
      return this;
  ...
  this.startDragColLine = (cell, x) => {
    let table, cellData;
    if (cell && cell.nodeType === 1 &&
      cell.id === CONST.ID.TABLE_RANGE_BORDER + '_start_right') {
      cellData = zone.start;
      cell = zone.start.cell;
      table = zone.table;
    } else if (cell && cell.nodeType === 1 &&
      cell.id === CONST.ID.TABLE_RANGE_BORDER + '_range_right') {
    ...
    let col = cellData.x, nextCellData;
    ...
    let startLeft = cell.offsetLeft + cell.offsetWidth;
    let rangeBorder = getRangeBorder();
    initTableContainer(rangeBorder);
  ...
  this.startDragRowLine = (cell, y) => {
    let table, cellData;
    if (cell && cell.nodeType === 1 &&
      cell.id === CONST.ID.TABLE_RANGE_BORDER + '_start_bottom') {
      cellData = zone.start;
      cell = zone.start.cell;
      table = zone.table;
    } else if (cell && cell.nodeType === 1 &&
      cell.id === CONST.ID.TABLE_RANGE_BORDER + '_range_bottom') {
      cellData = zone.grid[zone.range.maxY][zone.range.maxX];
      cell = cellData.cell;
      table = zone.table;
    } else {
    ...
    let row = cellData.y, nextCellData;
    ...
    let startTop = cell.offsetTop + cell.offsetHeight;
    let rangeBorder = getRangeBorder();
    initTableContainer(rangeBorder);
  ...
  this.switchCell = (target, direct) => {
    ...
    let x = target.x + direct.x;
    let y = target.y + direct.y;
    changeRowCheck();
...
},{"../../config/const":51,"../../libs/utils":59}],47:[function(require,module,exports){
/**
 * todolist 操作核心包 core
 */
const CONST = require('../../config/const');
...
const TodoCore = function() {
  let core = null;
  let env = null;
  let domUtils = null;
  let historyUtils = null;
  let rangeUtils = null;
  let todoRouteForClient = null;
  let todoStyle = null;
  let todoUtils = null;
  ...
  let todoRoute = null;
  let curTouchTarget = null;

  let checkTodo = (checkbox, e) => {
    let result = todoUtils.checkTodo(checkbox, todoRoute);
    if (result) {
      if (env.readonly) {
  ...
  let onClickTodoCheck = (e) => {
    ...
    let layer = todoUtils.getContainerFromChild(checkbox);
    let parentLayer = todoUtils.getContainerFromChild(layer.parentNode);
    if (parentLayer) {
  ...
  let patchForReader = {
    ...
    init: () => {
      if (env.readonly) {
        ...
        let checkList = env.body.querySelectorAll('.' + CONST.CLASS.TODO_CHECKBOX);
    ...
    addModifiedId: (checkResult) => {
      if (!env.readonly) {
        return;
      }
      let checkbox = checkResult.checkbox;
      let curChecked = checkResult.checked;
      let lastChecked = patchForReader.modifiedIdList[checkbox.wizTodoIndex];
    ...
    getModifiedIdList: () => {
      let idList = [];
      for (let k in patchForReader.modifiedIdList) {
    ...
    modifyDoc: () => {
      let html = '';
      let idList = patchForReader.getModifiedIdList();
      if (idList.length === 0) {
        return html;
      }
      html = todoRoute.getOriginalDoc();
      let iframe = env.doc.getElementById(CONST.ID.IFRAME_FOR_SAVE);
      if (!iframe) {
        iframe = env.doc.createElement('iframe');
      ...
      for (let i = idList.length - 1; i >= 0; i--) {
        let id = idList[i].id;
        let checked = idList[i].checked;
        let checkbox = checkList[id];
        if (checkbox) {
          let main = todoUtils.getMainFromChild(checkbox);
  ...
  let _event = {
    ...
    handler: {
      ...
      onCheckDocLock: (cancel, needCallAgain) => {
      ...
      onKeyDown: (e) => {
        ...
        if (!range.collapsed) {
          ...
          if (rangeList && rangeList.list.length > 0) {
            ...
            for (let i = rangeList.list.length - 1; i >= 0; i--) {
        ...
        let userInfo = todoUtils.getUserInfoInDom(container);
        let lastWithoutUser;
        // 在 todoList 内按下回车时：
        ...
        let tmpRange = range.cloneContents();
        let tmpDiv = env.doc.createElement('div');
        ...
        if (main.getBoundingClientRect().top + main.clientHeight > env.doc.documentElement.clientHeight
          ...
          let mainX = main.getBoundingClientRect().left + pageScroll.left;
          let mainY = pageScroll.top + main.clientHeight;
      ...
      onSelectionChange: () => {
        ...
        let isBefore = todoUtils.isCaretBeforeCheckbox();
        if (isBefore.enable) {
...
},{"../../config/const":51,"../../libs/utils":59}],48:[function(require,module,exports){
/**
 * todolist 客户端适配接口
 */
const base64 = require('../../libs/base64');

const TodoClientRoute = function () {
  let core = null;
  let env = null;
  ...
  let winExternal, qtEditor, androidWizNote;

  let RouteForWindows = function() {
    ...
    let getUserAvatarFileName = (size) => {
      try {
  ...
  let RouteForWeb = function() {
    let getUserAlias = () => {
      return 'zzz';
    };
    let getUserAvatarFileName = () => {
...
},{"../../libs/base64":56}],49:[function(require,module,exports){
/**
 * todoList 相关样式处理
 */
const CONST = require('../../config/const');
...
const CSS = {
  ...
  common: '.' + CONST.CLASS.TODO_MAIN + ' {padding-left: 12px;line-height:30px;}' +
  'li > .' + CONST.CLASS.TODO_MAIN + ' {padding-left: 0}' +
  '.' + CONST.CLASS.TODO_CHECKED + ' {color: #666;}' +
  '.' + CONST.CLASS.TODO_UNCHECKED + ' {text-decoration: initial;}' +
  '.' + CONST.CLASS.TODO_CHECKED + ' .' + CONST.CLASS.TODO_CHECKBOX + ' {background-image:url(' + ImgFile.todoChecked + ')}' +
  '.' + CONST.CLASS.TODO_UNCHECKED + ' .' + CONST.CLASS.TODO_CHECKBOX + ' {background-image:url(' + ImgFile.todoUnChecked + ')}' +
  '.' + CONST.CLASS.TODO_CHECKBOX + ' {border-radius:0;position:relative;top:-1px;vertical-align:middle;border:0;background-color:transparent;outline:none;width:18px !important; height:18px !important; cursor:default; padding:0px 10px 0px 5px;-webkit-user-select: none;background-size:18px;background-repeat:no-repeat;background-position:5px;box-sizing:initial;}' +
  '.' + CONST.CLASS.TODO_AVATAR + ' {border:0;background-color:transparent;outline:none;width:20px !important; height: 20px !important; vertical-align: -20%; padding:0; margin:0 10px 0 0; border-radius:100%;background-size:20px;background-repeat:no-repeat;}' +
  '.' + CONST.CLASS.TODO_USER_INFO + ' {padding-left: 20px;}' +
  //单独出来主要为了兼容旧的 todoList
  'input.' + CONST.CLASS.TODO_AVATAR + ' {position:relative;top:-4px;}' +
  '.' + CONST.CLASS.TODO_ACCOUNT + ', .' + CONST.CLASS.TODO_DATE + ' { color: #666; }'
};

const TodoStyle = function () {
  let core = null;
  let env = null;
  let domUtils = null;
  let wizStyle = null;
  ...
  let getGuidFromStyleId = (styleId) => {
    const guidReg = new RegExp('^' + CONST.ID.TODO_AVATAR_STYLE + '(.*)$', 'i');
    return styleId.replace(guidReg, '$1');
  };

  this.insertTodoStyle = (isForced) => {
    this.removeTodoOldStyle();
    let s = env.doc.getElementById(CONST.ID.TODO_STYLE);
    if (isForced || !s) {
      wizStyle.replaceStyleById(CONST.ID.TODO_STYLE, CSS.common, false);
    }
    this.removeUnUsedTodoStyle();

    let tmpTagId = 'wiz_todo_checkbox_img_preview';
    if (env.client.type.isIOS && !env.body.querySelector('#' + tmpTagId)) {
      ...
      let tmpTag = env.doc.createElement(CONST.TAG.TMP_TAG);
      let imgChecked = env.doc.createElement('img');
      let imgUnChecked = env.doc.createElement('img');
  ...
  this.removeTodoStyle = () => {
    this.removeTodoOldStyle();
    let style = env.doc.getElementById(CONST.ID.TODO_STYLE);
    domUtils.remove(style);
    let styleList = env.doc.querySelectorAll('style');
    for (let i = styleList.length - 1; i >= 0; i--) {
      style = styleList[i];
      if (style.id && style.id.indexOf(CONST.ID.TODO_AVATAR_STYLE) === 0) {
        let guid = getGuidFromStyleId(style.id);
        TodoStyleMap[guid] = style.innerHTML;
        domUtils.remove(style);
      }
    }
  };
  this.removeUnUsedTodoStyle = (guid) => {
    let styleList = env.doc.querySelectorAll('style'),
      style, sId, sClass, userAvatar;
   ...
   */
  this.restoreUserAvatarStyle = () => {
    for (let guid in TodoStyleMap) {
      if (TodoStyleMap.hasOwnProperty(guid)) {
        let styleId = CONST.ID.TODO_AVATAR_STYLE + guid;
        if (!env.doc.querySelector('#' + styleId) &&
          env.body.querySelector('.' + CONST.CLASS.TODO_USER_AVATAR + guid)) {
...
},{"../../config/const":51}],50:[function(require,module,exports){
/**
 * todolist 基本工具包
 */
const CONST = require('../../config/const');
...
// 1像素 的透明图片
const checkboxImg = 'data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw==';

const TodoUtils = function () {
  let core = null;
  let env = null;
  let lang = null;
  let domUtils = null;
  ...
  this.addUserInfo = (main, isChecked, todoId, todoRoute) => {
    if (!main) {
      return;
    }

    let userGuid, userName, avatarUrl, dt,
      userHtml, span, next;

    userGuid = this.deleteUserInfo(main.parentNode);
    if (!isChecked) {
    ...
    dt = this.getTime();
    this.setUserAvatarStyle(userGuid, avatarUrl);
    userHtml = this.getUserInfoHtml(userGuid, userName, dt);
  ...
  this.canCreateTodo = () => {
    ...
    let rangeList = rangeUtils.getRangeDomList({
      noSplit: true
    });

    // 判断当前光标范围内是否只有一个 块结构，否则不做任何操作
    let start = domUtils.getBlockParent(rangeList.startDom, true) || env.body,
      end = null;
    if (!range.collapsed && rangeList.endDom !== rangeList.startDom) {
      end = domUtils.getBlockParent(rangeList.endDom, true) || env.body;
      if (end !== start) {
        return false;
      }
    } else if (range.collapsed && start === env.body) {
      start = env.body.childNodes[0];
    }
  ...
  this.cancelTodo = (container, noSetRange) => {
    if (!container) {
      return;
    }
    let range = rangeUtils.getRange(), start, startOffset,
      main, todoFirst, userGuid;
    start = range ? range.startContainer : null;
    startOffset = range ? range.startOffset : 0;
    main = this.getMainInDom(container);
    userGuid = this.deleteUserInfo(container);
    todoStyle.removeUnUsedTodoStyle(userGuid);
    todoFirst = this.deleteMain(main);
    ...
    if ((!todoFirst || domUtils.isEmptyDom(container)) && !container.querySelector('br')) {
    ...
    //修正 todoList style
    this.checkTodoStyle(false);
  };
  this.check = (main, isChecked) => {
    let layer = this.getContainerFromChild(main);
    if (isChecked) {
  ...
  this.checkTodo = (checkbox, todoRoute) => {
    ...
    let main = this.getMainFromChild(checkbox),
      isChecked;
    if (!main || main.children[0] !== checkbox) {
      this.fixCheckbox(checkbox, false);
    }
    ...
    if (!todoRoute.isPersonalDocument()) {
      this.addUserInfo(main, isChecked, checkbox.id, todoRoute);
    }

    return result;
  };
  this.checkTodoStyle = (isForced) => {
    let todoObj = env.doc.querySelector('.' + CONST.CLASS.TODO_CHECKBOX);
    if (todoObj) {
  ...
  this.clearBlock = (dom) => {
    ...
    for (let i = 0; i < dom.childNodes.length; i++) {
      ...
      if (this.clearBlock(child) && child !== dom.childNodes[i]) {
        i--;
      }
    }

    let isFragment = dom.nodeType === 11,
      // isMain = isFragment ? false : this.isMain(dom),
      isTodoTag = isFragment ? false : this.isTodoTag(dom),
   ...
   * 复制 todo
   */
  this.cloneTodo = (container) => {
    if (!container) {
      return null;
    }
    let _container, _main, _last,
      main, last;
  ...
  this.deleteMain = (main) => {
    return domUtils.peelDom(main, (dom) => {
      return (!this.isMain(dom) && !this.isCheckbox(dom))
    });
  };
  this.deleteUserInfo = (container) => {
    let userGuid = '';
    let main = this.getMainInDom(container);
    let userAvatar = container.querySelector('.' + CONST.CLASS.TODO_AVATAR),
    ...
    let nextSib = main ? main.nextElementSibling : container.firstChild;
    while (nextSib) {
      if (this.isMain(nextSib)) {
        break;
      }
      if (this.isUserInfo(nextSib)) {
        let tmpNode = nextSib;
     ...
     */
    this.fixCheckbox = (checkItem, isForOld) => {
      if (!checkItem) {
        return;
      }
      // 修正缺失 的 id
      if (!checkItem.id) {
        checkItem.id = this.getCheckId();
      }
      ...
      if (!isForOld && this.getMainFromChild(checkItem)) {
        return;
      }

      let container = domUtils.getBlockParent(checkItem),
        canBeContainer = this.canBeContainer(container),
      ...
      let layer = this.getContainerFromChild(newContainer.parentNode);
      if (layer) {
  ...
  this.fixMain = (main) => {
    ...
    let last = domUtils.getLastDeepChild(main),
      check = this.getCheckbox(main),
      parent, i;
    ...
    parent = this.getContainerFromChild(main);
    ...
    //处理 被嵌套的 main
    let subMainList = main.querySelectorAll('.' + CONST.CLASS.TODO_MAIN);
  ...
  this.fixNewTodo = (targetTodo) => {
    ...
    let fixEmptyLayer = (_container) => {
      ...
      checkItem = this.getCheckbox(_container);
      if (!checkItem) {
    ...
    for (let i = 0; i < todoList.length; i++) {
      ...
      if (this.isTodoTag(subParent) || this.isLayer(subParent)) {
        start = subTodoItem;
      } else {
        start = subParent;
      }
    ...
    for (let i = 0; i < mainList.length; i++) {
      let mainItem = mainList[i];
      this.fixMain(mainItem);
    }
    ...
    for (let i = 0; i < checkList.length; i++) {
      ...
      this.fixCheckbox(checkItem, false);
    }

    //修正结尾的 <br>
    if (targetTodo) {
      let lastDom = domUtils.getLastDeepChild(container);
      if (domUtils.isTag(lastDom, 'br')) {
        domUtils.remove(lastDom);
      }
    }
  };
  this.getCheckbox = (main) => {
    if (!main) {
      return null;
    }
  ...
  this.getContainerFromChild = (child) => {
    if (!child) {
      return null;
    }
    return domUtils.getParentByFilter(child, (dom) => {
      return this.isLayer(dom);
    }, true);
  };
  this.getMainByCaret = () => {
    let range = rangeUtils.getRange();
    ...
    let start = rangeUtils.getRangeDetail(range.startContainer, range.startOffset);
    return this.getMainFromChild(start.container);
    //
  ...
  this.getMainHtml = () => {
    let str = '<span class="' + CONST.CLASS.TODO_MAIN + ' ' + CONST.CLASS.TODO_UNCHECKED + '">' +
      '<img src="' + checkboxImg + '" id="%1" class="' + CONST.CLASS.TODO_CHECKBOX + ' ' + CONST.CLASS.IMG_NOT_DRAG + '" ' +
      CONST.ATTR.TODO_CHECK + '="unchecked" />' +
      '</span>';
    str = str.replace('%1', this.getCheckId());
    return str;
  };
  this.getMainInDom = (dom) => {
  ...
  this.getUserInfoFromChild = (dom) => {
    if (!dom) {
      return null;
    }
    return domUtils.getParentByClass(dom, CONST.CLASS.TODO_USER_INFO, true);
  };
  this.getUserInfoInDom = (dom) => {
    return dom.querySelector('.' + CONST.CLASS.TODO_USER_INFO);
  };
  this.getUserInfoHtml = (userGuid, userName, dt) => {
    let html = '<span class="' + CONST.CLASS.TODO_ACCOUNT + '">' +
      '<input disabled readonly class="%1" />' +
      '%2, ' +
      '</span>' +
      '<span class="' + CONST.CLASS.TODO_DATE + '">%3.</span>';
    let avatarClass = CONST.CLASS.TODO_USER_AVATAR + base64.encode(userGuid);
   ...
   */
  this.isCaretAfterCheckbox = () => {
    let range = rangeUtils.getRange();
    if (!range) {
      return false;
    }
    let start, prev, main, str;

    main = this.getMainByCaret();
    if (!main) {
      return false;
    }
    if (range.collapsed) {
  ...
  this.isCaretBeforeCheckbox = () => {
    ...
    let caretDom = range.endContainer;
    if (caretDom.nodeType === 1) {
      ...
      && range.endOffset === caretDom.nodeValue.length && !this.getContainerFromChild(caretDom)) {
      // 如果 textNode 本身已经在 todoContainer 以内 则不进行处理
      caretDom = domUtils.getNextNode(caretDom, false);
      if (caretDom) {
        caretDom = domUtils.getParentByFilter(caretDom, (dom) => {
          return this.isLayer(dom);
        }, true);
      }
    }

    if (this.isLayer(caretDom) || this.isMain(caretDom)) {
      result.enable = true;
      result.checkbox = this.getCheckbox(caretDom);
    } else if (this.isCheckbox(caretDom)) {
      result.enable = true;
  ...
  this.isEmptyContainer = (container) => {
    if (!container) {
      return true;
    }
    ...
    for (let i = 0; i < childNodes.length; i++) {
      let child = childNodes[i];
      if (this.isMain(child)) {
        if (!this.isEmptyMain(child)) {
          return false;
        }
  ...
  this.isEmptyMain = (main) => {
    ...
    for (let i = 0; i < childNodes.length; i++) {
      let child = childNodes[i];
      if (!this.isCheckbox(child) && !domUtils.isEmptyDom(child)) {
        return false;
      }
    }
    return true;
  ...
  this.setTodo = (container, todoRoute) => {
    let rangeList, start, end,
    ...
    if (container) {
      ...
      let canCreate = this.canCreateTodo();
      if (canCreate === false) {
        return null;
      }
      ...
      // start = domUtils.getBlockParent(rangeList.startDom, true) || env.body;
      //
      // if (!range.collapsed && rangeList.endDom != rangeList.startDom) {
      //     end = domUtils.getBlockParent(rangeList.endDom, true) || env.body;
      //     if (end != start) {
      ...
      if (start !== env.body) {
        ...
        let areaStart = domUtils.getPrevBlock(start);
        let areaEnd = domUtils.getNextBlock(start);
    ...
    // 首先清理 start 内的 block
    for (let i = 0, j = start.childNodes.length; i < j; i++) {
      let child = start.childNodes[i];
      if (domUtils.isBlock(child)) {
    ...
    main = this.getMainInDom(start);
    userInfo = this.getUserInfoInDom(start);
    let hasCheckbox = !!this.getCheckbox(main);
    if (main || userInfo) {
      this.cancelTodo(start);
    }
    if (hasCheckbox) {
      return null;
    }

    mainHtml = this.getMainHtml();
    tmpDom = env.doc.createElement('div');
    domUtils.addClass(tmpDom, CONST.CLASS.TODO_LAYER);
    if (!this.canBeContainer(start)) {
    ...
    main = this.getMainInDom(tmpDom);
    end = main.lastChild;

    this.insertToMain(mainChildren, main);
  ...
  this.setUserAvatarStyle = (userGuid, avatarUrl) => {
    let guid = base64.encode(userGuid);
    let sId = CONST.ID.TODO_AVATAR_STYLE + guid;
    let sClass = CONST.CLASS.TODO_USER_AVATAR + guid;
    let style = env.doc.getElementById(sId);
    if (style) {
      return;
    } else {
    ...
    domUtils.convertImageToBase64(avatarUrl, 50, 50, (baseStr) => {
      //有可能同时点击多个 todoList
      let style = env.doc.getElementById(sId);
      if (style) {
        return;
  ...
  this.oldPatch = {
    fixImg: (img) => {
      ...
      let iObj = env.doc.createElement('img');
     ...
     */
    fixOldTodo: () => {
      let subLabelList, subLabel, subContainer,
        checkImgList, labelList, tailList;

      //修正未被 label 封装的 checkImg
      checkImgList = env.body.querySelectorAll('.' + CONST.CLASS.TODO_CHECK_IMG_OLD);
      for (let i = checkImgList.length - 1; i >= 0; i--) {
        let checkImg = checkImgList[i];
        let label = this.oldPatch.getLabelFromChild(checkImg);
        if (!label || label.children[0] !== checkImg) {
          this.fixCheckbox(checkImg, true);
        }
        //将 image 转换为 i
        this.oldPatch.fixImg(checkImg);
      }
    ...
    fixUserInfo: (label) => {
      let parent = label.parentNode,
        check = this.oldPatch.getCheckImg(label),
        id = check ? check.id : '',
    ...
    getLabelFromChild: (dom) => {
      if (!dom) {
        return null;
      }
      return domUtils.getParentByFilter(dom, (_dom) => {
    ...
    packageTodo: (label) => {
      ...
      let parent = label.parentNode;
      if (parent !== env.body && this.oldPatch.isFirstLabel(label)) {
        //如果 label 是首元素，则直接返回 label 的父元素
        return parent;
      }

      // 如果 label 不是首元素，则 对齐进行打包
      let check = this.oldPatch.getCheckImg(label),
        id = check ? check.id : '',
...
},{"../../config/const":51,"../../libs/base64":56,"../../libs/utils":59}],51:[function(require,module,exports){
/**
 * 内部使用的标准常量.
 */
...
let base64 = require('../libs/base64');

const WizNoteIosCmdProtocol = 'wiznotecmd://';
...
function setOptions(old, newOptions) {
  ...
  for (k in old) {
    if (old.hasOwnProperty(k) && typeof newOptions[k] !== 'undefined') {
      if (/^(editor|reader|callback|timeout|markdownPlugIn|ios|pc|table)$/.test(k)) {
...
function setClientType(type, env) {
  ...
  if (type.indexOf('windows') > -1) {
    ...
    env.client.sendCmdToWiznote = (cmd, options) => {
    ...
    env.client.sendCmdToWiznote = (cmd, options) => {
      ...
      let tmpTag = env.doc.createElement(CONST.TAG.TMP_TAG);
      let iframe = env.doc.createElement('iframe');
    ...
    let ua = env.win.navigator.userAgent;
    if (!/Mac/i.test(ua)) {
...
function getDefaultEnv(dependencyUrl, version) {
  let GlobalEvent = {};
  return {
    options: {
      document: null,
    ...
    dependency: {
      files: {
        css: {
          fonts: `${dependencyUrl}/fonts.css?v=${version}`,
          github2: `${dependencyUrl}/github2.css?v=${version}`,
          wizToc: `${dependencyUrl}/wizToc.css?v=${version}`
        },
        cursor: {
          formatPainter: `${dependencyUrl}/format-painter-wiz.cur?v=${version}`,
        },
        js: {
          codemirror: `${dependencyUrl}/codemirror/codemirror.js?v=${version}`,
          cmActiveLine: `${dependencyUrl}/codemirror/addon/selection/active-line.js?v=${version}`,
          cmMatchBrackets: `${dependencyUrl}/codemirror/addon/edit/matchbrackets.js?v=${version}`,
          cmMode: `${dependencyUrl}/codemirror/mode/mode.js?v=${version}`,
          flowchart: `${dependencyUrl}/flowchart.js?v=${version}`,
          jquery: `${dependencyUrl}/jquery-1.11.3.js?v=${version}`,
          mathJax: `${dependencyUrl}/mathjax/MathJax.js?config=TeX-AMS-MML_SVG&v=${version}`,
          prettify: `${dependencyUrl}/prettify.js?v=${version}`,
          raphael: `${dependencyUrl}/raphael.js?v=${version}`,
          sequence: `${dependencyUrl}/sequence-diagram.js?v=${version}`,
          underscore: `${dependencyUrl}/underscore.js?v=${version}`
        },
      },
      css: {
        fonts: ['fonts'],
...
const ENV = function (_options, version) {
  let dependencyUrl = _options.dependencyUrl.replace(/\\/g, '/');
  let env = getDefaultEnv(dependencyUrl, version);

  setOptions(env.options, _options);
  env.readonly = null;

  let doc = env.options.document || window.document;
  let user;
  ...
  if (env.options.container === env.doc ||
    env.options.container === env.body) {
    env.options.container = null;
    env.options.noFrame = true;
  ...
  if (env.options.userNameEncoder === 'base64') {
    ...
    if (env.options.userData) {
      for (let i = 0; i < env.options.usersData.length; i++) {
...
},{"../libs/base64":56,"./const":51}],53:[function(require,module,exports){
// 专门用于等待加载控制 callback
const LoadController = function () {
  ...
  this.callback = () => {
    ...
    for (let i = callbackList.length - 1; i >= 0; i--) {
      let cb = callbackList.splice(i, 1)[0];
      cb();
    }
    callbackList = [];
  }
...
(function () {
  ...
  var inlineTags = new RegExp(['^(<\\/?(a|abbr|acronym|applet|area|b|basefont|',
    ...
    'sub|sup|textarea|time|tt|u|var|wbr|ul|ol|li)[^>]*>|',
    '<(br)\\s?\\/?>)$'].join(''), 'i');
...
function makeCallback(doc, loadFiles, callback) {
    let count = 0, max = loadFiles.length;
...
let dependLoader = {
    ...
    loadJs: function (doc, loadFiles, callback) {
        let cb = makeCallback(doc, loadFiles, callback);
        cb();
    },
...
},{"../config/const":51,"./scriptLoader":58}],58:[function(require,module,exports){
/**
 * 用于加载 js
 * options 是数组，值有
 *  字符串：js地址
...
let scriptLoader = {
    ...
    load: function (doc, options, callback) {
        ...
        let i, j, s, c, id = (new Date()).valueOf(), allLoaded = true;
        for (i = 0, j = options.length; i < j; i++) {
            if (typeof (options[i]) === "string") {
                ...
                let jsUrl = options[i].link, jsId = createJsId(options[i].id), jsVersion = options[i].version;
                if (window.localStorage) {
                    let jsInfo = JSON.parse(localStorage.getItem(jsId));
...
const utils = {
  ...
  isSameAmendTime: function (time1, time2) {
    ...
    let t1 = utils.getDateForTimeStr(time1),
  ...
  rgb2Hex: function (str) {
    ...
    let rgb = str.replace(/.*\((.*)\)/ig, '$1').split(',');
    if (rgb.length < 3) {
      return '';
    }
...
},{"../config/const":51}],60:[function(require,module,exports){
/**
...
let xssUtils = {
  xssFilter: (function () {
    ...
    let fileName = decodeURIComponent(location.pathname),
    ...
    let hrefReg = /^((file|wiz(note)?):\/\/)|(index_files\/)|(data:image\/(?!svg))/;
    let hrefFileSrc = '#';
    let hrefFileReg = new RegExp('^(' + hrefFileSrc.escapeRegex() +
      (fileName ? ('|' + fileName.escapeRegex()) : '') + ')', 'i');

    let tagAttrReg = /^(id|class|name|style|data|width|height)/i;

    let xss = new filterXSS.FilterXSS({
      onIgnoreTag: function (tag, html, options) {
        //针对白名单之外的 tag 处理
        ...
        let x = filterXSS.parseAttr(html, function (name, value) {
...
},{"./xss":60}],62:[function(require,module,exports){
let LANG = {};
...
const Lang = function (type, env) {
  let userLangType = 'en';
  let userLang = {};

  // 默认情况根据浏览器判断语言
  if (!type) {
    let win = env.win || window;
...
const WizDocument = require('./WizDocument');
...
let wizEditor = {
  init: (options) => {
    ...
    wizDocument = new WizDocument(options, (documentCore) => {
      core = documentCore;
      if (coreCallback) {
    ...
    wizEditor.removeFormat = (removeAll, isRemoveColor, isRemoveAllStyles) => {
      wizDocument.editor.removeFormat(removeAll, isRemoveColor, isRemoveAllStyles);
      return wizEditor;
    };
    wizEditor.removeStyleById = (id) => {
    ...
    wizEditor.addListener = (eName, fun) => {
      wizDocument.editor.addListener(eName, fun);
      return wizEditor;
    };
    wizEditor.removeListener = (eName, fun) => {
      wizDocument.editor.removeListener(eName, fun);
      return wizEditor;
    };
    wizEditor.triggerListener = (eName, params) => {
    ...
    wizEditor.nightMode.on = wizDocument.editor.nightMode.on;
    wizEditor.nightMode.off = wizDocument.editor.nightMode.off;
    ...
    wizEditor.insertCustomStyle = (id, customCss, isTemp) => {
      wizDocument.insertCustomStyle(id, customCss, isTemp);
      return wizEditor;
    };
    wizEditor.insertDefaultStyle = (onlyReplace, customCss) => {
      wizDocument.insertDefaultStyle(onlyReplace, customCss);
      return wizEditor;
    };

    wizReader.on = (options, callback) => {
      wizDocument.reader.on(options, callback);
      return wizReader;
    };
    wizReader.off = (options, callback) => {
      wizDocument.editor.on(options, callback);
      return wizReader;
    };
    ...
    wizReader.insertDefaultStyle = (onlyReplace, customCss) => {
      wizDocument.insertDefaultStyle(onlyReplace, customCss);
      return wizReader;
    };
    wizReader.removeStyleById = (id) => {
      wizDocument.removeStyleById(id);
      return wizReader;
    };
    ...
    wizReader.nightMode.on = wizDocument.reader.nightMode.on;
    wizReader.nightMode.off = wizDocument.reader.nightMode.off;
...
const setCoreCallback = function (_coreCallback) {
...
},{"./WizDocument":5}],64:[function(require,module,exports){
const WizEditor = require('./wizEditor');

// let wizEditor = WizEditor.wizEditor;
// let wizReader = WizEditor.wizReader;
let setCoreCallback = WizEditor.setCoreCallback;
