...
function setWizTemplateStatus (bEditing) {
    var f = window.document.getElementById('ueditor_0');
    if (bEditing) {
        f.contentWindow.WizTemplate.on(false).setContenteditable(true).focus();
    } else {
        f.contentWindow.WizTemplate.off().setContenteditable(false);
    }
    wiz_EditorInnerHtml = editor.document.body.innerHTML;
    wiz_EditorBodyClass = editor.document.body.className;    
}
...
function initWizEnvironment (bEditing) {
    ...
    if (f.contentWindow.WizTemplate) {            
    	...
    	var templateJs = WizEditor.getWizTemplateJsFile();
    	var wizJsDocument = loadSingleJs(editor.document, templateJs);
...
function setWebWizEditorStatus (bEditing) {
	// var f = window.document.getElementById('ueditor_0');
	// if (bEditing) {
		
	// } else {
	// 	f.contentWindow.WizEditor.off();
	// }
}
...
function initWebWizEditor (bEditing) {
	...
	if (f.contentWindow.WizEditor) {
    	...
    	var wizEditorDocument = loadSingleJs(editor.document, webEditor);
...
function setWizReaderStatus (bEditing) {
	var f = window.document.getElementById('ueditor_0');
	if (bEditing) {
		f.contentWindow.WizReader.off();
	} else {
		f.contentWindow.WizReader.on({noAmend: false});
	}
}
...
function renderMarkdown() {    
	if (WizEditor.canRenderMarkdown()) {
		var f = window.document.getElementById('ueditor_0');
		if (!f.contentWindow.WizReader) {
			console.log("wizreader is not ready, can not render");
			return;
		}
...
function updateCurrentNoteHtml()
{
    if (m_currentGUID == WizEditor.currentNoteGUID())
    {
        ...
        wiz_bodyStyle = WizEditor.currentBodyStyle();
    }
}
...
function updateCustomCss() {
	...
	if (!WizEditor.shouldAddCustomCSS())
        return;
	//
	var strStyle = 'html,body{font-size:15px}body{font-family:Helvetica,"Hiragino Sans GB","微软雅黑","Microsoft YaHei UI",SimSun,SimHei,arial,sans-serif;line-height:1.6;margin:0;padding:20px 36px;padding:1.33rem 2.4rem}h1,h2,h3,h4,h5,h6{margin:20px 0 10px;margin:1.33rem 0 .667rem;padding:0;font-weight:bold}h1{font-size:21px;font-size:1.4rem}h2{font-size:20px;font-size:1.33rem}h3{font-size:18px;font-size:1.2rem}h4{font-size:17px;font-size:1.13rem}h5{font-size:15px;font-size:1rem}h6{font-size:15px;font-size:1rem;color:#777;margin:1rem 0}div,p,ul,ol,dl,li{margin:0}blockquote,table,pre,code{margin:8px 0}ul,ol{padding-left:32px;padding-left:2.13rem}blockquote{border-left:4px solid #ddd;padding:0 12px;padding:0 .8rem;color:#aaa}blockquote>:first-child{margin-top:0}blockquote>:last-child{margin-bottom:0}img{border:0;max-width:100%;height:auto !important;margin:2px 0}table{border-collapse:collapse;border:1px solid #bbb}td{padding:4px 8px;border-collapse:collapse;border:1px solid #bbb}@media screen and (max-width:660px){body{padding:20px 18px;padding:1.33rem 1.2rem}}@media only screen and (-webkit-max-device-width:1024px),only screen and (-o-max-device-width:1024px),only screen and (max-device-width:1024px),only screen and (-webkit-min-device-pixel-ratio:3),only screen and (-o-min-device-pixel-ratio:3),only screen and (min-device-pixel-ratio:3){html,body{font-size:17px}body{line-height:1.7;padding:.75rem .9375rem;color:#353c47}h1{font-size:2.125rem}h2{font-size:1.875rem}h3{font-size:1.625rem}h4{font-size:1.375rem}h5{font-size:1.125rem}h6{color:inherit}ul,ol{padding-left:2.5rem}blockquote{border-left:4px solid #c8d4e8;padding:0 .9375rem;color:#b3c2dd}}';	//
...
function resetEditorBodyClassName () {
	editor.document.body.removeAttribute('class');
    ...
    for (var i = attrs.length - 1; i >= 0; i--) {        
        ...
        var attrString = attr[1].replace('"', ''); //attrValue.join('');
        if (attr[0] == 'class' && attrString != '') {
        	editor.document.body.setAttribute(attr[0], attrString);
        }             
    }
}
...
function wizEditorGetContentHtml () {
	var f = window.document.getElementById('ueditor_0');
	if (f.contentWindow.WizEditor) {
		var contentHtml = f.contentWindow.WizEditor.getContentHtml();
		if (f.contentWindow.WizTemplate) {
			contentHtml = f.contentWindow.WizTemplate.hideTemplateFormHtml(contentHtml);
		}
		return contentHtml;
	}
...
function wizIsContentsChanged () {
    ...
    var bodyClass = editor.document.body.className;

    var keepSame = (html == wiz_EditorInnerHtml && bodyClass == wiz_EditorBodyClass);
    return !keepSame;
}

function wizIsTemplate () {
    var f = window.document.getElementById('ueditor_0');
    if (f.contentWindow.WizTemplate) {
        return f.contentWindow.WizTemplate.getTemplate() != null;
    }
    return false;
}